...
rmgjs.getNetwork = function() {
  return rmgjs.networks[common.getRmgNetwork()];
};

rmgjs.makeRandomKey = function() {
  return rmgjs.ECPair.makeRandom({ network: rmgjs.getNetwork() });
};

HDNode.prototype.getKey = function(network) {
  network = network || rmgjs.getNetwork();
  var k = this.keyPair;
  var result = new rmgjs.ECPair(k.d, k.d ? null : k.Q, { network: network, compressed: k.compressed });
...
var deriveFast = function (hdnode, index) {
  ...
  var isHardened = index >= rmgjs.HDNode.HIGHEST_BIT;
  if (isHardened) {
  ...
  var keyPair = new rmgjs.ECPair(null, Ki, {network: hdnode.network});
  hd = new rmgjs.HDNode(keyPair, IR);
...
if (secp256k1) {
  rmgjs.ECPair.prototype.sign = function (hash) {
    if (!this.d) {
      throw new Error('Missing private key');
    }
    var sig = secp256k1.sign(hash, this.d.toBuffer(32)).signature;
    return rmgjs.ECSignature.fromDER(secp256k1.signatureExport(sig));
  };

  rmgjs.ECPair.prototype.verify = function (hash, signature) {
    signature = new rmgjs.ECSignature(signature.r, signature.s);
