...
speakeasy.hotp = function(options) {
  ...
  //var hmac1 = crypto.createHmac('sha1', new Buffer(key));
  var hmac = new sjcl.misc.hmac(sjcl.codec.utf8String.toBits(key), sjcl.hash.sha1);
  ...
  var counter_buffer = octet_array;

  // update hmac with the counter
  //hmac1.update(counter_buffer);
  var hash = hmac.mac(sjcl.codec.bytes.toBits(counter_buffer));

  // get the digest in hex format
  //var digest1 = hmac1.digest('hex');
  var digest = sjcl.codec.hex.fromBits(hash);

  // convert the result to an array of bytes
  var digest_bytes = Crypto.util.hexToBytes(digest);
