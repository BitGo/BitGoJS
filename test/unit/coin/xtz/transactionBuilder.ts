...
describe('Tezos Transaction builder', function() {
  ...
  describe('should parse', () => {
    it('an unsigned init transaction', async () => {
      ...
      Object.keys(tx.getIndexByTransactionType()).length.should.equal(1);
    });
  });
  ...
  describe('should sign', () => {
    ...
    it('an init transaction and reveal the source', async () => {
      ...
      const indexesByTransactionType = tx.getIndexByTransactionType();
    ...
    it('a send transaction to multiple destinations', async () => {
      ...
      const firstTransferSignatures = tx.getTransferSignature(indexesByTransactionType.transaction[0]);
      ...
      const secondTransferSignatures = tx.getTransferSignature(indexesByTransactionType.transaction[1]);
