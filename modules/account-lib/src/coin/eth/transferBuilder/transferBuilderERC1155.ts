  ...
  signAndBuild(): string {
    if (this.hasMandatoryFields()) {
      ...
      if (this._tokenIds.length === 1) {
        ...
        const contractCall = new ContractCall('safeTransferFrom', ERC1155SafeTransferTypes, values);
        this._data = contractCall.serialize();
      } else {
        const values = [this._fromAddress, this._toAddress, this._tokenIds, this._values, ''];
        const contractCall = new ContractCall('safeBatchTransferFrom', ERC1155BatchTransferTypes, values);
        this._data = contractCall.serialize();
      }
