...
describe('Eos unlink auth builder', () => {
  ...
  describe('setter validation', async () => {
    it('should fail with an invalid account name', async () => {
      const authBuilder = builder.unlinkAuthActionBuilder('eosio', [sender.name]);
      should.throws(
  ...
  describe('build transaction', () => {
    it('should build a transaction', async () => {
      builder
        ...
        .sign({ key: sender.privateKey });
      builder
        .unlinkAuthActionBuilder('eosio', [sender.name])
        .account(sender.name)
        .code(sender.name)
      ...
      should.deepEqual(
        ...
        EosResources.unlinkAuthTransaction.serializedTransaction,
      );
    });
    ...
    it('should build a trx from a raw transaction', async () => {
      builder.testnet();
      builder.from(EosResources.unlinkAuthTransaction.serializedTransaction);
      const tx = await builder.build();
      const json = await tx.toJson();
      should.deepEqual(json.actions[0].data.account, sender.name);
      should.deepEqual(json.actions[0].data.code, sender.name);
