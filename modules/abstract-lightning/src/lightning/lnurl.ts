...
export class LnurlProcessor {
   ...
   */
  async decodeLnurlPay(lnurl: string): Promise<DecodedLnurlPayRequest> {
    try {
   ...
   */
  parseLightningInvoice(invoiceStr: unknown): ParsedLightningInvoice {
    if (typeof invoiceStr !== 'string') {
      throw new Error('invoice is malformed');
    }

    // For signet invoices that start with lntbs
    const network = this.getNetworkForInvoice(invoiceStr);
   ...
   */
  private getNetworkForInvoice(invoice: string) {
   ...
   */
  validateLnurlInvoice(invoice: ParsedLightningInvoice, millisatAmount: bigint, metadata: string): void {
   ...
   */
  async fetchLnurlPayInvoice(params: LnurlPayParams): Promise<LnurlPayInvoiceResponse> {
    try {
      ...
      const parsedInvoice = this.parseLightningInvoice(invoiceResponse.pr);
      this.validateLnurlInvoice(parsedInvoice, millisatAmount, metadata);
  ...
  async processLnurlPayment(
    ...
    try {
      ...
      const payResponse = await this.decodeLnurlPay(lnurl);
      ...
      const invoiceResponse = await this.fetchLnurlPayInvoice({
