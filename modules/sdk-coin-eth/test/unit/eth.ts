    ...
    describe('Build Unsigned Sweep for Self-Custody Cold Wallets (MPCv2)', function () {
      ...
      it('should generate an unsigned sweep without derivation seed', async function () {
        ...
        const sweepResult = await (basecoin as AbstractEthLikeNewCoins).recover({
          commonKeyChain: params.commonKeyChain, // Box A Data
        ...
        const otuput = sweepResult as UnsignedSweepTxMPCv2;
        otuput.should.have.property('txRequests');
        otuput.txRequests.should.have.length(1);
        otuput.txRequests[0].should.have.property('transactions');
        otuput.txRequests[0].transactions.should.have.length(1);
        otuput.txRequests[0].should.have.property('walletCoin');
        otuput.txRequests[0].transactions.should.have.length(1);
        otuput.txRequests[0].transactions[0].should.have.property('unsignedTx');
        otuput.txRequests[0].transactions[0].unsignedTx.should.have.property('serializedTxHex');
        otuput.txRequests[0].transactions[0].unsignedTx.should.have.property('signableHex');
        otuput.txRequests[0].transactions[0].unsignedTx.should.have.property('derivationPath');
        otuput.txRequests[0].transactions[0].unsignedTx.should.have.property('feeInfo');
        otuput.txRequests[0].transactions[0].unsignedTx.should.have.property('parsedTx');
        const parsedTx = otuput.txRequests[0].transactions[0].unsignedTx.parsedTx as { spendAmount: string };
        parsedTx.should.have.property('spendAmount');
        (otuput.txRequests[0].transactions[0].unsignedTx.parsedTx as { outputs: any[] }).should.have.property(
          'outputs'
        );
      });
