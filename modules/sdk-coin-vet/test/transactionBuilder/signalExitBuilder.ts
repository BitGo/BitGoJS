...
import { TransactionBuilderFactory, Transaction, SignalExitTransaction } from '../../src/lib';
import should from 'should';
import { SIGNAL_EXIT_METHOD_ID, VALIDATOR_REGISTRATION_STAKER_CONTRACT_ADDRESS_TESTNET } from '../../src/lib/constants';
import EthereumAbi from 'ethereumjs-abi';
import utils from '../../src/lib/utils';

describe('VET Signal Exit Transaction', function () {
  ...
  const createBasicTxBuilder = () => {
    const txBuilder = factory.getSignalExitBuilder();
  ...
  it('should build a signal exit transaction', async function () {
    ...
    tx.should.be.instanceof(SignalExitTransaction);

    const signalExitTx = tx as SignalExitTransaction;
    signalExitTx.stakingContractAddress.should.equal(VALIDATOR_REGISTRATION_STAKER_CONTRACT_ADDRESS_TESTNET);
    signalExitTx.validator.should.equal(validatorAddress);

    signalExitTx.clauses.length.should.equal(1);
    should.exist(signalExitTx.clauses[0].to);
    signalExitTx.clauses[0].to?.should.equal(VALIDATOR_REGISTRATION_STAKER_CONTRACT_ADDRESS_TESTNET);
    signalExitTx.clauses[0].value?.should.equal('0');

    should.exist(signalExitTx.clauses[0].data);
    const txData = signalExitTx.clauses[0].data;
    txData.should.startWith(SIGNAL_EXIT_METHOD_ID);
    ...
    signalExitTx.recipients.length.should.equal(1);
    signalExitTx.recipients[0].address.should.equal(VALIDATOR_REGISTRATION_STAKER_CONTRACT_ADDRESS_TESTNET);
    signalExitTx.recipients[0].amount.should.equal('0');
  });

  it('should produce the correct method ID 0xcb652cef', function () {
    const methodName = 'signalExit';
    const types = ['address'];
    const method = EthereumAbi.methodID(methodName, types);
    const methodId = '0x' + method.toString('hex');
    methodId.should.equal('0xcb652cef');
  });
  ...
  it('should serialize and deserialize round-trip', async function () {
    ...
    const tx2 = txBuilder2.transaction as SignalExitTransaction;
    tx2.should.be.instanceof(SignalExitTransaction);
  ...
  describe('decodeSignalExitData', function () {
    it('should correctly decode signal exit transaction data', function () {
      const methodName = 'signalExit';
      const types = ['address'];
      const params = [validatorAddress];
      const method = EthereumAbi.methodID(methodName, types);
      ...
      const decodedData = utils.decodeSignalExitData(encodedData);
