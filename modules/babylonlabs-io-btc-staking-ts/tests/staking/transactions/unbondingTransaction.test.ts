import { unbondingTransaction } from "../../../src";
  ...
  ({ networkName, network, datagen }) => {
    describe.each(Object.values(datagen))(
      ...
      (dataGenerator) => {
        ...
        const { stakingTx, stakingAmountSat } =
        ...
        describe(`${networkName} - `, () => {
          ...
          it("should return psbt for unbonding transaction", () => {
            const unbondingFee = dataGenerator.getRandomIntegerBetween(
              1,
            ...
            // check output value
            expect(transaction.outs[0].value).toBe(
              stakingAmountSat - unbondingFee,
            );
