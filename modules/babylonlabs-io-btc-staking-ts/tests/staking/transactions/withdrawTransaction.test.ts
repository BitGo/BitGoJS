...
import {
  KeyPair,
  SlashingType,
  StakingDataGenerator,
} from "../../helper/datagen/base";
...
const setupTestData = (
  ...
  const { stakingTx, stakerInfo, params, stakingAmountSat } =
    dataGenerator.generateRandomStakingTransaction(network, 1, stakerKeyPair);
    ...
    describe.each(Object.values(datagen))(
      ...
      (dataGenerator) => {
        ...
        beforeEach(() => {
          ...
          testData = setupTestData(network, dataGenerator);
        });
        ...
        it(`${networkName} - should throw if not enough funds to cover fees`, () => {
          const { stakingTx, stakerInfo, keyPair, stakingAmountSat } =
            dataGenerator.generateRandomStakingTransaction(network);
          const stakingScripts =
            dataGenerator.generateMockStakingScripts(keyPair);
          const unitWithdrawTxFee = getWithdrawTxFee(1);
          const feeRateToExceedAmount =
            Math.ceil(stakingAmountSat / unitWithdrawTxFee) * 10;

          expect(() =>
            withdrawEarlyUnbondedTransaction(
              {
                unbondingTimelockScript: stakingScripts.unbondingTimelockScript,
        ...
        it(`${networkName} - should throw if output is less than dust limit`, () => {
          ...
          const amountToNotCoverDustLimit =
            BTC_DUST_SAT + estimatedFee - 1 + params.unbondingFeeSat;

          const { stakingTx, stakerInfo, stakingInstance, stakingTxFee } =
        ...
        it(`${networkName} - should create the withdraw slashing transactions successfully`, () => {
          const slashingTypes: SlashingType[] = [
            "earlyUnbonded",
            "timelockExpire",
          ];
          slashingTypes.forEach((type) => {
            const { tx: slashingTx } =
              dataGenerator.generateSlashingTransaction(
                ...
                {
                  minSlashingTxFeeSat:
                    testData.params.slashing?.minSlashingTxFeeSat!!,
                  slashingPkScriptHex:
                    testData.params.slashing?.slashingPkScriptHex!!,
            ...
            const outputIndex = findMatchingTxOutputIndex(
              slashingTx,
              deriveSlashingOutput(testData.stakingScripts, network)
                .outputAddress,
              network,
            );
            ...
            const remainingAmout =
              slashingTx.outs[outputIndex].value -
              getWithdrawTxFee(DEFAULT_TEST_FEE_RATE);
            expect(psbt.psbt.txOutputs[0].value).toBe(
              Math.floor(remainingAmout),
            );
          });
        });
      },
