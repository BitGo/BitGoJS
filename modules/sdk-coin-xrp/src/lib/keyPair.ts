...
const DEFAULT_SEED_SIZE_BYTES = 32; // WP -> 32 but old SDK > 512/8=16;
...
export class KeyPair extends Secp256k1ExtendedKeyPair {
  ...
  signMessage(message: string): Buffer {
    ...
    const signature = xrpUtils.signString(messageToSign, prv);
  ...
  verifySignature(message: string, signature: Buffer): boolean {
    ...
    return xrpUtils.verifySignature(messageToVerify, pubKey, signature.toString('hex'));
  }
}
