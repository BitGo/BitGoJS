...
import { EcdsaTypes, EcdsaRangeProof, EcdsaPaillierProof } from '@bitgo/sdk-lib-mpc';
...
export const optionalDeps = {
  ...
  get EthLikeTx(): typeof EthLikeTxLib {
    try {
  ...
  get EthLikeCommon(): typeof EthLikeCommon {
    try {
...
export abstract class AbstractEthLikeNewCoins extends BaseCoin {
   ...
   */
  private static getEthLikeCommon(eip1559?: EIP1559, replayProtectionOptions?: ReplayProtectionOptions) {
    ...
    const defaultHardfork = !!eip1559 ? 'london' : optionalDeps.EthLikeCommon.Hardfork.TangerineWhistle;

    let ethLikeCommon;
    // if replay protection options are set, override the default common setting
    if (replayProtectionOptions) {
      const customChainName = AbstractEthLikeNewCoins.getCustomChainName(replayProtectionOptions.chain as number);
      const customChain = optionalDeps.EthLikeCommon.CustomChain[customChainName];
  ...
  static buildTransaction(
    ...
    const unsignedEthTx = !!params.eip1559
      ? optionalDeps.EthLikeTx.FeeMarketEIP1559Transaction.fromTxData(
          {
            ...baseParams,
      ...
      : optionalDeps.EthLikeTx.Transaction.fromTxData(
          {
            ...baseParams,
  ...
  async queryAddressBalance(address: string): Promise<any> {
    ...
    if (!result || !result.result || isNaN(result.result)) {
      throw new Error(`Could not obtain address balance for ${address} from explorer, got: ${result.result}`);
    }
  ...
  async queryAddressTokenBalance(tokenContractAddress: string, walletContractAddress: string): Promise<any> {
    ...
    if (!result || !result.result || isNaN(result.result)) {
      throw new Error(
        `Could not obtain token address balance for ${tokenContractAddress} from explorer, got: ${result.result}`
      );
    }
    return new optionalDeps.ethUtil.BN(result.result, 10);
  }
  ...
  async getAddressNonce(address: string): Promise<number> {
    ...
    if (!result || !Array.isArray(result.result)) {
      throw new Error('Unable to find next nonce from explorer, got: ' + JSON.stringify(result));
    }
  ...
  async recoveryBlockchainExplorerQuery(query: Record<string, string>): Promise<any> {
    ...
    switch (this._staticsCoin.name) {
      case 'polygon':
        token = common.Environments[this.bitgo.getEnv()].polygonscanApiToken;
        break;
    }
  ...
  async signFinalEthLike(params: SignFinalOptions): Promise<FullySignedTransaction> {
    const signingKey = new EthLikeKeyPair({ prv: params.prv }).getKeys().prv;
  ...
  async signTransaction(params: SignTransactionOptions): Promise<SignedTransaction> {
    ...
    txBuilder.transfer().key(new EthLikeKeyPair({ prv: params.prv }).getKeys().prv!);
  ...
  validateRecoveryParams(params: RecoverOptions): void {
    ...
    if (params.walletPassphrase === undefined && !params.userKey.startsWith('xpub') && !params.isTss) {
  ...
  async formatForOfflineVault(
    ...
    const response: OfflineVaultTxInfo = {
      ...
      amount: txInfo.recipient.amount.toString(),
  ...
  protected async recoverTSS(params: RecoverOptions): Promise<RecoveryInfo | OfflineVaultTxInfo> {
    ...
    const [backupKeyAddress, userKeyCombined, backupKeyCombined] = ((): [
      ...
      if (isUnsignedSweep) {
        const backupKeyPair = new EthLikeKeyPair({ pub: backupPrivateOrPublicKeyShare });
        ...
        const backupKeyPair = new EthLikeKeyPair({ pub: backupKeyCombined.xShare.y });
  ...
  async recoverEthLike(params: RecoverOptions): Promise<RecoveryInfo | OfflineVaultTxInfo> {
    ...
    if (isUnsignedSweep) {
      ...
      const keyPair = new EthLikeKeyPair({ prv: backupPrv });
    ...
    txBuilder.transfer().key(new EthLikeKeyPair({ prv: userKey }).getKeys().prv as string);
  ...
  validateEvmBasedRecoveryParams(params: RecoverOptions): void {
    if (params.bitgoFeeAddress === undefined || !this.isValidAddress(params.bitgoFeeAddress)) {
  ...
  async recoverEthLikeTokenforEvmBasedRecovery(
    ...
    if (params.walletPassphrase) {
      ...
      Object.assign(response, halfSignedTxn);
  ...
  protected async recoverEthLikeforEvmBasedRecovery(
    ...
    if (params.walletPassphrase) {
      ...
      const keyPair = new EthLikeKeyPair({ prv: userKeyPrv });
    ...
    if (bitgoFeePercentage > 0) {
      if (bitgoDestinationAddress === undefined || !this.isValidAddress(bitgoDestinationAddress)) {
  ...
  preCreateBitGo(params: PrecreateBitGoOptions): void {
    ...
    if (params.enterprise === undefined && params.newFeeAddress === undefined) {
      throw new Error(
    ...
    if (params.enterprise !== undefined && typeof params.enterprise !== 'string') {
      throw new Error(`enterprise should be a string - got ${params.enterprise} (type ${typeof params.enterprise})`);
    }

    if (params.newFeeAddress !== undefined && typeof params.newFeeAddress !== 'boolean') {
      throw new Error(
  ...
  async validateHopPrebuild(
    ...
    const builtHopTx = optionalDeps.EthLikeTx.TransactionFactory.fromSerializedData(optionalDeps.ethUtil.toBuffer(tx));
  ...
  async getExtraPrebuildParams(buildParams: BuildOptions): Promise<BuildOptions> {
    if (
      buildParams.hop !== undefined &&
      buildParams.hop &&
      buildParams.wallet !== undefined &&
      buildParams.recipients !== undefined &&
      buildParams.walletPassphrase !== undefined
    ) {
  ...
  async postProcessPrebuild(params: TransactionPrebuild): Promise<TransactionPrebuild> {
    if (params.hopTransaction !== undefined && params.wallet !== undefined && params.buildParams !== undefined) {
  ...
  async isWalletAddress(params: VerifyEthAddressOptions): Promise<boolean> {
    ...
    if (baseAddress === undefined || !this.isValidAddress(baseAddress)) {
      throw new InvalidAddressError('invalid base address');
    }

    if (!Array.isArray(coinSpecific) && coinSpecific instanceof Object === false) {
  ...
  async verifyTransaction(params: VerifyEthTransactionOptions): Promise<boolean> {
    ...
    if (txParams.hop && txPrebuild.hopTransaction) {
      ...
      const decodedHopTx = optionalDeps.EthLikeTx.TransactionFactory.fromSerializedData(
      ...
      if (
        this.isValidAddress(txParams.recipients[0].address) &&
