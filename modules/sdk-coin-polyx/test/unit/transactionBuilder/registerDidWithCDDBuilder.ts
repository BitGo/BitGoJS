...
describe('Polyx Register DID with CDD builder Builder - Testnet', () => {
  ...
  describe('build registerDidWithCDD transaction', () => {
    ...
    it('should build an unsigned cdd transaction', async () => {
      builder
        .to({ address: receiver.address })
        .sender({ address: sender.address })
        .validity({ firstValid: 3933, maxDuration: 64 })
...
describe('Polyx Register DID with CDD builder Builder - Mainnet', () => {
  let builder: RegisterDidWithCDDBuilder;
  ...
  describe('build registerDidWithCDD transaction on mainnet', () => {
    it('should build a register did with cdd transaction', async () => {
      builder
        .to({ address: receiver.address })
        .sender({ address: sender.address })
    ...
    it('should build from raw signed tx', async () => {
      ...
      const txJson = tx.toJson();
      should.deepEqual(txJson.amount, '0');
      should.deepEqual(txJson.to, '2EdivKhDvxWSYPPHnkHUDMZa27iHQ8rKnYofHf3AjgB6ovDU');
      should.deepEqual(txJson.sender, '2EVjkYBtLznJZEagH6FC6vsS1trb25DwRZYEToUtjPCAGJjU');
      should.deepEqual(txJson.blockNumber, 3933);
      should.deepEqual(txJson.referenceBlock, '0x149799bc9602cb5cf201f3425fb8d253b2d4e61fc119dcab3249f307f594754d');
      should.deepEqual(txJson.genesisHash, mainGenesisHash);
      should.deepEqual(txJson.specVersion, Number(testnetMaterial.specVersion));
    ...
    it('should build from raw unsigned tx', async () => {
      ...
      should.deepEqual(txJson.amount, '0');
      should.deepEqual(txJson.to, '2EdivKhDvxWSYPPHnkHUDMZa27iHQ8rKnYofHf3AjgB6ovDU');
      should.deepEqual(txJson.sender, '2Hh9SU3wwGERBGp1T8BY1T8xp7krH7urC9FsNGSfsyTyED8D');
      should.deepEqual(txJson.blockNumber, 3933);
      should.deepEqual(txJson.referenceBlock, '0x149799bc9602cb5cf201f3425fb8d253b2d4e61fc119dcab3249f307f594754d');
      should.deepEqual(txJson.genesisHash, mainGenesisHash);
      should.deepEqual(txJson.specVersion, Number(testnetMaterial.specVersion));
