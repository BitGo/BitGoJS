...
describe('Tron FreezeBuilder', function () {
  let freezeBuilder: FreezeTransactionBuilder;
  let wrappedBuilder;
  ...
  describe.skip('builder integration', () => {
    it('should be able to deserialize a valid freeze transaction using from method', () => {
      ...
      assert.strictEqual(wrappedBuilder._builder instanceof FreezeTransactionBuilder, true);
    });
  });
  ...
  describe('transaction type', () => {
    it('should set transaction type to StakingActivate', () => {
      assert.strictEqual(freezeBuilder['transactionType'], TransactionType.StakingActivate);
    });
  });
});
