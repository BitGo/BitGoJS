...
describe('Util library should', function () {
  ...
  it('get a pub from a prv', () => {
    ...
    assert.strictEqual(derivedPubHex, pub);
  });
  ...
  it('sign a string', () => {
    ...
    assert.strictEqual(signedString, signed);
  });
  ...
  it('should return transaction data', () => {
    ...
    assert.strictEqual(data.timestamp, UnsignedTransferContractTx.tx.raw_data.timestamp);
    assert.strictEqual(data.expiration, UnsignedTransferContractTx.tx.raw_data.expiration);
  ...
  it('should decode a transfer contract', () => {
    ...
    assert.strictEqual(parsedContract[0].parameter.value.to_address, toAddress);
    assert.strictEqual(parsedContract[0].parameter.value.owner_address, ownerAddress);
    assert.strictEqual(parsedContract[0].parameter.value.amount, amount);
  });
  ...
  it('should decode an AccountPermissionUpdate Contract', () => {
    ...
    const ownerAddress = Utils.getBase58AddressFromHex(value.owner_address);
    assert.strictEqual(parsedTx.ownerAddress, ownerAddress);
    assert.strictEqual(parsedTx.owner.type, 0);
    assert.equal(parsedTx.owner.threshold, 2);
    assert.strictEqual(parsedTx.actives.length, 1);
  ...
  it('should encode and decode data parameters for transfer', () => {
    ...
    assert.strictEqual(decodedData[0], addressHex.toLocaleLowerCase());
    assert.strictEqual(decodedData[1].toString(), amount);
  });

  it('should correctly map testnet and mainnet tokens', () => {
    assert.ok(tokenMainnetContractAddresses.includes('TSSMHYeV2uE9qYH95DqyoCuNCzEL1NvU3S'));
    assert.ok(tokenTestnetContractAddresses.includes('TGkfUshdbAiNj5G1mynp2meq2BfF6XSGPf'));
  });
});
