...
describe('Tron DelegateResource builder', function () {
  const initTxBuilder = () => {
    ...
    builder.setBalance(DELEGATION_BALANCE);
  ...
  describe('should build successfully', () => {
    it('a transaction with all correct inputs', async () => {
      ...
      assert.equal(tx.type, TransactionType.DelegateResource);
      ...
      assert.deepStrictEqual(txJson.raw_data.contract, DELEGATE_RESOURCE_CONTRACT);
    });
    ...
    it('a transaction signed multiple times', async () => {
      ...
      assert.deepStrictEqual(rawData.contract, DELEGATE_RESOURCE_CONTRACT);
  ...
  describe('should validate', () => {
    ...
    it('transaction mandatory fields', async () => {
      const txBuilder = (getBuilder('ttrx') as WrappedBuilder).getDelegateResourceTxBuilder();
