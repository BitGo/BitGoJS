...
import { PARTICIPANTS, BLOCK_HASH, BLOCK_NUMBER, EXPIRATION, WITHDRAW_EXPIRE_UNFREEZE_CONTRACT } from '../../resources';
import { getBuilder } from '../../../src/lib/builder';
import { Transaction, WrappedBuilder } from '../../../src';

describe('Tron WithdrawExpireUnfreeze builder', function () {
  const initTxBuilder = () => {
    const builder = (getBuilder('ttrx') as WrappedBuilder).getWithdrawExpireUnfreezeTxBuilder();
  ...
  describe('should build successfully', () => {
    it('a transaction with correct inputs', async () => {
      ...
      assert.equal(tx.type, TransactionType.StakingWithdraw);
      ...
      assert.deepStrictEqual(txJson.raw_data.contract, WITHDRAW_EXPIRE_UNFREEZE_CONTRACT);
    });
    ...
    it('a transaction signed multiple times', async () => {
      ...
      assert.deepStrictEqual(rawData.contract, WITHDRAW_EXPIRE_UNFREEZE_CONTRACT);
  ...
  describe('should validate', () => {
    ...
    it('transaction mandatory fields', async () => {
      const txBuilder = (getBuilder('ttrx') as WrappedBuilder).getWithdrawExpireUnfreezeTxBuilder();
