...
import { WithdrawBuilder } from '../../src/lib/withdrawBalanceBuilder';
...
describe('Tron WithdrawBuilder', function () {
  let withdrawBuilder: WithdrawBuilder;
  let wrappedBuilder;

  beforeEach(() => {
    wrappedBuilder = getBuilder('ttrx');
    // Get WithdrawBuilder from the wrapped builder
    withdrawBuilder = wrappedBuilder.getWithdrawBuilder();
  });
  ...
  describe.skip('builder integration', () => {
    it('should be able to deserialize a valid withdraw transaction using from method', () => {
      assert.doesNotThrow(() => wrappedBuilder.from(validWithdrawUnsignedTx));
      assert.strictEqual(wrappedBuilder._builder instanceof WithdrawBuilder, true);
    });
  });
});
