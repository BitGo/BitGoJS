...
describe('Tron TransactionBuilder', function () {
  ...
  describe('Transaction builder', () => {
    it('should build an update account tx', async () => {
      ...
      assert.strictEqual(signedTxJson.txID, UnsignedAccountPermissionUpdateContractTx.txID);
      assert.strictEqual(signedTxJson.raw_data_hex, UnsignedAccountPermissionUpdateContractTx.raw_data_hex);
      assert.strictEqual(
    ...
    it('should build an half signed tx', async () => {
      ...
      assert.strictEqual(tx.id, '80b8b9eaed51c8bba3b49f7f0e7cc5f21ac99a6f3e2893c663b544bf2c695b1d');
      assert.strictEqual(tx.type, TransactionType.Send);
    ...
    it('should build the right JSON after is half signed tx', async () => {
      ...
      assert.strictEqual(signedTxJson.txID, UnsignedBuildTransaction.txID);
      assert.strictEqual(signedTxJson.raw_data_hex, UnsignedBuildTransaction.raw_data_hex);
      assert.strictEqual(JSON.stringify(signedTxJson.raw_data), JSON.stringify(UnsignedBuildTransaction.raw_data));
      assert.strictEqual(signedTxJson.signature.length, 1);
      assert.strictEqual(
        signedTxJson.signature[0],
        'bd08e6cd876bb573dd00a32870b58b70ea8b7908f5131686502589941bfa4fdda76b8c81bbbcfc549be6d4988657cea122df7da46c72041def2683d6ecb04a7401'
    ...
    it('should extend an unsigned tx', async () => {
      ...
      assert.notStrictEqual(tx.id, UnsignedBuildTransaction.txID);
      assert.strictEqual(tx.id, '764aa8a72c2c720a6556def77d6092f729b6e14209d8130f1692d5aff13f2503');
      const oldExpiration = UnsignedBuildTransaction.raw_data.expiration;
      assert.strictEqual(tx.validTo, oldExpiration + extendMs);
      assert.strictEqual(tx.type, TransactionType.Send);
      assert.strictEqual(tx.inputs.length, 1);
      assert.strictEqual(tx.inputs[0].address, 'TTsGwnTLQ4eryFJpDvJSfuGQxPXRCjXvZz');
      assert.strictEqual(tx.inputs[0].value, '1718');
      ...
      assert.strictEqual(tx.outputs[0].address, 'TNYssiPgaf9XYz3urBUqr861Tfqxvko47B');
      assert.strictEqual(tx.outputs[0].value, '1718');
      assert.strictEqual(tx.validFrom, 1571811410819);
    });
