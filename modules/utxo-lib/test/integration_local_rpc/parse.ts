...
function runTestParse<TNumber extends number | bigint>(
  ...
  describe(fixtureName, function () {
    ...
    function getValueScaled(value: number, scaleFactor = 1e8): TNumber {
      if (amountType === 'number') {
        return (Number(value) * scaleFactor) as TNumber;
      } else {
    ...
    function getPrevOutputValue(input: InputLookup): TNumber {
      return getValueScaled(getPrevOutput(input).value);
    }
    ...
    it(`recreate from unsigned hex`, function () {
      ...
      fixture.transaction.vout.forEach((o) => {
        txbUnsigned.addOutput(Buffer.from(o.scriptPubKey.hex, 'hex'), getValueScaled(o.value));
      });
