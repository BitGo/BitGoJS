...
import { taproot, HDSigner, Psbt, PsbtTransaction, Stack, Transaction, TxOutput } from '../';
import {
  PartialSig,
  PsbtInputUpdate,
  TapBip32Derivation,
 ...
 */
export function unsign(tx: Transaction<bigint>, prevOuts: TxOutput<bigint>[]): PsbtInputUpdate[] {
  ...
  for (let vin = 0; vin < tx.ins.length; vin++) {
    ...
    if (input.witness && input.witness.length) {
      ...
      if (taproot.VALID_LEAF_VERSIONS.has(witness[witness.length - 1][0] & 0xfe)) {
        ...
        const hashFn = (hashType) => tx.hashForWitnessV0(vin, prevOut.script, prevOut.value, hashType);
    ...
    if (input.script && input.script.length) {
      ...
      if (update.witnessScript) {
        ...
        // P2SH
        const hashFn = (hashType) => tx.hashForSignature(vin, prevOut.script, hashType);
