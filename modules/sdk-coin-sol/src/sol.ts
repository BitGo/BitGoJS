...
const BLIND_SIGNING_TX_TYPES_TO_CHECK = {
  enabletoken: 'AssociatedTokenAccountInitialization',
  closetoken: 'CloseAssociatedTokenAccount',
};
...
export class Sol extends BaseCoin {
  ...
  throwIfMissingAtaClosuresOrReturn(explanation: TransactionExplanation): IAtaClosure[] {
    if (!explanation.ataClosures || explanation.ataClosures.length === 0) {
      throw new Error('Missing tx ata closures data on close token tx prebuild');
    }
    return explanation.ataClosures;
  }

  throwIfMissingCloseTokenConfigOrReturn(txParams: TransactionParams): TokenEnablement[] {
    if (!txParams.closeTokens || txParams.closeTokens.length === 0) {
      throw new Error('Missing close token config');
    }
    return txParams.closeTokens;
  }

  /**
   * Verify that each ATA closure in the prebuild matches the expected wallet owner address.
   * The destination and authority of every close instruction must be the wallet's root address.
   */
  verifyCloseTokenDestination(ataClosuresPrebuild: IAtaClosure[], walletRootAddress: string): void {
    for (const closure of ataClosuresPrebuild) {
      if (closure.destinationAddress !== walletRootAddress) {
        throw new Error(
          `Invalid close token destination: expected ${walletRootAddress}, got ${closure.destinationAddress}`
        );
      }
      if (closure.authorityAddress !== walletRootAddress) {
        throw new Error(
          `Invalid close token authority: expected ${walletRootAddress}, got ${closure.authorityAddress}`
        );
      }
    }
  }
  ...
  async verifyTransaction(params: SolVerifyTransactionOptions): Promise<boolean> {
    ...
    if (txParams.type === 'closetoken' && verificationOptions?.verifyTokenEnablement) {
      this.verifyTxType(txParams.type, explainedTx.type);
      const ataClosuresPrebuild = this.throwIfMissingAtaClosuresOrReturn(explainedTx);
      this.throwIfMissingCloseTokenConfigOrReturn(txParams);

      if (walletRootAddress) {
        this.verifyCloseTokenDestination(ataClosuresPrebuild, walletRootAddress);
      }
    }
  ...
  setCoinSpecificFieldsInIntent(intent: PopulatedIntent, params: PrebuildTransactionWithIntentOptions): void {
    ...
    // Propagate close token config for the closetoken intent type
    if (params.closeTokens) {
      intent.closeTokens = params.closeTokens;
    }
  }
}
