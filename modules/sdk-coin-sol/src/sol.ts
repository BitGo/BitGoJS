...
export class Sol extends BaseCoin {
  ...
  /**
   * Validates that the explained transaction type matches the expected txParams type
   * @param explainedTxType - The transaction type from the explained transaction
   * @param txParamsType - The expected transaction type from txParams
   * @throws Error if types don't match (for known types)
   * @logs Warning for unknown transaction types
   */
  private validateTransactionTypeMatch(explainedTxType: string | undefined, txParamsType: string | undefined): void {
    if (!explainedTxType || !txParamsType) {
      console.warn(`Unknown transaction type - explainedTx: ${explainedTxType}, txParams: ${txParamsType}`);
      return;
    }

    if (explainedTxType === 'Send' && txParamsType !== 'transfer') {
      throw new Error(`Tx type '${explainedTxType}' does not match with expected txParams type '${txParamsType}'`);
    }
  }
  ...
  async verifyTransaction(params: SolVerifyTransactionOptions): Promise<any> {
    ...
    this.validateTransactionTypeMatch(explainedTx.type, txParams.type);
