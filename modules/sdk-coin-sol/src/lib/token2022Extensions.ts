...
import { getMint, TOKEN_2022_PROGRAM_ID, Mint } from '@solana/spl-token';
...
export async function fetchExtensionAccounts(
  ...
  try {
    ...
    // Get the mint data using spl-token's getMint function (only if we have extensions to process)
    let mint: any = null;
    try {
 ...
 * @private
 */
async function processTransferHook(mint: any, mintPubkey: PublicKey, connection: Connection): Promise<AccountMeta[]> {
