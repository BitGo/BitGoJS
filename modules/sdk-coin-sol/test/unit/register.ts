...
import { SolToken } from '../../src/solToken';

describe('SOL Register', function () {
  let bitgo: BitGoAPI;
  ...
  describe('register', function () {
    it('should register base coins and token constructors', function () {
      ...
      assert.ok(registeredNames.includes('sol'));
      assert.ok(registeredNames.includes('tsol'));

      // SOL tokens from statics should be registered
      const solTokenCount = SolToken.createTokenConstructors().length;
      assert.strictEqual(registerSpy.callCount, 2 + solTokenCount);
    });
  });
  ...
  describe('registerWithCoinMap', function () {
    ...
    it('should add dynamic SOL tokens to the global coin map', function () {
      registerWithCoinMap(bitgo, coins);

      // registerToken should have been called for SOL tokens in the coin map
    ...
    it('should not add tokens to the global coin map when coin map has no SOL tokens', function () {
      const limitedCoinMap = coins.filter((coin) => !(coin instanceof SolCoin));

      registerWithCoinMap(bitgo, limitedCoinMap);

      // registerToken should not be called since no SOL tokens are in the map
