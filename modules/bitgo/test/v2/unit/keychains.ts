...
describe('V2 Keychains', function () {
  ...
  describe('Create BitGo Key from OVC JSON', function () {
    ...
    it('Passes the enterprise with correct value to bitgo if provided by caller', async function () {
      const bitgoNock = nock(bgUrl)
        .post('/api/v2/tsol/key', _.matches({ source: 'bitgo', keyType: 'tss' }))
        .reply(200, (uri, body: any) => {
          body.should.have.property('enterprise');
          body.enterprise.should.equal('enterpriseId');
          return bitGoKeyResult;
        });
      const platformOutput = await bitgo
        .coin('tsol')
        .keychains()
        .createTssBitGoKeyFromOvcShares(ovcOutputJson, 'enterpriseId');
      should.equal(platformOutput.bitGoKeyId, bitGoKeyResult.id);
      should.deepEqual(platformOutput.bitGoOutputJsonForOvc, expectedBitGoOutput);
      bitgoNock.isDone();
    });
