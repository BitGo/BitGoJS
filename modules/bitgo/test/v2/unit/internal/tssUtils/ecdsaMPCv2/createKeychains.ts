...
describe('TSS Ecdsa MPCv2 Utils:', async function () {
  ...
  let bitgoGpgPrvKey, bitgoGpgPubKey, userGpgPubKey, backupGpgPubKey: { partyId: number; gpgKey: string };
  ...
  describe('TSS key chains', async function () {
    ...
    it('should create TSS MPCv2 key chains with OVC', async function () {
      const MPCv2Utils = new ECDSAUtils.EcdsaMPCv2Utils(bitgo, baseCoin);
      const OVC1_INDEX = 1;
      const OVC2_INDEX = 2;
      const bitgoSession = new DklsDkg.Dkg(3, 2, 2);
      ...
      // Round 1 BitGo
      const bitgoToOVC1Round1Payload = await MPCv2Utils.keyGenSMCRound1('testId', OVC2ToBitgoRound1Payload);
      ...
      // Round 2 BitGo
      // call bitgo round 2
      const bitgoToOVC1Round2Payload = await MPCv2Utils.keyGenSMCRound2('testId', OVC2ToBitgoRound2Payload);
      ...
      const bitgoToOVC1Round3Payload = await MPCv2Utils.keyGenSMCRound3('testId', OVC1ToBitgoRound3BPayload);
      ...
      let OVC1ToOVC2Round4Payload;
      {
        ...
        OVC1ToOVC2Round4Payload = {
          ...
          wallet: { ...bitgoToOVC1Round3Payload.wallet, state: 10 },
        };
      }
      ...
      const keychains = await MPCv2Utils.keyGenSMCAddOthersKeys(backupCommonKeychain);
