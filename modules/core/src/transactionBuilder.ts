...
exports.createTransaction = function(params) {
  ...
  if (params.feeSingleKeyWIF) {
    const feeSingleKey = bitcoin.ECPair.fromWIF(params.feeSingleKeyWIF, getNetwork());
  ...
  recipients.forEach(function(recipient) {
    if (_.isString(recipient.address)) {
      ...
      if (!!recipient.script) {
        ...
        if (bitcoin.address.toOutputScript(recipient.address, getNetwork()).toString('hex') !== recipient.script) {
  ...
  // The transaction.
  let transaction = new bitcoin.TransactionBuilder(getNetwork());
    ...
    .then(function() {
      ...
      if (shouldComputeBestFee) {
        ...
        if (shouldRecurse) {
          ...
          inputAmount = 0;
          transaction = new bitcoin.TransactionBuilder(getNetwork());
  ...
  const collectOutputs = function() {
    ...
    recipients.forEach(function(recipient) {
      let script;
      if (_.isString(recipient.address)) {
        script = bitcoin.address.toOutputScript(recipient.address, getNetwork());
    ...
    .then(function(result) {
      ...
      extraOutputs.forEach(function(output) {
        output.script = bitcoin.address.toOutputScript(output.address, getNetwork());
