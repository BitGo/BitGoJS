...
describe('Eos:', function () {
  ...
  it('should generate a valid transaction signature', async function() {
    ...
    const keyPair = basecoin.generateKeyPair(seed);
    const hdNode = bitcoin.HDNode.fromBase58(keyPair.prv);
    const privateKey = hdNode.keyPair.toWIF();
    const eosKeypair ={ pub: ecc.privateToPublic(privateKey), prv: privateKey };
    await basecoin.signTransaction({ txPrebuild: tx, keyPair: eosKeypair });
  });
