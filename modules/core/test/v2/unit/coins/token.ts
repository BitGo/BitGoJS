import * as Bluebird from 'bluebird';
import * as should from 'should';
import { BaseCoin } from '../../../../src/v2/baseCoin';

import { TestBitGo } from '../../../lib/test_bitgo';

describe('Virtual Token:', function () {
  let bitgo;

  before(function () {
    bitgo = new TestBitGo({ env: 'test' });
    bitgo.initializeTestVars();
  });

  it('should not instantiate coin interface before loading client constants', function () {
    (() => bitgo.coin('mycrappytoken')).should.throw('Coin or token type mycrappytoken is either not supported, not compiled, or cannot be uniquely identified without explicitly providing its network. \n      Try specifying both the network name and the identifier to determine if this coin is supported.');
  });

  it('should wait for client constants before instantiating coin', async function () {
    const promise = bitgo.token('terc');
    should(promise).be.instanceOf(Bluebird.Promise);
    const erc = await promise;
    erc.should.be.instanceOf(BaseCoin);
    erc.type.should.equal('terc');
    erc.tokenContractAddress.should.equal('0x945ac907cf021a6bcd07852bb3b8c087051706a9');
  });
});
