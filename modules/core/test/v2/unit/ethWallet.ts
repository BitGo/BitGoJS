...
describe('Ethereum batch transaction data validation', function() {
  ...
  const runBadBatch = co(function *(badRecipients, expectedErrMsg) {
    ...
    yield coin.verifyTransaction(badParams).should.be.rejectedWith(expectedErrMsg);
  });
  ...
  it('Fails when recipient address is not Batcher', co(function *() {
    yield runBadBatch([{
      ...
      amount: '10',
      data: fixtures.goodBatchData
    }], 'batch transactions must be addressed to the Batcher');
  }));

  it('Fails when total send amount is negative', co(function *() {
    yield runBadBatch([{
      address: batcherContractAddress,
      amount: '-15',
      data: fixtures.goodBatchData
    }], 'total amount to send not in valid string form');
  }));
  ...
  it('Fails when prebuild array is not of size 1', co(function *() {
    yield runBadBatch([{
      ...
      amount: '10',
      data: ''
    }], 'prebuild recipients array should always be of length 1, was of length 2 here');
  }));
  ...
  it('Fails when function signature does not match', co(function *() {
    runBadBatch([{
      address: batcherContractAddress,
      amount: '10',
      data: fixtures.badBatchDataExamples.wrongFunctionSignature
  ...
  it('Fails when invalid addresses in data', co(function* () {
    yield runBadBatch([{
      address: batcherContractAddress,
      amount: '10',
      data: fixtures.badBatchDataExamples.invalidAddress
  ...
  it('Fails when invalid amount in data', co(function *() {
    yield runBadBatch([{
      address: batcherContractAddress,
      amount: '10',
      data: fixtures.badBatchDataExamples.invalidAmount
    }], 'batch data must be a hex string');
  }));
  ...
  it('Fails when bad array length', co(function *() {
    yield runBadBatch([{
      address: batcherContractAddress,
      amount: '10',
      data: fixtures.badBatchDataExamples.badArrayLength
  ...
  it('Fails when recipients and amounts are not the same size', co(function *() {
    yield runBadBatch([{
      address: batcherContractAddress,
      amount: '10',
      data: fixtures.badBatchDataExamples.unequalArrayLengths
    }], 'recipients and amounts arrays must be the same size');
  }));
  ...
  it('Fails when recipients and amounts do not match original recipients', co(function *() {
    yield runBadBatch([{
      address: batcherContractAddress,
      amount: '10',
      data: fixtures.badBatchDataExamples.notOriginalRecipients
