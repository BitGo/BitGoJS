...
describe('ETH:', function () {
  ...
  before(async function () {
    ...
    wallet = await bitgo.coin('teth').wallets().getWallet({ id: TestBitGo.V2.TEST_ETH_WALLET_ID });
  });
  ...
  describe('Non-BitGo Recovery', function () {
    ...
    it('should error when the backup key is unfunded (cannot pay gas)', async function () {
      const basecoin = bitgo.coin('teth');
      const error = await bitgo.getAsyncError(basecoin.recover({
  ...
  describe('Non-BitGo Token Recovery', function () {
    it('should construct a token recovery transaction without BitGo', async function () {
      const basecoin = await bitgo.token('0x06d22e6fa60fda26b6ca28f73d2d4a81bd9aa2de');
      const recovery = await basecoin.recover({
    ...
    it('should construct a token recovery transaction without passphrase for advanced cold wallets', async function () {
      const basecoin = await bitgo.token('0x945ac907cf021a6bcd07852bb3b8c087051706a9');
      const recovery = await basecoin.recover({
    ...
    it('should construct a token recovery transaction without BitGo and with KRS', async function () {
      const basecoin = await bitgo.token('0x06d22e6fa60fda26b6ca28f73d2d4a81bd9aa2de');
      const recovery = await basecoin.recover({
        userKey: '{"iv":"+TkmT3GJ5msVWQjBrt3lsw==","v":1,"iter":10000,"ks":256,"ts":64,"mode":"ccm","adata":"","cipher":"aes","salt":"cCE20fGIobs=","ct":"NVIdYIh91J3aRI8GG0JE3DhXW3AUmz2G5RqMejdz1+t4/vovIP7lleegI7VYyWiiLvlM0OCFf3EVvV/RyXr8+2vsnQ0Vn8c2CV5FRZ80OjGYrW3A/6T/zpOz6E8CMvnD++iIpeO4r2eZJavejZxdzlxF0BRz7VI="}',
  ...
  describe('Token Flush Thresholds', function () {
    it('should be updated', async function () {
      await wallet.updateTokenFlushThresholds({ terc: 10 });
      wallet.tokenFlushThresholds().should.match({ terc: '10' });

      // tokens that are not updated remain unchanged
      await wallet.updateTokenFlushThresholds({ tbst: 100 });
