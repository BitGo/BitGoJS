...
describe('Express ETH', () => {
  ...
  it(
    ...
    co(function* () {
      ...
      let res = yield agent.get(`/api/v2/teth/wallet/${testWalletId}/addresses`).set(authHeader);
      res.statusCode.should.equal(200);
      const { addresses } = res.body;
      const destAddress = addresses[1].address;

      res = yield agent.post(`/api/v2/teth/wallet/${testWalletId}/sendcoins`).set(authHeader).send({
  ...
  it(
    ...
    co(function* () {
      ...
      res = yield agent
        ...
        .send({
          walletPassphrase: testWalletPassphrase,
          recipients: [{ address: destAddress1, amount: '10000' }],
        });
      res.should.have.status(200);
    })
  );
  ...
  it(
    'can create new wallet and delete it',
    co(function* () {
  ...
  it(
    'can list wallets',
    co(function* () {
      const res = yield agent.get('/api/v2/teth/wallet').set(authHeader);
      res.statusCode.should.equal(200);
  ...
  it(
    'can fetch testWallet',
    co(function* () {
      const res = yield agent.get(`/api/v2/teth/wallet/${testWalletId}`).set(authHeader);
      res.statusCode.should.equal(200);
  ...
  it(
    'can list deposit addresses',
    co(function* () {
      const res = yield agent.get(`/api/v2/teth/wallet/${testWalletId}/addresses`).set(authHeader);
      res.statusCode.should.equal(200);
      res.body.addresses.filter(({ address }) => address === testWalletFirstAddress).length.should.equal(1);
    })
  );

  it(
    'can start or stop manual deployment and flushing of forwarder',
    co(function* () {
      ...
      const res = yield agent.post(`/api/v2/teth/wallet/${testWalletId}`).set(authHeader).send(payload);
      res.should.have.status(200);
      res.body.coinSpecific.deployForwardersManually.should.equal(deployForwardersManually);
      res.body.coinSpecific.flushForwardersManually.should.equal(flushForwardersManually);
    })
  );
  ...
  it(
    ...
    co(function* () {
      const res = yield agent.post(`/api/v2/teth/wallet/${testWalletId}/address`).set(authHeader);
      res.statusCode.should.equal(200);
    })
  );
});
