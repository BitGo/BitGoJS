...
describe('WalletSignTx codec tests', function () {
  ...
  describe('PostWalletSignTx route definition', function () {
    ...
    it('should have path parameter names matching codec parameter names', function () {
      const pathParams = extractPathParams(PostWalletSignTx.path);
      const codecParams = getCodecParamNames(WalletSignTxParams);
      ...
      assert.deepStrictEqual(
        ...
        `Path parameters ${JSON.stringify(pathParams)} do not match codec parameters ${JSON.stringify(codecParams)}. ` +
          `This will cause runtime validation errors! Path: ${PostWalletSignTx.path}`
      );
    });
  });
});
