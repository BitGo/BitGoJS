  ...
  describe('Response Types', function () {
    ...
    describe('TxRequestResponse', function () {
      it('should validate response with all required fields', function () {
          ...
          unsignedTxs: [
            {
              ...
              // Include optional fields with undefined values
              feeInfo: undefined,
              coinSpecific: undefined,
              parsedTx: undefined,
            },
          ],
          latest: true,
        ...
        assert.strictEqual(decoded.walletType, validResponse.walletType);
        assert.strictEqual(decoded.version, validResponse.version);
        assert.strictEqual(decoded.state, validResponse.state);
        assert.strictEqual(decoded.date, validResponse.date);
        assert.strictEqual(decoded.userId, validResponse.userId);
        assert.deepStrictEqual(decoded.intent, validResponse.intent);
        assert.strictEqual(decoded.policiesChecked, validResponse.policiesChecked);
        // Use deepEqual instead of deepStrictEqual for arrays with optional fields
        assert.deepEqual(decoded.unsignedTxs, validResponse.unsignedTxs);
      ...
      it('should validate response with optional fields', function () {
          ...
          messages: [
            {
              ...
              derivationPath: "m/44'/0'/0'/0/0",
              // Include all optional fields with undefined values
              messageEncoded: undefined,
              messageBroadcastable: undefined,
              messageStandardType: undefined,
              combineSigShare: undefined,
              txHash: undefined,
              commitmentShares: undefined,
            },
          ],
        ...
        assert.strictEqual(decoded.walletType, validResponse.walletType);
        assert.strictEqual(decoded.version, validResponse.version);
        assert.strictEqual(decoded.enterpriseId, validResponse.enterpriseId);
        assert.strictEqual(decoded.state, validResponse.state);
        assert.strictEqual(decoded.date, validResponse.date);
        assert.strictEqual(decoded.userId, validResponse.userId);
        assert.deepStrictEqual(decoded.intent, validResponse.intent);
        assert.strictEqual(decoded.pendingApprovalId, validResponse.pendingApprovalId);
        assert.strictEqual(decoded.policiesChecked, validResponse.policiesChecked);
        // Use deepEqual instead of deepStrictEqual for arrays with optional fields
        assert.deepEqual(decoded.signatureShares, validResponse.signatureShares);
        assert.deepEqual(decoded.commitmentShares, validResponse.commitmentShares);
        assert.deepEqual(decoded.pendingTxHashes, validResponse.pendingTxHashes);
        assert.deepEqual(decoded.txHashes, validResponse.txHashes);
        assert.deepEqual(decoded.unsignedMessages, validResponse.unsignedMessages);
        assert.deepEqual(decoded.unsignedTxs, validResponse.unsignedTxs);
        assert.deepEqual(decoded.transactions, validResponse.transactions);
        assert.deepEqual(decoded.messages, validResponse.messages);
