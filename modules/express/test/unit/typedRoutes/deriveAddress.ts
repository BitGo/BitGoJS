...
describe('DeriveAddress codec tests', function () {
  ...
  describe('DeriveAddressBody', function () {
    ...
    it('should reject body with a negative index', function () {
      assert.throws(() => {
        assertDecode(t.type(DeriveAddressBody), { keychains: [{ pub: 'xpub1...' }], index: -1 });
      });
    });

    it('should reject body with a non-integer index', function () {
      assert.throws(() => {
        assertDecode(t.type(DeriveAddressBody), { keychains: [{ pub: 'xpub1...' }], index: 1.5 });
      });
    });

    it('should reject body with a negative chain', function () {
      assert.throws(() => {
        assertDecode(t.type(DeriveAddressBody), { keychains: [{ pub: 'xpub1...' }], index: 0, chain: -1 });
      });
    });
