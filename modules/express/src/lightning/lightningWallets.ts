...
export async function getLightningKeychain(coin: BaseCoin, wallet: Wallet): Promise<LightningKeychain> {
  if (coin.getFamily() !== 'lnbtc') {
    throw new Error('Invalid coin to get lightning Keychain');
  }

  if (wallet.coin() !== coin.getChain()) {
    throw new Error('Invalid wallet coin to get lightning keychain');
  }
...
export async function getLightningAuthKeychains(
  ...
  if (coin.getFamily() !== 'lnbtc') {
    throw new Error('Invalid coin to get lightning auth keychains');
  }

  if (wallet.coin() !== coin.getChain()) {
    throw new Error('Invalid wallet coin to get lightning auth keychains');
  }
