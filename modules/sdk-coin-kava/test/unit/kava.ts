...
describe('Kava', function () {
  ...
  it('should return the right info', function () {
    const kava = bitgo.coin('kava');
    const tkava = bitgo.coin('tkava');

    kava.getChain().should.equal('kava');
    kava.getFamily().should.equal('kava');
    kava.getFullName().should.equal('Kava');
    kava.getBaseFactor().should.equal(1e6);

    tkava.getChain().should.equal('tkava');
    tkava.getFamily().should.equal('kava');
    tkava.getFullName().should.equal('Testnet Kava');
    tkava.getBaseFactor().should.equal(1e6);
  });
  ...
  describe('Address Validation', () => {
    ...
    it('should validate wallet receive address', async function () {
      const receiveAddress = {
        address: 'kava196n9432kj8gq9c63zqp0tw84vzxl5fe42g6mcf?memoId=7',
        coinSpecific: {
          rootAddress: 'kava196n9432kj8gq9c63zqp0tw84vzxl5fe42g6mcf',
          memoID: '7',
        },
      };
  ...
  describe('Parse Transactions: ', () => {
    ...
    it('should fail to parse a transfer transaction when explainTransaction response is undefined', async function () {
      const stub = sinon.stub(Kava.prototype, 'explainTransaction');
