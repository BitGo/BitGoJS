 ...
 * @prettier
 */
import { CoinMap, getFormattedTokens } from '@bitgo/statics';
...
function registerCoinConstructors(globalCoinFactory: CoinFactoryV2, coinMap: CoinMap): void {
  ...
  AdaToken.createTokenConstructors([...tokens.bitcoin.ada.tokens, ...tokens.testnet.ada.tokens]).forEach(
    ({ name, coinConstructor }) => globalCoinFactory.register(name, coinConstructor)
  );
...
export const GlobalCoinFactoryV2 = (coinMap: CoinMap): CoinFactoryV2 => {
  const globalCoinFactory = new CoinFactoryV2(coinMap);
  registerCoinConstructors(globalCoinFactory, coinMap);
  return globalCoinFactory;
};
