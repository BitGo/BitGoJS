...
import { fixedScriptWallet, BIP32, message } from '@bitgo/wasm-utxo';
...
describe('Custom Change Wallets', function () {
  ...
  const customChangeKeyIds = customChangeWalletKeys.triple.map((k) => getSeed(k.neutered().toBase58()).toString('hex'));
  const customChangeKeySignatures = Object.fromEntries(
    (['user', 'backup', 'bitgo'] as const).map((name, i) => [
      name,
      Buffer.from(message.signMessage(customChangeWalletKeys.triple[i].neutered().toBase58(), userPrivateKey)).toString(
        'hex'
      ),
    ])
  ...
  const psbt = fixedScriptWallet.BitGoPsbt.fromBytes(utxolibPsbt.toBuffer(), 'btc');
  ...
  function nockKeyFetch(keyIds: string[], keys: utxolib.bitgo.RootWalletKeys): nock.Scope[] {
    return keyIds.map((id, i) =>
      nock(bgUrl).get(`/api/v2/${coin.getChain()}/key/${id}`).reply(200, { pub: keys.triple[i].neutered().toBase58() })
    );
  }
  ...
  it('classifies custom change output when customChangePubs is provided', function () {
    const explanation = explainPsbt(psbt, rootWalletKeys, customChangeWalletKeys);
  ...
  it('classifies custom change output as external without customChangePubs', function () {
    const explanation = explainPsbt(psbt, rootWalletKeys, undefined);
  ...
  describe('parseTransaction', function () {
    it('fetches custom change wallet keys and verifies signatures', async function () {
      ...
      const parsed = await coin.parseTransaction({
        ...
        txPrebuild: { txHex: utxolibPsbt.toHex() },
