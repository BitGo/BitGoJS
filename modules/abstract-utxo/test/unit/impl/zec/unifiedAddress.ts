...
describe('Zec Unified Address support', function () {
  ...
  describe('resolveOutputScript', function () {
    ...
    it('resolves the transparent receiver for a unified address when preference is not shielded', function () {
    ...
    it('throws for a unified address without a transparent receiver when preference is not shielded', function () {
      const orchardOnlyUa = fixedScriptWallet.ZcashUnifiedAddress.encodeOrchardReceiver(
        Buffer.from(TESTNET_UA.ironwoodReceiverHex as string, 'hex'),
        'tzec'
      );
      assert.throws(() => tzec.resolveOutputScript(orchardOnlyUa));
  ...
  describe('decodeTransaction / decodeTransactionFromPrebuild', function () {
    ...
    it('decodes a v6 (Ironwood) PSBT as a ZcashIronwoodBitGoPsbt', function () {
      // `ZcashPsbt.fromBytes` reads the transaction version from the parsed metadata and
      // dispatches to the format-specific implementation; no error-message sniffing involved.
      const psbt = fixedScriptWallet.ZcashIronwoodBitGoPsbt.createEmpty('zec', walletKeys, { blockHeight: 4200000 });
      psbt.addWalletInput({ txid: '33'.repeat(32), vout: 0, value: 100000n }, walletKeys, {
        scriptId: { chain: 0, index: 0 },
      });
      psbt.addWalletOutput(walletKeys, { chain: 1, index: 0, value: 90000n });
      const decoded = zec.decodeTransaction(Buffer.from(psbt.serialize()));
      assert.ok(decoded instanceof fixedScriptWallet.ZcashIronwoodBitGoPsbt);
    });

    it('propagates a non-wasm-utxo error from the PSBT deserializer', function () {
      const fromBytesStub = sinon.stub(fixedScriptWallet.ZcashPsbt, 'fromBytes').throws(new Error('boom'));

      try {
        const psbtMagicBytes = Buffer.from([0x70, 0x73, 0x62, 0x74, 0xff, 0x00]);
        assert.throws(() => zec.decodeTransaction(psbtMagicBytes), /boom/);
      } finally {
        fromBytesStub.restore();
      }
    });
  });
