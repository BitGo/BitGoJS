...
export function mockWalletUnspent<TNumber extends number | bigint = number>(
  ...
  let deriveAddress: string;
  if (utxolib.isTestnet(network)) {
    deriveAddress = wasmUtxo.fixedScriptWallet.address(walletKeys, chain, index, network);
  } else {
    const derived = getOutputScript(walletKeys, chain, index);
    deriveAddress = utxolib.address.fromOutputScript(derived.scriptPubKey, network);
  }
