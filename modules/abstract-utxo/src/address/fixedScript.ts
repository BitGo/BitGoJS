...
import { AddressDetails, GenerateAddressOptions } from './types';
import { canonicalAddress } from './addressFormat';
...
type CreateAddressFormat = 'base58' | 'cashaddr';
...
export function generateAddress(
...
): AddressDetails<FixedScriptAddressCoinSpecific> {
  ...
  const {
    ...
    redeemScript,
    witnessScript,
  } = utxolib.bitgo.outputScripts.createOutputScript2of3(derivedKeys, addressType);

  const address = utxolib.address.fromOutputScript(outputScript, network);

  return {
    address: canonicalAddress(network, address, params.format),
...
export function assertFixedScriptWalletAddress(
  ...
  const expectedAddress = generateAddress(network, coinName, {
    format,
