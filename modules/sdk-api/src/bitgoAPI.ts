...
export class BitGoAPI implements BitGoBase {
  ...
  constructor(params: BitGoAPIOptions = {}) {
    ...
    if (this._authVersion === 4 && params.accessTokenId) {
      this._tokenId = params.accessTokenId;
    }
    ...
    req.then = (onfulfilled, onrejected) => {
      ...
      if (this._authVersion === 4) {
        ...
        if (this._token && !this._tokenId) {
          throw new Error(
            'v4 auth requires both accessToken and accessTokenId (tokenId). ' +
              'Set accessTokenId in the constructor or call authenticateWithAccessToken({ accessToken, tokenId }).'
          );
        }
