...
describe('processV4TokenIssuance', function () {
  let bitgo: BitGoAPI;
  ...
  it('should decrypt signing key and return with token ID', function () {
    const tokenId = 'v4_token_id_12345';
    const signingKeyPlaintext = 'signing_key_secret_12345';
    const ecdhXprv =
      'xprv9s21ZrQH143K2fJ91S4BRsupcYrE6mmY96fcX5HkhoTrrwmwjd16Cn87cWinJjByrfpojjx7ezsJLx7TAKLT8m8hM5Kax9YcoxnBeJZ3t2k';
    const derivationPath = 'm/999999/54719676/90455048';
    const encryptedToken =
      '{"iv":"test","v":1,"iter":10000,"ks":256,"ts":64,"mode":"ccm","adata":"","cipher":"aes","salt":"test","ct":"test"}';

    // Stub handleTokenIssuance to return the decrypted signing key
    const handleTokenIssuanceStub = sinon.stub(bitgo, 'handleTokenIssuance').returns({ token: signingKeyPlaintext });

    const result = bitgo.processV4TokenIssuance({
      tokenId,
      encryptedToken,
      derivationPath,
      ecdhXprv,
    ...
    result.should.have.property('signingKey');
    result.tokenId.should.equal(tokenId);
    result.signingKey.should.be.a.String();
    result.signingKey.should.equal(signingKeyPlaintext);

    // Verify handleTokenIssuance was called correctly
    sinon.assert.calledOnce(handleTokenIssuanceStub);
  ...
  it('should throw error with invalid ecdhXprv', function () {
    (() => {
      bitgo.processV4TokenIssuance({
        tokenId: 'valid_token_id',
        encryptedToken: 'validToken',
        derivationPath: 'm/999999/0/0',
        ecdhXprv: 'invalid-xprv',
      });
    }).should.throw();
  });

  it('should throw error with invalid encrypted token', function () {
    const ecdhXprv =
      'xprv9s21ZrQH143K2fJ91S4BRsupcYrE6mmY96fcX5HkhoTrrwmwjd16Cn87cWinJjByrfpojjx7ezsJLx7TAKLT8m8hM5Kax9YcoxnBeJZ3t2k';
  ...
  it('should produce consistent signing keys for same input', function () {
    const tokenId = 'v4_token_id_consistency';
    const signingKeyPlaintext = 'signing_key_consistency';
    const ecdhXprv =
      'xprv9s21ZrQH143K2fJ91S4BRsupcYrE6mmY96fcX5HkhoTrrwmwjd16Cn87cWinJjByrfpojjx7ezsJLx7TAKLT8m8hM5Kax9YcoxnBeJZ3t2k';
    const encryptedToken =
      '{"iv":"test","v":1,"iter":10000,"ks":256,"ts":64,"mode":"ccm","adata":"","cipher":"aes","salt":"test","ct":"test"}';
    ...
    // Stub handleTokenIssuance
    const handleTokenIssuanceStub = sinon.stub(bitgo, 'handleTokenIssuance').returns({ token: signingKeyPlaintext });

    const result1 = bitgo.processV4TokenIssuance({
      tokenId,
      encryptedToken,
      derivationPath,
      ecdhXprv,
    ...
    const result2 = bitgo.processV4TokenIssuance({
      tokenId,
      encryptedToken,
      derivationPath,
      ecdhXprv,
    ...
    result1.tokenId.should.equal(result2.tokenId);
    result1.signingKey.should.equal(result2.signingKey);

    handleTokenIssuanceStub.restore();
  });

  it('should produce different signing keys for different tokens', function () {
    const tokenId1 = 'v4_token_id_1';
    const tokenId2 = 'v4_token_id_2';
    const signingKey1 = 'signing_key_1';
    const signingKey2 = 'signing_key_2';
    const ecdhXprv =
      'xprv9s21ZrQH143K2fJ91S4BRsupcYrE6mmY96fcX5HkhoTrrwmwjd16Cn87cWinJjByrfpojjx7ezsJLx7TAKLT8m8hM5Kax9YcoxnBeJZ3t2k';
    const encryptedToken1 =
      '{"iv":"test1","v":1,"iter":10000,"ks":256,"ts":64,"mode":"ccm","adata":"","cipher":"aes","salt":"test","ct":"test1"}';
    const encryptedToken2 =
      '{"iv":"test2","v":1,"iter":10000,"ks":256,"ts":64,"mode":"ccm","adata":"","cipher":"aes","salt":"test","ct":"test2"}';
    const derivationPath = 'm/999999/54719676/90455048';

    // Stub to return different signing keys for different encrypted tokens
    const handleTokenIssuanceStub = sinon
      .stub(bitgo, 'handleTokenIssuance')
      .onFirstCall()
      .returns({ token: signingKey1 })
      .onSecondCall()
      .returns({ token: signingKey2 });
    ...
    // Different tokens should produce different signing keys
    result1.signingKey.should.not.equal(result2.signingKey);
    result1.tokenId.should.not.equal(result2.tokenId);
...
describe('V4 Token Issuance', function () {
  ...
  it('should validate V4 response structure in addAccessToken', async function () {
    ...
    // Mock a V4 response missing required fields
    const mockResponse = {
      body: {
        // Missing 'id', 'encryptedToken', 'derivationPath'
        label: 'test-token',
      },
    };
...
describe('V4 edge cases', function () {
  ...
  it('should handle missing tokenId in V4 response gracefully', async function () {
    const bitgo = new BitGoAPI({
      env: 'test',
      authVersion: 4,
    });
