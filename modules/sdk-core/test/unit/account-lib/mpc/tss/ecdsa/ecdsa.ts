...
describe('ecdsa tss', function () {
  ...
  describe('key share invalid arguments', function () {
    it('should throw error if index is not positive', function () {
      (() => ecdsa.keyShareStep1(0, 0, 3)).should.throw('Invalid KeyShare Config');
      (() => ecdsa.keyShareStep1(-1, -1, 3)).should.throw('Invalid KeyShare Config');
    });

    it('should throw error if index is larger than numShares', function () {
      (() => ecdsa.keyShareStep1(3, 0, 2)).should.throw('Invalid KeyShare Config');
      (() => ecdsa.keyShareStep1(10, -1, 3)).should.throw('Invalid KeyShare Config');
    });

    it('should throw error if threshold is not 2', function () {
      (() => ecdsa.keyShareStep1(1, 0, 3)).should.throw('Invalid KeyShare Config');
      (() => ecdsa.keyShareStep1(6, -1, 10)).should.throw('Invalid KeyShare Config');
    });

    it('should throw error if numShares is less than 2', function () {
      (() => ecdsa.keyShareStep1(1, 2, 1)).should.throw('Invalid KeyShare Config');
    });

    it('should throw error if length of seed is less than 64', function () {
      (() => ecdsa.keyShareStep1(1, 2, 3, Buffer.alloc(1))).should.throw(
        'Seed must have a length of at least 64 bytes'
      );
      (() => ecdsa.keyShareStep1(1, 2, 3, Buffer.alloc(63))).should.throw(
        'Seed must have a length of at least 64 bytes'
      );
    });
  });

  describe('key generation and tx signing', function () {
    ...
    before('generate key and sign phase 1 to 4', async function () {
      ...
      const yCommitments1 = {
        2: uyShare2.comDecomY.commitment,
        3: uyShare3.comDecomY.commitment,
      };

      const yCommitments2 = {
        1: uyShare1.comDecomY.commitment,
        3: uyShare3.comDecomY.commitment,
      };
      ...
      // keyCombine() should fail if commitment is incorrect.
      keyShare3.nShares[1].comY = randomBytes(32);

      (() => ecdsa.keyCombine(keyShare1.pShare, [keyShare2.nShares[1], keyShare3.nShares[1]])).should.throw(
        'Could not verify commitment to y from participant 3'
      );
      ...
      keyShare1.nShares[3].comY = yCommitments3[1];
      keyShare2.nShares[3].comY = yCommitments3[2];
    ...
    it('sign phase 5 fail - malicious player cheats with bad s share', async function () {
      ...
      const [publicUTShares_1, publicUTShares_2] = [UT1 as PublicUTShare, UT2 as PublicUTShare];
      (() => ecdsa.verifyUTShares(UT1, [publicUTShares_2])).should.throw(
        'Sum of all U_i does not match sum of all T_i'
      );
      (() => ecdsa.verifyUTShares(UT2, [publicUTShares_1])).should.throw(
        'Sum of all U_i does not match sum of all T_i'
      );
    });
  });
