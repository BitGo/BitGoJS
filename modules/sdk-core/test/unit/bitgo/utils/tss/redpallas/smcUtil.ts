...
import { RedpallasMPCv2SMCUtils } from '../../../../../../src/bitgo/utils/tss/redpallas/SMC/utils';

describe('RedPallas MPCv2 SMC Utils:', function () {
  const enterpriseId = '6449153a6f6bc20006d66771cdbe15d3';
  const coinName = 'zec';

  let smcUtils: RedpallasMPCv2SMCUtils;
  ...
  const buildRound1Payload = (
    state: RedpallasKeyCreationMPCv2StateEnum = RedpallasKeyCreationMPCv2StateEnum.WaitingForBitgoRound1Data
  ): RedpallasOVC1ToBitgoRound1Payload =>
    ({
      state,
      tssVersion: '0.0.1',
      ...
      ovc: {
        [OVCIndexEnum.ONE]: {
          gpgPubKey: 'user-gpg-pubkey',
          ovcMsg1: fakeSignedMessage('user-1'),
        },
        [OVCIndexEnum.TWO]: {
          gpgPubKey: 'backup-gpg-pubkey',
          ovcMsg1: fakeSignedMessage('backup-1'),
        },
      },
    } as unknown as RedpallasOVC1ToBitgoRound1Payload);

  const buildRound2Payload = (
    sessionId = 'test-session-id',
    state: RedpallasKeyCreationMPCv2StateEnum = RedpallasKeyCreationMPCv2StateEnum.WaitingForBitgoRound2Data
  ): RedpallasOVC2ToBitgoRound2Payload =>
    ({
      state,
      tssVersion: '0.0.1',
  ...
  beforeEach(function () {
    ...
    smcUtils = new RedpallasMPCv2SMCUtils(mockBitgo, mockBaseCoin);
  });
  ...
  describe('keyGenRound1BySender', function () {
    it('returns a well-formed BitGo→OVC1 round 1 response on success', async function () {
      ...
      } as unknown as RedpallasMPCv2KeyGenRound1Response);

      const response = (await smcUtils.keyGenRound1BySender(
        senderFn as never,
        payload
      )) as RedpallasBitgoToOVC1Round1Response;

      assert.strictEqual(response.state, RedpallasKeyCreationMPCv2StateEnum.WaitingForOVC1Round2Data);
    ...
    it('rejects when the payload state is not WaitingForBitgoRound1Data', async function () {
      const payload = buildRound1Payload(RedpallasKeyCreationMPCv2StateEnum.WaitingForOVC1Round2Data);
      ...
      await assert.rejects(smcUtils.keyGenRound1BySender(senderFn as never, payload), {
        message: `Invalid state for round 1, expected: ${RedpallasKeyCreationMPCv2StateEnum.WaitingForBitgoRound1Data}, got: ${RedpallasKeyCreationMPCv2StateEnum.WaitingForOVC1Round2Data}`,
      });
  ...
  describe('keyGenRound2BySender', function () {
    ...
    it('returns a well-formed BitGo→OVC1 round 2 response and adds the BitGo keychain', async function () {
      ...
      } as unknown as RedpallasMPCv2KeyGenRound2Response);

      const response = (await smcUtils.keyGenRound2BySender(
        senderFn as never,
        payload
      )) as RedpallasBitgoToOVC1Round2Response;

      assert.strictEqual(response.state, RedpallasKeyCreationMPCv2StateEnum.WaitingForOVC1GenerateKey);
    ...
    it('rejects when the payload state is not WaitingForBitgoRound2Data', async function () {
      const payload = buildRound2Payload('session-xyz', RedpallasKeyCreationMPCv2StateEnum.WaitingForOVC2Round2Data);
      const senderFn = sinon.stub().rejects(new Error('sender should not be invoked'));

      await assert.rejects(smcUtils.keyGenRound2BySender(senderFn as never, payload), {
        message: `Invalid state for round 2, expected: ${RedpallasKeyCreationMPCv2StateEnum.WaitingForBitgoRound2Data}, got: ${RedpallasKeyCreationMPCv2StateEnum.WaitingForOVC2Round2Data}`,
      });
      assert.ok(senderFn.notCalled);
  ...
  describe('keyGenRound1 (enterprise)', function () {
    ...
    it('POSTs to the MPCv2 generatekey endpoint with curveType RedPallas and returns a parsed round 1 response', async function () {
      const payload = buildRound1Payload();
      ...
      assert.strictEqual(sentBody.curveType, 'RedPallas');
  ...
  describe('keyGenRound2 (enterprise)', function () {
    it('rejects for an invalid payload state without calling the API', async function () {
      ...
      await assert.rejects(smcUtils.keyGenRound2(enterpriseId, invalidPayload), {
        message: `Invalid state for round 2, expected: ${RedpallasKeyCreationMPCv2StateEnum.WaitingForBitgoRound2Data}, got: ${RedpallasKeyCreationMPCv2StateEnum.WaitingForOVC1Round2Data}`,
      });
      assert.ok((mockBitgo.post as sinon.SinonStub).notCalled);
    });
  });
