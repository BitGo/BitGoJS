...
describe('Keychains.updatePassword (safe mode)', function () {
  ...
  it('propagates a mid-chunk failure after earlier chunks were sent and destroys the session', async function () {
    const page = Array.from({ length: 501 }, (_, i) => key(`key-${i}`));
    stubListPages([page]);
    let chunkIndex = 0;
    send.callsFake((body: SentBulkBody) => {
      chunkIndex += 1;
      if (chunkIndex === 2) {
        return { result: sinon.stub().rejects(new Error('boom')) };
      }
      return {
        result: sinon.stub().resolves({ updates: body.updates.map((u) => ({ keyId: u.keyId, updated: true })) }),
      };
    ...
    // the first chunk went out before the second failed; the SDK-owned session is destroyed
    sinon.assert.calledTwice(put);
    sentBodies()[0].updates.length.should.equal(500);
    sinon.assert.calledOnce(sessionDestroy);
  });

  it('rejects a chunk response missing the updates array', async function () {
    stubListPages([[key('root-1')]]);
    send.returns({ result: sinon.stub().resolves({ unexpected: true }) });

    await keychains
      .updatePassword({ oldPassword: OLD_PASSWORD, newPassword: NEW_PASSWORD, safeId: SAFE_ID })
      .should.be.rejectedWith(/invalid key bulk update response/);
  ...
  it('keeps skipping undecryptable keys in legacy mode', async function () {
    // getEncryptionVersion re-parses the old envelope in the legacy path, so it must be JSON
    const goodEnvelope = JSON.stringify({ v: 1, ct: 'prv-good' });
    decryptStub.callsFake(async ({ input }: { input: string; password: string }) => {
      if (input !== goodEnvelope) {
        throw new Error('private key is incorrect');
      }
      return 'prv-good';
    });
    sinon.stub(keychains, 'list').resolves({
      keys: [key('bad', { encryptedPrv: 'corrupt-envelope' }), key('good', { encryptedPrv: goodEnvelope })],
      nextBatchPrevId: undefined,
    });

    const changedKeys = await keychains.updatePassword({ oldPassword: OLD_PASSWORD, newPassword: NEW_PASSWORD });

    changedKeys.should.deepEqual({ 'pub-good': 'new-envelope' });
    sinon.assert.notCalled(put);
  });
});
