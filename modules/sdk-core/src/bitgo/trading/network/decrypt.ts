...
import { _decryptBrowserRsa } from './decrypt-rsa-browser';
 ...
 * @returns {string} The decrypted text
 */
export async function decryptBrowserRsaWithAesGcm(privateKey: string, encryptedText: string): Promise<string> {
  const [encryptedKey, encrypted] = encryptedText.split('\n');
  const gcmKey = await _decryptBrowserRsa(privateKey, encryptedKey);
