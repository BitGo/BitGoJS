...
export class EcdsaMPCv2Utils extends BaseEcdsaUtils {
  ...
  async createOfflineRound1Share(params: {
    ...
    if (useV2) {
      ...
      try {
        const encryptedRound1Session = await session.encrypt(sessionData);
        const encryptedUserGpgPrvKey = await session.encrypt(userGpgKey.privateKey);
    ...
    const encryptedRound1Session = this.bitgo.encrypt({ input: sessionData, password: walletPassphrase, adata });
  ...
  async createOfflineRound2Share(params: {
    ...
    if (useV2) {
      ...
      try {
        const encryptedRound2Session = await encSession.encrypt(sessionData);
    ...
    const encryptedRound2Session = this.bitgo.encrypt({ input: sessionData, password: walletPassphrase, adata });
