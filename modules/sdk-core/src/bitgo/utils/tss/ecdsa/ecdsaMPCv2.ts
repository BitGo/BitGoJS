...
export class EcdsaMPCv2Utils extends BaseEcdsaUtils {
      ...
      case MPCv2PartiesEnum.BACKUP:
        ...
        if (encryptionSession) {
          ...
          reducedEncryptedPrv = await this.bitgo.encrypt({
            ...
            // The browser deals with a Buffer as Uint8Array, therefore on browser .toString('base64') just creates a comma seperated string of the array values.
  ...
  async createOfflineRound2Share(params: {
    ...
    const round1Session = await this.bitgo.decrypt({ input: encryptedRound1Session, password: walletPassphrase });
  ...
  async createOfflineRound3Share(params: {
    ...
    const round2Session = await this.bitgo.decrypt({ input: encryptedRound2Session, password: walletPassphrase });
 ...
 * @param {BitGoBase} bitgo - BitGo instance for v1/v2 auto-detect decrypt
 * @returns {Promise<boolean>} - Resolves `true` if the decrypted data contains valid signing material, otherwise `false`.
 */
export async function isGG18SigningMaterial(
  keyShare: string,
  walletPassphrase: string | undefined,
  bitgo: BitGoBase
): Promise<boolean> {
  try {
    const prv = await bitgo.decrypt({ password: walletPassphrase, input: keyShare });
...
async function getMpcV2RecoveryKeySharesFromReducedKey(
  ...
  const backupPrvJSON: DklsTypes.ReducedKeyShare = DklsTypes.getDecodedReducedKeyShare(backupMaterial.signingKeyShare);
...
async function getKeyCombinedFromTssKeyShares(
  ...
  let userPrv;
  try {
    backupPrv = await bitgo.decrypt({ password: walletPassphrase, input: encryptedGG18BackupKey });
    userPrv = await bitgo.decrypt({ password: walletPassphrase, input: encryptedGG18UserKey });
  } catch (e) {
