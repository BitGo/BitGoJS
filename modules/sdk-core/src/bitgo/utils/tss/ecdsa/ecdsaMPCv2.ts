...
async function decryptWalletKeyAsync(
  walletPassphrase: string | undefined,
  encryptedKey: string,
  bitgo: BitGoBase
): Promise<string> {
  return bitgo.decryptAsync({ password: walletPassphrase, input: encryptedKey });
}
...
async function getMpcV2RecoveryKeySharesFromReducedKey(
  ...
  if (bitgo) {
    userCompressedPrv = Buffer.from(
      await decryptWalletKeyAsync(walletPassphrase, encryptedMPCv2UserKey, bitgo),
      'base64'
    );
    bakcupCompressedPrv = Buffer.from(
      await decryptWalletKeyAsync(walletPassphrase, encryptedMPCv2BackupKey, bitgo),
      'base64'
    );
  } else {
...
async function getKeyCombinedFromTssKeyShares(
  ...
  try {
    if (bitgo) {
      backupPrv = await decryptWalletKeyAsync(walletPassphrase, encryptedGG18BackupKey, bitgo);
      userPrv = await decryptWalletKeyAsync(walletPassphrase, encryptedGG18UserKey, bitgo);
    } else {
