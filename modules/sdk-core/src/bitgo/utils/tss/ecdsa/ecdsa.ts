...
export class EcdsaUtils extends baseTSSUtils<KeyShare> {
  ...
  async getBackupEncryptedNShare(
    ...
    if (isThirdPartyBackup) {
      if (!backupShare.bitGoHeldKeyShares?.keyShares) {
        // Throwing the bitGoHeldKeyShares temporarily for Nike debugging
        throw new Error(`Missing backup key shares: ${JSON.stringify(backupShare.bitGoHeldKeyShares)}`);
      }
