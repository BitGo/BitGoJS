 ...
 * @param index
 * @param requestType
 * @param pallierModulus - the pallier public key $n$ for an ecdsa wallet
 * that bitgo needs to generate a challenge for
 */
export async function getTxRequestChallenge(
  ...
  index: string,
  requestType: RequestType,
  // TODO(BG-78654): make this mandatory
  pallierModulus?: string
  ...
  return await bitgo.post(bitgo.url(urlPath, 2)).send({ pallierModulus }).result();
}
