...
import {
  ...
  KShare,
  XShare,
  YShare, KeyCombinedWithNTilde, XShareWithNTilde, YShareWithNTilde, RangeProofWithCheck,
} from "./types";
...
export default class Ecdsa {
  ...
  async signConvert(shares: SignConvert): Promise<SignConvertRT> {
    ...
    if ((shareToBeSent as AShare).alpha) {
      ...
      const alpha = fakeDecrypt(modPow(v, -gamma, N * N) * hexToBigInt(aShareToBeSent.alpha) % (N * N)) + k_real * gamma;
      gShareParticipant.alpha = bigIntToBufferBE(Ecdsa.curve.scalarReduce(alpha), 32).toString('hex');
      const mu = fakeDecrypt((modPow(v, -w, N * N) * hexToBigInt(aShareToBeSent.mu as string)) % (N * N)) + k_real * w; // recheck encrypted number
      /****** Modified code ********** */
