...
export class EddsaKeyDeriver {
  ...
  static readonly ROOT_PRV_KEY_LENGTH = 263;
  static readonly ROOT_PUB_KEY_LENGTH = 133;
  ...
  static async createRootKeys(seed?: Buffer): Promise<RootKeys> {
    ...
    return {
      prv: this.formatRootPrvKey(skString, pkString, chaincode, prefix),
  ...
  private static formatRootPrvKey(prv: string, pub: string, chaincode: string, prefix: string): string {
    return this.ROOT_PRV_KEY_PREFIX + prv + ':' + pub + ':' + chaincode + ':' + prefix;
  }

  static parseRootPubKey(pub: string): { pub: string; chaincode: string } {
  ...
  static parseRootPrvKey(prv: string): { prv: string; pub: string; chaincode: string; prefix: string } {
  ...
  static async deriveKeyWithSeed(key: string, seed: string): Promise<{ key: string; derivationPath: string }> {
    ...
    } else if (key.startsWith(this.ROOT_PRV_KEY_PREFIX)) {
      await Eddsa.initialize();
  ...
  static isValidRootPrvKey(prv: string): boolean {
    return prv.length === this.ROOT_PRV_KEY_LENGTH && prv.startsWith(this.ROOT_PRV_KEY_PREFIX);
  }

  static isValidRootPubKey(pub: string): boolean {
    return pub.length === this.ROOT_PUB_KEY_LENGTH && pub.startsWith(this.ROOT_PUB_KEY_PREFIX);
  }
}
