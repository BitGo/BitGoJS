...
import { BaseCoin as CoinConfig, AcalaSpecNameType } from '@bitgo/statics';
import {
  DecodedSignedTx,
...
export abstract class TransactionBuilder extends BaseTransactionBuilder {
   ...
   * Validates the token
   */
  validateToken(token: TokenSymbol): void {
    if (!utils.isValidToken(token)) {
      throw new BuildTransactionError(`Token validation failed`);
    }
  }
