import { OfflineVaultTxInfo, optionalDeps, runTokenTestInitialization } from '@bitgo/abstract-eth';
...
describe('Arbeth Token Tests', () => {
  const coinName = 'Arbeth';
  const tokenNetworkName = 'Arbitrum Test LINK';

  describe('Arbeth tokens in test env:', () => {
    ...
    const arbethTokenCoin: any = bitgo.coin(testData.NETWORK_TOKEN_IDENTIFIER);

    it('Arbeth run token tests', () => {
      runTokenTestInitialization(ArbethToken, coinName, tokenNetworkName, testData);
    });
