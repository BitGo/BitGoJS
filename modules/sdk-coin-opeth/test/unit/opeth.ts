...
function buildBatchTxHex(
  ...
  opts: { overrideInnerAddresses?: string[]; overrideInnerAmounts?: string[] } = {}
): string {
  ...
  const innerData = Buffer.concat([
    EthereumAbi.methodID('batch', ['address[]', 'uint256[]']),
    EthereumAbi.rawEncode(['address[]', 'uint256[]'], [innerAddresses, innerAmounts]),
  ]);
  ...
  const sendMultisigData = Buffer.concat([
    ...
    EthereumAbi.rawEncode(
      ...
      [batcherAddress, total, innerData, 1700000000, 1, Buffer.alloc(0)]
    ),
  ]);
...
describe('Optimism', function () {
  ...
  const buildUnsignedTransaction = async function ({
    ...
    const txBuilder: TransactionBuilder = getBuilder('topeth') as TransactionBuilder;
  ...
  describe('Transaction Verification', function () {
    ...
    it('should verify a native batch txPrebuild whose inner calldata matches txParams.recipients', async function () {
      const wallet = new Wallet(bitgo, basecoin, {});
      const batcherAddress = (basecoin?.staticsCoin?.network as EthereumNetwork).batcherContractAddress as string;
      const batchRecipients = [
        { amount: '1000000000000', address: address1 },
        { amount: '2500000000000', address: address2 },
      ];

      const txParams = { recipients: batchRecipients, wallet, walletPassphrase: 'fakeWalletPassphrase' };
      const txPrebuild = {
    ...
    it('should reject a native batch txPrebuild whose inner recipient amount differs from txParams', async function () {
      const wallet = new Wallet(bitgo, basecoin, {});
      const batcherAddress = (basecoin?.staticsCoin?.network as EthereumNetwork).batcherContractAddress as string;
      const batchRecipients = [
        { amount: '1000000000000', address: address1 },
      ...
      const txPrebuild = {
        recipients: [{ amount: '3500000000000', address: batcherAddress }],
        // Total preserved but per-recipient split tampered.
        txHex: buildBatchTxHex(batchRecipients, batcherAddress, {
  ...
  describe('Evm Based Cross Chain Recovery transaction:', function () {
    ...
    it('should generate an half signed recovery txn of an unsupported token for hot wallet', async function () {
      ...
      transaction.should.have.property('coin');
      transaction.coin.should.equal('topeth');
      transaction.should.have.property('contractSequenceId');
      transaction.should.have.property('expireTime');
      transaction.should.have.property('gasLimit');
      transaction.gasLimit.should.equal('500000');
