...
describe('Ton Transfer Builder', () => {
  ...
  it('should build a send from rawTx', async function () {
    const txBuilder = factory.from(testData.signedSendTransaction.tx);
    const builtTx = await txBuilder.build();
    const jsonTx = builtTx.toJson();
    should.equal(builtTx.type, TransactionType.Send);
    should.equal(builtTx.signablePayload.toString('base64'), testData.signedSendTransaction.signable);
    should.equal(builtTx.id, testData.signedSendTransaction.txId);
    ...
    should.equal(builtTx.toBroadcastFormat(), testData.signedSendTransaction.tx);
    ...
    txBuilder.from(testData.signedSendTransaction.tx);
    const jsonTx3 = builtTx3.toJson();
    should.equal(jsonTx3.bounceable, false);
    should.equal(builtTx3.signablePayload.toString('base64'), testData.signedSendTransaction.bounceableSignable);
    should.equal(builtTx3.id, testData.signedSendTransaction.txIdBounceable);
    should.equal(builtTx3.toBroadcastFormat(), testData.signedSendTransaction.tx);
  ...
  it('should parse a raw transaction and set flags', async function () {
    const factory = new TransactionBuilderFactory(coins.get('tton'));
    const txBuilder = factory.from(testData.signedSendTransaction.tx);
    const txBuilderBounceable = factory.from(testData.signedSendTransaction.txBounceable);
