...
import utils, { computeAtomicImportFee } from './utils';
...
export class ImportInCTxBuilder extends AtomicInCTransactionBuilder {
  /** Live C-chain base fee in nFLR supplied by the caller. When set the builder
   * computes the required fee internally using the correct coreth gas formula
   * instead of relying on an externally pre-computed value. */
  private _cChainBaseFeeNFLR: bigint | undefined;
  ...
  /**
   * Set the live C-chain base fee so the builder can compute the correct import
   * fee internally.
   *
   * When this is supplied the builder uses the coreth atomic-import gas formula:
   *   gas = AtomicTxBaseCost(10_000) + txSizeBytes + numSigs × 1_000
   *   fee = gas × baseFee × 1.25   (25% volatility padding)
   *
   * The fee computed here supersedes any value previously set via `.fee()`.
   *
   * @param baseFeeNFLR - C-chain base fee in nFLR (nanoFLR, i.e. 1 gwei = 1 nFLR)
   */
  cChainBaseFee(baseFeeNFLR: bigint | string): this {
    this._cChainBaseFeeNFLR = typeof baseFeeNFLR === 'string' ? BigInt(baseFeeNFLR) : baseFeeNFLR;
    return this;
  }
  ...
  protected buildFlareTransaction(): void {
    ...
    // If a live C-chain base fee was supplied, derive the correct import fee
    // using the coreth gas formula (ATOMIC_BASE + txBytes + numSigs×1000) × baseFee × 1.25.
    // The transaction serialisation is not yet available at this point, so we build a
    // temporary unsigned tx to measure the byte length, then recompute the fee and
    // build the final transaction with the correct output amount.
    if (this._cChainBaseFeeNFLR !== undefined) {
      this.transaction._fee.fee = this._computeFeeFromBaseFee(this._cChainBaseFeeNFLR).toString();
    }
  ...
  /**
   * Compute the required import fee using the live C-chain base fee.
   *
   * We build a temporary import transaction using a placeholder fee of 0 to
   * determine the serialised byte length, then apply the coreth formula:
   *   gas  = ATOMIC_TX_BASE_GAS + txBytes × TX_BYTES_GAS + numSigs × SECP256K1_VERIFY_GAS
   *   fee  = gas × baseFee × 1.25
   *
   * Using 0 as the placeholder is correct because the output amount (and therefore
   * byte length) changes negligibly between fee=0 and the real fee for a typical
   * import amount.
   */
  private _computeFeeFromBaseFee(baseFeeNFLR: bigint): bigint {
    const assetId = utils.cb58Encode(Buffer.from(this.transaction._assetId, 'hex'));
    const nativeUtxos = utils.decodedToUtxos(this.transaction._utxos, assetId);
    const signingAddresses = this.getSigningAddresses();

    const tempTx = evm.newImportTx(
      this.transaction._context,
      this.transaction._to[0],
      signingAddresses,
      nativeUtxos,
    ...
    const txBytes = BigInt(tempTx.toBytes().length);
    const numSigs = BigInt(
      this.transaction._utxos.reduce((sum, utxo) => sum + (utxo.threshold || this.transaction._threshold), 0)
    );

    return computeAtomicImportFee(txBytes, numSigs, baseFeeNFLR);
  }
