...
/**
 * Compute the total nFLR fee for a Flare C-chain atomic import transaction.
 *
 * The coreth formula is:
 *   gas = AtomicTxBaseCost(10_000) + txSizeBytes × TxBytesGas(1) + numSigs × Secp256k1FxVerifyCost(1_000)
 *   fee = gas × baseFeeGwei × paddingMultiplier
 *
 * The 25% padding (paddingMultiplier = 1.25) guards against base-fee volatility
 * between signing and broadcast.
 *
 * @param txSizeBytes - Serialised transaction size in bytes
 * @param numSigs - Total number of SECP256K1 signatures across all inputs
 * @param cChainBaseFeeNFLR - Live C-chain base fee in nFLR (nanoFLR = 1e-9 FLR)
 * @returns Total fee in nFLR (as bigint)
 */
export function computeAtomicImportFee(txSizeBytes: bigint, numSigs: bigint, cChainBaseFeeNFLR: bigint): bigint {
  const gas = ATOMIC_TX_BASE_GAS + TX_BYTES_GAS * txSizeBytes + SECP256K1_VERIFY_GAS * numSigs;
  const rawFee = gas * cChainBaseFeeNFLR;
  return (rawFee * BASE_FEE_PADDING_NUMERATOR + BASE_FEE_PADDING_DENOMINATOR - 1n) / BASE_FEE_PADDING_DENOMINATOR;
}
