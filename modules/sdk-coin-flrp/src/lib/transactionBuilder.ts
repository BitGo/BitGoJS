...
export abstract class TransactionBuilder extends BaseTransactionBuilder {
   ...
   * Hex strings are obtained from FlareJS getUTXOs API response
   * @param utxoHexStrings - Array of UTXO hex strings
   */
  utxos(utxoHexStrings: string[]): this {
    if (!utxoHexStrings || utxoHexStrings.length === 0) {
      throw new BuildTransactionError('UTXOs array cannot be empty');
    }
    // Store hex strings for later conversion
    this._transaction._utxoHexStrings = utxoHexStrings;
    // Parse to native UTXOs and convert to decoded format
    const nativeUtxos = utils.parseUtxoHexArray(utxoHexStrings);
    this._transaction._utxos = utils.utxosToDecoded(nativeUtxos, this._transaction._network);
    return this;
  }
