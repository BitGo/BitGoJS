...
describe('Flr P Key Pair', () => {
  describe('should create a valid KeyPair', () => {
    it('from an empty value', () => {
      ...
      should.exists(pubKey);
      should.exists(prvKey);
    });
    ...
    it('from a public key', () => {
      const keyPair = new KeyPair({ pub: testData.ACCOUNT_3.publicKey });
      should.equal(keyPair.getKeys().pub, testData.ACCOUNT_3.publicKey);
      should.exists(keyPair.getAddress());
    });

    it('Should get same address key for account 3 private key ', () => {
      const keyPair = new KeyPair({ prv: testData.ACCOUNT_3.privateKey });
      should.equal(keyPair.getKeys().prv, testData.ACCOUNT_3.privateKey);
      should.equal(keyPair.getKeys().pub, testData.ACCOUNT_3.publicKey);
    ...
    it('Should get same address key for account 4 private key ', () => {
      const keyPair = new KeyPair({ prv: testData.ACCOUNT_4.privateKey });
      should.equal(keyPair.getKeys().prv, testData.ACCOUNT_4.privateKey);
      should.equal(keyPair.getKeys().pub, testData.ACCOUNT_4.publicKey);
