...
import { erc7984Registry, getWrapperPair, getActiveWrapperPairs, Erc7984WrapperPair } from '../../src/erc7984Registry';
...
describe('ERC-7984 Statics Registry', function () {
  ...
  describe('getWrapperPair', function () {
    it('returns the correct Hoodi testnet pair for hteth:ctest1', function () {
      const pair = getWrapperPair(Networks.test.hoodi.name, '0x7b1d59bbcd291daa59cb6c8c5bc04de1afc4aba1');
    ...
    it('is case-insensitive for the wrapper address', function () {
      const lower = getWrapperPair(Networks.test.hoodi.name, '0x7b1d59bbcd291daa59cb6c8c5bc04de1afc4aba1');
      const mixed = getWrapperPair(Networks.test.hoodi.name, '0x7B1D59BbCD291daA59CB6C8C5bC04DE1aFC4ABA1');
    ...
    it('returns undefined for an address not in the registry', function () {
      const pair = getWrapperPair(Networks.test.hoodi.name, '0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef');
      (pair === undefined).should.be.true();
    });
    ...
    it('returns the correct mainnet pair for eth:cusdt', function () {
      const pair = getWrapperPair(Networks.main.ethereum.name, '0xae0207c757aa2b4019ad96edd0092ddc63ef0c50');
      pair.should.not.be.undefined();
      pair!.underlyingErc20Address.should.equal('0xdac17f958d2ee523a2206206994597c13d831ec7');
      pair!.requiresApprovalReset.should.be.true();
    });

    it('returns the correct mainnet pair for eth:cusdc', function () {
      const pair = getWrapperPair(Networks.main.ethereum.name, '0xe978f22157048e5db8e5d07971376e86671672b2');
      pair.should.not.be.undefined();
