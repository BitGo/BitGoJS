...
describe('Ada Keypair', () => {
  ...
  describe('Keypair creation', () => {
    ...
    it('initialization from private key', () => {
      ...
      should.equal(
        keyPair.getKeys().prv,
        toHex(CardanoWasm.PrivateKey.from_bech32(enterpriseAccounts.account1.secretKey).as_bytes())
      );
      ...
      should.equal(
        keyPair.getKeys().prv,
        toHex(CardanoWasm.PrivateKey.from_bech32(enterpriseAccounts.account2.secretKey).as_bytes())
      );
    ...
    it('initialization from public key', () => {
      ...
      should.equal(
        keyPair.getKeys().pub,
        toHex(CardanoWasm.PublicKey.from_bech32(enterpriseAccounts.account1.publicKey).as_bytes())
      );

      keyPair = new KeyPair({ pub: enterpriseAccounts.account2.publicKey });
      should.equal(keyPair.getAddress('testnet'), enterpriseAccounts.account2.baseAddress);
  ...
  describe('getAddress', () => {
    it('should get an address', () => {
      ...
      let address = keyPair.getAddress('testnet');
      ...
      address = keyPair.getAddress('testnet');
