...
describe('utils', () => {
  ...
  it('should validate signature correctly', () => {
    should.equal(
      utils.isValidSignature(toHex(CardanoWasm.Ed25519Signature.from_bech32(signatures.signature1).to_bytes())),
      true
    );
    should.equal(
      utils.isValidSignature(toHex(CardanoWasm.Ed25519Signature.from_bech32(signatures.signature2).to_bytes())),
      true
    );
  });
  ...
  it('should validate invalid signature correctly', () => {
    ...
    should.equal(utils.isValidSignature(privateKeys.prvKeyExtended), false); // this is a different ed25519 key that should return false
