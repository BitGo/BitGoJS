...
describe('CIP8 Utils', function () {
  ...
  describe('createCSLSigStructure', function () {
    it('should create valid signature structure', async function () {
      const message = 'Hello, Cardano!';
      const result = createCSLSigStructure(addressCborBytes, message);
      ...
      const decodedHeader = await Decoder.decodeFirst(result.protectedHeaderCborBytes);
      ...
      const decodedSigStructure = await Decoder.decodeFirst(result.sigStructureCborBytes);
  ...
  describe('constructCSLCoseObjects', function () {
    it('should construct valid COSE objects', function () {
      const message = 'Hello, Cardano!';
      const { protectedHeaderCborBytes, payloadBytes } = createCSLSigStructure(addressCborBytes, message);

      const signatureBytes = cip8TestResources.createTestSignature(message);
      const pubKey = cip8TestResources.createTestPublicKey();

      const result = constructCSLCoseObjects(protectedHeaderCborBytes, payloadBytes, signatureBytes, pubKey);
  ...
  describe('coseObjectsOutputToBuffer and bufferToCoseObjectsOutput', function () {
    it('should convert CSLCoseObjectsOutput to buffer and back', async function () {
      const message = 'Hello, Cardano!';
      const { protectedHeaderCborBytes, payloadBytes } = createCSLSigStructure(addressCborBytes, message);
      ...
      const originalObjects = constructCSLCoseObjects(protectedHeaderCborBytes, payloadBytes, signatureBytes, pubKey);

      const buffer = coseObjectsOutputToBuffer(originalObjects);
      should.ok(buffer.length > 0);

      const reconstructedObjects = await bufferToCoseObjectsOutput(buffer);
