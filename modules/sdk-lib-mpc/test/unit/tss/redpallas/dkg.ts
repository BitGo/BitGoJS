...
import { RedPallasMPSDkg, RedPallasMPSTypes } from '../../../../src/tss/redpallas-mps';
import { RedPallasDkgState } from '../../../../src/tss/redpallas-mps/types';
import { generateRedPallasDKGKeyShares } from './util';
...
describe('RedPallas MPS DKG', function () {
  ...
  describe('DKG Initialization', function () {
    it('should initialize DKG sessions for all parties', async function () {
      assert.strictEqual(user.getState(), RedPallasDkgState.Uninitialized);
      assert.strictEqual(backup.getState(), RedPallasDkgState.Uninitialized);
      assert.strictEqual(bitgo.getState(), RedPallasDkgState.Uninitialized);
      ...
      assert.strictEqual(user.getState(), RedPallasDkgState.Init);
      assert.strictEqual(backup.getState(), RedPallasDkgState.Init);
      assert.strictEqual(bitgo.getState(), RedPallasDkgState.Init);

      const userMessage = user.getFirstMessage();
      const backupMessage = backup.getFirstMessage();
      const bitgoMessage = bitgo.getFirstMessage();

      assert.strictEqual(user.getState(), RedPallasDkgState.WaitMsg1);
    ...
    it('should reject invalid decryption keys and party pubkey counts', async function () {
      await assert.rejects(
        user.initDkg(Buffer.alloc(31), [backupKP.pubKey, bitgoKP.pubKey]),
        /Missing or invalid decryption key/
      );
      await assert.rejects(user.initDkg(userKP.privKey, [backupKP.pubKey]), /Expected 2 other parties' public keys/);
    });
  });
  ...
  describe('DKG Protocol Execution', function () {
    ...
    it('should complete full DKG protocol and generate key shares', async function () {
      ...
      assert.strictEqual(backup.getState(), RedPallasDkgState.WaitMsg2);
      assert.strictEqual(bitgo.getState(), RedPallasDkgState.WaitMsg2);
      ...
      assert.strictEqual(backup.getState(), RedPallasDkgState.Complete);
      assert.strictEqual(bitgo.getState(), RedPallasDkgState.Complete);
  ...
  describe('Seed-based Key Generation', function () {
    ...
    it('should create key shares without party seeds (random)', async function () {
      ...
      const bitgoPk = bitgoDkg.getSharePublicKey().toString('hex');
    ...
    it('should generate valid reduced key shares', async function () {
      ...
      const bitgoReduced = bitgoDkg.getReducedKeyShare();
      ...
      const userDecoded = RedPallasMPSTypes.getDecodedReducedKeyShare(userReduced);
      const backupDecoded = RedPallasMPSTypes.getDecodedReducedKeyShare(backupReduced);
      const bitgoDecoded = RedPallasMPSTypes.getDecodedReducedKeyShare(bitgoReduced);
      ...
      assert.strictEqual(
        userPub,
        userDkg.getSharePublicKey().toString('hex'),
        'Reduced pub should match getSharePublicKey'
      );

      // keyShare must be present and non-empty (opaque WASM bincode)
      ...
      // RedPallas reduced shares do not carry a BIP32 rootChainCode
      assert.strictEqual('rootChainCode' in userDecoded, false, 'RedPallas reduced share has no rootChainCode');
    });
  });
  ...
  describe('Message Serialization', function () {
    it('should serialize and deserialize messages round-trip', async function () {
      ...
      const serialized = RedPallasMPSTypes.serializeMessages(r1Messages);
      assert(
  ...
  describe('Session Management', function () {
    it('should export and restore DKG session state', async function () {
      await user.initDkg(userKP.privKey, [backupKP.pubKey, bitgoKP.pubKey]);
      await backup.initDkg(backupKP.privKey, [userKP.pubKey, bitgoKP.pubKey]);
      await bitgo.initDkg(bitgoKP.privKey, [userKP.pubKey, backupKP.pubKey]);
      ...
      const restoredUser = new RedPallasMPSDkg.RedPallasDKG(3, 2, 0);
      const restoredBackup = new RedPallasMPSDkg.RedPallasDKG(3, 2, 1);
      const restoredBitgo = new RedPallasMPSDkg.RedPallasDKG(3, 2, 2);
