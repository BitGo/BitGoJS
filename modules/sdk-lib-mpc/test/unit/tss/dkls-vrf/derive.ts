...
describe('DKLS hard derive (VRF backed)', function () {
  ...
  it('should expose session data for restore and resume mid-protocol', async function () {
    ...
    const userRound1Snapshot = user.getSessionData();
    assert.equal(userRound1Snapshot.deriveState, DeriveState.Round1);
    const userFromRound1 = await DklsDrv.Derive.restoreSession(
      3,
      2,
      0,
    ...
    const userMsg2 = userFromRound1.handleIncomingMessages({
      p2pMessages: [],
    ...
    const snapshot = userFromRound1.getSessionData();
