...
export async function generateEdDsaDKGKeyShares(
  ...
  seedBackup?: Buffer,
  seedBitgo?: Buffer
): Promise<[EdDSA.DKG, EdDSA.DKG, EdDSA.DKG]> {
  const user = new EdDSA.DKG(3, 2, 0, seedUser);
  const backup = new EdDSA.DKG(3, 2, 1, seedBackup);
  const bitgo = new EdDSA.DKG(3, 2, 2, seedBitgo);

  const publicKeys = [user.getPublicKey(), backup.getPublicKey(), bitgo.getPublicKey()];
  const publicKeyConcat = EdDSA.concatBytes(publicKeys);
