...
import { DsgState } from '../../../../src/tss/eddsa-dkls/types';

/**
 * Helper function to execute the complete DSG protocol
 */
async function executeDSGProtocol(sessions: EdDSA.DSG[]): Promise<Buffer[]> {
 ...
 */
async function createDSGSessions(user: EdDSA.DKG, bitgo: EdDSA.DKG, msg: Buffer): Promise<EdDSA.DSG[]> {
  const userSession = new EdDSA.DSG(user.getKeyShare(), 0, 'm', msg);
  const bitgoSession = new EdDSA.DSG(bitgo.getKeyShare(), 2, 'm', msg);
...
describe('EdDSA DSG (Distributed Signature Generation)', function () {
  ...
  it('should complete DSG protocol and generate valid signatures', async function () {
    const sessions = await createDSGSessions(message);
  ...
  it('should produce different signatures for different messages', async function () {
    ...
    const sessions2 = await createDSGSessions(differentMessage);
  ...
  describe('DSG State Management', function () {
    it('should track state transitions correctly', async function () {
      const userSession = new EdDSA.DSG(user.getKeyShare(), 0, 'm', message);
      const bitgoSession = new EdDSA.DSG(bitgo.getKeyShare(), 2, 'm', message);
      ...
      // After getting first message, state should transition to WaitMsg1
      userSession.getFirstMessage();
      bitgoSession.getFirstMessage();
      assert.strictEqual(userSession.getState(), DsgState.WaitMsg1, 'State should be WaitMsg1 after getFirstMessage()');
      assert.strictEqual(
        bitgoSession.getState(),
        DsgState.WaitMsg1,
        'State should be WaitMsg1 after getFirstMessage()'
      );
    ...
    it('should enforce state-based method access', async function () {
      ...
      assert.throws(() => userSession.getFirstMessage(), /DSG session must be in Init state/);
      ...
      assert.throws(() => userSession.init(), /DSG session already initialized/);
    });
  });
});
