...
describe('EdDSA MPS DSG', function () {
  ...
  describe('DSG Initialization', function () {
    it('should accept valid inputs and produce a first message', async function () {
      ...
      await dsg.initDsg(userKeyShare, MESSAGE, 'm', 2);
    ...
    it('should throw on empty keyShare', async function () {
      const dsg = new EddsaMPSDsg.DSG(0);
      await assert.rejects(dsg.initDsg(Buffer.alloc(0), MESSAGE, 'm', 2), /Missing or invalid keyShare/);
    });

    it('should throw on empty message', async function () {
      const dsg = new EddsaMPSDsg.DSG(0);
      await assert.rejects(dsg.initDsg(userKeyShare, Buffer.alloc(0), 'm', 2), /Missing or invalid message/);
    });

    it('should throw when otherPartyIdx equals own partyIdx', async function () {
      const dsg = new EddsaMPSDsg.DSG(0);
      await assert.rejects(dsg.initDsg(userKeyShare, MESSAGE, 'm', 0), /Invalid otherPartyIdx/);
    });

    it('should throw when otherPartyIdx is out of range', async function () {
      const dsg = new EddsaMPSDsg.DSG(0);
      await assert.rejects(dsg.initDsg(userKeyShare, MESSAGE, 'm', 5), /Invalid otherPartyIdx/);
    });

    it('should throw when partyIdx is out of range', async function () {
      const dsg = new EddsaMPSDsg.DSG(7);
      await assert.rejects(dsg.initDsg(userKeyShare, MESSAGE, 'm', 0), /Invalid partyIdx/);
    });
  ...
  describe('DSG Protocol Execution (2-of-3)', function () {
    it('should complete full DSG between user (0) and bitgo (2) and produce identical signatures', async function () {
      ...
      await MPSUtil.executeTillRound(3, dsgA, dsgB, userKeyShare, bitgoKeyShare, MESSAGE, 'm');
    ...
    it('should produce a signature that verifies under the DKG public key', async function () {
    ...
    it('should sign the same message identically across all 2-of-3 party combinations', async function () {
      const userBackupSig = (await MPSUtil.executeTillRound(
        3,
        new EddsaMPSDsg.DSG(0),
      ...
      const backupBitgoSig = (await MPSUtil.executeTillRound(
        3,
        new EddsaMPSDsg.DSG(1),
        new EddsaMPSDsg.DSG(2),
        backupKeyShare,
      ...
      const userBitgoSig = (await MPSUtil.executeTillRound(
        3,
        new EddsaMPSDsg.DSG(0),
        new EddsaMPSDsg.DSG(2),
        userKeyShare,
    ...
    it('should sign arbitrary message lengths', async function () {
      ...
      const shortSig = (await MPSUtil.executeTillRound(
        3,
        new EddsaMPSDsg.DSG(0),
        new EddsaMPSDsg.DSG(2),
        userKeyShare,
      ...
      const longSig = (await MPSUtil.executeTillRound(
        3,
        new EddsaMPSDsg.DSG(0),
        new EddsaMPSDsg.DSG(2),
        userKeyShare,
    ...
    it('should throw when handleIncomingMessages is called after completion', async function () {
      const dsgA = new EddsaMPSDsg.DSG(0);
      await MPSUtil.executeTillRound(3, dsgA, new EddsaMPSDsg.DSG(2), userKeyShare, bitgoKeyShare, MESSAGE, 'm');
    ...
    it('should fail when parties sign different messages', async function () {
      ...
      await dsg1.initDsg(userKeyShare, Buffer.from('MESSAGE'), 'm', 2);
      await dsg2.initDsg(bitgoKeyShare, Buffer.from('DIFFERENT_MESSAGE'), 'm', 0);
  ...
  describe('Derivation Paths', function () {
    it('should produce different signatures for different derivation paths', async function () {
      const rootSig = (await MPSUtil.executeTillRound(
        3,
        new EddsaMPSDsg.DSG(0),
        new EddsaMPSDsg.DSG(2),
        userKeyShare,
      ...
      const derivedSig = (await MPSUtil.executeTillRound(
        3,
        new EddsaMPSDsg.DSG(0),
        new EddsaMPSDsg.DSG(2),
        userKeyShare,
  ...
  describe('Message Serialization', function () {
    it('should serialize and deserialize DSG messages round-trip', async function () {
      const dsgA = new EddsaMPSDsg.DSG(0);
      const dsgB = new EddsaMPSDsg.DSG(2);
      await dsgA.initDsg(userKeyShare, MESSAGE, 'm', 2);
      await dsgB.initDsg(bitgoKeyShare, MESSAGE, 'm', 0);
