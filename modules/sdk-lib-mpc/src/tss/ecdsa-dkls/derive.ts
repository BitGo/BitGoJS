...
export class Derive {
  ...
  async initDerive(): Promise<DeserializedBroadcastMessage> {
    ...
    if (this.seed && this.seed.length !== DERIVE_SEED_LENGTH) {
      throw Error(`Seed should be ${DERIVE_SEED_LENGTH} bytes, got ${this.seed.length}.`);
    }
    if (
  ...
  /**
   * Restore a session snapshot captured after initDerive() or round 1 processing.
   * Do not call this before initDerive() has created session bytes.
   * The round state is re-derived from the wasm session bytes, not the caller enum.
   */
