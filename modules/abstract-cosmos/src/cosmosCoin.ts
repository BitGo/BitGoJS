...
export class CosmosCoin extends BaseCoin {
  ...
  async recover(params: RecoveryOptions): Promise<CosmosLikeCoinRecoveryOutput> {
    ...
    // Step 2: Instantiate the ECDSA signer and fetch the address details
    ...
    // Step 3: Fetch account details such as accountNo, balance and check for sufficient funds once gasAmount has been deducted
    ...
    // Step 4: Once sufficient funds are present, construct the recover tx messsage
    const amount: Coin[] = [
      {
    ...
    // Step 5: Build the unsigned tx using the constructed message
