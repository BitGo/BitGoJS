  ...
  protected isValidTransactionPayload(payload: TransactionPayload): boolean {
    try {
      ...
      if (!(payload instanceof TransactionPayloadEntryFunction)) {
        console.error('Invalid transaction payload: not an entry function');
        return false;
      }

      // Create a temporary CustomTransaction to leverage existing validation
      const tempTransaction = new CustomTransaction(this._coinConfig);

      try {
        // Use the transaction's built-in parsing which includes validation
        tempTransaction['parseTransactionPayload'](payload);
        return true;
      } catch (error) {
        console.error('Transaction payload validation failed:', error.message);
        return false;
      }
    } catch (error) {
      console.error('Error validating transaction payload:', error);
      return false;
    }
  }
