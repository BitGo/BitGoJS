...
import { parse, type Exact } from "@blaze-cardano/data";
...
import {
  ...
  MIN_LOVELACE,
  RealfiSDKV1Family,
  type IClassifiedOrderAction,
  type IExecutionValidityContext,
  type IOrderInfo,
  type IRequestOrigin,
  type IV1FamilyConstructorParams,
} from "../v1/family.js";
import type {
  IV1FamilySchemas,
  IV1FamilyScripts,
...
import {
  MAX_DIFFUSION_RATE_SPAN_MS,
  nextDepositDiffusion,
  pendingRemaining,
  settledBacking,
} from "./diffusion.js";
 ...
 */
export class RealfiSDKV1_1Rc1<
  P extends Provider,
  W extends Wallet,
> extends RealfiSDKV1Family<P, W, SettingsV1, VaultDatumV1> {
  readonly version = "V1_1_Rc1" as const;

  private readonly nowFn: () => bigint;
  private readonly defaultDiffusionDurationMs?: bigint;
  private readonly executionValidityWindowMs: bigint;
  private readonly executionClockBackoffMs: bigint;
  private readonly diffusionShortfallThresholdMs: bigint;
  private readonly throwOnDiffusionWindowShortfall: boolean;
  /**
   ...
   */
  private executionNowMs?: bigint;
  private executionValidUntilMs?: bigint;
  ...
  static create<P extends Provider, W extends Wallet>(
    ...
    const protocolMintScript =
      new V1_1Rc1Types.V1_1Rc1ProtocolMintProtocolMintWithdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script;

    const protocolStakeScript =
      new V1_1Rc1Types.V1_1Rc1ProtocolStakeProtocolStakeWithdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script;

    const protocolManagementScript =
      new V1_1Rc1Types.V1_1Rc1ProtocolManagementProtocolManagementWithdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script;
    ...
    const yieldOracleBootstrap =
      params.yieldOracleBootstrap ?? YIELD_ORACLE_BOOTSTRAP_PLACEHOLDER;
    const yieldOracleScript =
      new V1_1Rc1Types.V1_1Rc1YieldOracleYieldOracleMint(
        {
    ...
    const distributionOracleScript =
      new V1_1Rc1Types.V1_1Rc1DistributionOracleDistributionOracleWithdraw(
        oneShotPolicyId,
        oracleNftPolicyId,
        enableTrace,
      ).Script;
    ...
    const migrationScript =
      new V1_1Rc1Types.V1_1Rc1ProtocolMigrationV1_0ToV1_1ProtocolMigrationV1_0ToV1_1Withdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script;
    ...
    const protocolOrchestratorScript =
      new V1_1Rc1Types.V1_1Rc1ProtocolOrchestratorProtocolOrchestratorWithdraw(
        oneShotPolicyId,
        protocolMintScript.hash(),
        protocolStakeScript.hash(),
        protocolManagementScript.hash(),
        yieldOracleValidatorHash,
        enableTrace,
      ).Script;
    ...
    const treasuryScript = params.useV0_1Treasury
      ...
      : new V1_1Rc1Types.V1_1Rc1TreasuryTreasurySpend(
          {
            transaction_id: params.treasuryBootstrap.txHash,
            output_index: params.treasuryBootstrap.outputIndex,
          },
    ...
    const orderScript = new V1_1Rc1Types.V1_1Rc1OrderOrderSpend(
      oneShotPolicyId,
    ...
    const stakingVaultScript = params.useV0_4StakingVault
      ...
      : new V1_1Rc1Types.V1_1Rc1StakingVaultStakingVaultSpend(
          {
            transaction_id: params.stakingVaultBootstrap.txHash,
            output_index: params.stakingVaultBootstrap.outputIndex,
          },
    ...
    return new RealfiSDKV1_1Rc1(
      ...
      // project_v1_1_rc1_vs_audited_v1_1_divergence); remap them here.
      {
        SignedPayload_ProtocolRedeemerV1:
          V1_1Rc1Types.SignedPayload_v1_1_ProtocolRedeemerV1,
        SignedRedeemer_ExtraProtocolRedeemerV1:
          V1_1Rc1Types.SignedRedeemer_v1_1_ExtraProtocolRedeemerV1,
      },
      {
        now: params.now,
  ...
  protected constructor(
    ...
    schemas: IV1FamilySchemas<SettingsV1, VaultDatumV1>,
    scripts: IV1FamilyScripts,
    cachedReferenceInputs: ICachedReferenceInputs | undefined,
    signingSchemas: IV1SigningSchemas,
    options: IV1_1Rc1Options,
  ) {
    super(
      blaze,
      params,
    ...
    this.executionClockBackoffMs =
      options.now === undefined ? DEFAULT_CLOCK_BACKOFF_MS : 0n;
    this.defaultDiffusionDurationMs = options.defaultDiffusionDurationMs;
    this.diffusionShortfallThresholdMs =
      options.diffusionShortfallThresholdMs ?? 0n;
    ...
    if (this.diffusionShortfallThresholdMs < 0n) {
      throw new RangeError(
        `diffusionShortfallThresholdMs must be non-negative, got ${this.diffusionShortfallThresholdMs} ms.`,
      );
    }
    this.throwOnDiffusionWindowShortfall =
      options.throwOnDiffusionWindowShortfall ?? false;
    this.executionValidityWindowMs =
      options.executionValidityWindowMs ?? DEFAULT_EXECUTION_VALIDITY_WINDOW_MS;
    const validitySpanMs =
      this.executionClockBackoffMs + this.executionValidityWindowMs;
    if (
      this.executionValidityWindowMs < 0n ||
      validitySpanMs > MAX_DIFFUSION_RATE_SPAN_MS
    ) {
      throw new RangeError(
        `Execution validity span must be at most 1 hour (${MAX_DIFFUSION_RATE_SPAN_MS} ms), got ${validitySpanMs} ms.`,
      );
    }
  ...
  protected override settledVaultBacking(
  ...
  protected override buildTreasuryRequest(
    datum: Parameters<
      RealfiSDKV1Family<P, W, SettingsV1, VaultDatumV1>["buildTreasuryRequest"]
    >[0],
    ...
    const action = datum.action as OrderActionV1;
    const diffusionEnd =
      "ODeposit" in action ? action.ODeposit.diffusion_end : 0n;
    return {
  ...
  protected override get splitsYield(): boolean {
    return true;
  }
   ...
   */
  async computeDepositAlpha(): Promise<IYieldSplitAlpha> {
    const stablecoinAssetId = Core.AssetId(
      this.stablecoinPolicyId + this.assetNameHex,
    );
    const { vaultUtxo } = await this.getVaultDatum();
    const vaultUSDr =
    ...
    return treasuryCirculating > 0n
      ? { numerator: vaultUSDr, denominator: treasuryCirculating }
      : { numerator: 0n, denominator: 1n };
  }

  protected override async buildDepositAction(
    requests: TreasuryRequestV1[],
    alpha?: IYieldSplitAlpha,
  ): Promise<
    Awaited<
      ReturnType<
        RealfiSDKV1Family<P, W, SettingsV1, VaultDatumV1>["buildDepositAction"]
      >
    >
  > {
    ...
    if (!alpha) {
      throw new Error(
        "v1_1_rc1: a deposit batch needs its yield-split alpha — pass the batch's stored alpha (created with computeDepositAlpha)",
      );
    }
    if (alpha.denominator <= 0n) {
      throw new Error(
        `v1_1_rc1: yield-split alpha denominator must be positive, got ${alpha.denominator}`,
      );
    }
    if (alpha.numerator < 0n || alpha.numerator > alpha.denominator) {
      throw new Error(
        `v1_1_rc1: yield-split alpha must be within [0, 1], got ${alpha.numerator}/${alpha.denominator}`,
      );
    }
    ...
    return { Deposit: { requests, alpha } } as unknown as Awaited<
      ReturnType<
        RealfiSDKV1Family<P, W, SettingsV1, VaultDatumV1>["buildDepositAction"]
      >
    >;
   ...
   */
  protected override resolveDepositYieldShares(
    totalYield: bigint,
    vaultUSDr: bigint,
    ...
    action: Parameters<
      RealfiSDKV1Family<
        ...
        SettingsV1,
        VaultDatumV1
      >["resolveDepositYieldShares"]
    >[3],
  ): { stakedYieldShare: bigint; unstakedYieldShare: bigint } {
    const deposit = (
      action as unknown as {
        Deposit?: { alpha: { numerator: bigint; denominator: bigint } };
      }
    ).Deposit;
    if (!deposit) {
      // Defensive: a non-Deposit action routed here → live-state split.
      return super.resolveDepositYieldShares(
        totalYield,
        vaultUSDr,
        treasuryCirculating,
        action,
    ...
    const { numerator, denominator } = deposit.alpha;
    // BigInt division truncates toward zero, matching Aiken quotient_integer.
    const stakedYieldShare =
      denominator > 0n ? (totalYield * numerator) / denominator : 0n;
    return {
      stakedYieldShare,
  ...
  protected override serializeOrchestratorWithdrawalRedeemer(
    redeemer: Parameters<
      RealfiSDKV1Family<
        P,
        W,
    ...
    } as unknown as Exact<typeof V1_1Rc1Types.ProtocolOrchestratorRedeemerV1>);
  }

  protected override updateDepositVaultOutput(
    tx: TxBuilder,
    ...
    for (const orderInfo of ctx.orderInfos) {
      const action = orderInfo.datum.action as OrderActionV1;
    ...
    const window = nextDepositDiffusion(
      parsedVaultDatum,
   ...
   */
  override async buildDepositOrderTx(params: {
    principal: bigint;
    yield: bigint;
    ...
    if (params.yield >= 0n) {
      ...
      valueToLock =
        totalUSDrBacking > 0n
          ? makeValue(MIN_LOVELACE, [
              reserveAssetId,
              usdrToReserveCeil(totalUSDrBacking, ra),
            ])
          : makeValue(MIN_LOVELACE);
    } else {
      ...
      const { unstakedYieldShare } = calculateYieldShares(
        params.yield,
        vaultUSDr,
        parsedTreasuryDatum.circulating_supply,
      );
  ...
  override async buildExecuteOrdersTx(
    params: Parameters<
      RealfiSDKV1Family<P, W, SettingsV1, VaultDatumV1>["buildExecuteOrdersTx"]
    >[0],
  ): Promise<TxBuilder> {
    ...
    this.executionNowMs = slotAlignedTimeMs(
      provider,
      wallNowMs - this.executionClockBackoffMs,
    );
    this.executionValidUntilMs = slotAlignedTimeMs(
      provider,
      wallNowMs + this.executionValidityWindowMs,
    );
    try {
  ...
  protected override async applyExecutionValidityBounds(
    tx: TxBuilder,
    context: IExecutionValidityContext<VaultDatumV1>,
  ): Promise<void> {
    ...
    const { actionType, parsedVaultDatum } = context;
    ...
    const needsBounds =
      ...
      ((actionType === "stake" || actionType === "unstake") &&
        (parsedVaultDatum?.pending_yield ?? 0n) > 0n);
    if (!needsBounds) {
      return;
    }
   ...
   */
  private rateTimeMs(): bigint {
    return this.executionNowMs ?? this.nowFn();
  }

  private resolveDiffusionEnd(params: {
    diffusionEnd?: bigint;
    diffusionDurationMs?: bigint;
  }): bigint {
    ...
    const duration =
      params.diffusionDurationMs ?? this.defaultDiffusionDurationMs;
  ...
  private checkDiffusionWindow(
    ...
    ctx: { stakedYieldShare: bigint; totalYield: bigint },
    windowEndMs: bigint,
    nowMs: bigint,
  ): void {
    ...
    if (windowEndMs > 0n) {
      ...
      if (remainingMs <= 0n) {
        message =
          `deposit diffusion window lapsed ${-remainingMs}ms before execution ` +
          `(diffusion_end=${windowEndMs}, execution time=${nowMs}); the staked ` +
          `yield settles INSTANTLY instead of diffusing. Re-create the order ` +
          `with a longer window to diffuse it.`;
      } else {
        message =
          `deposit diffusion window has only ${remainingMs}ms left at ` +
          `execution (diffusion_end=${windowEndMs}); the staked yield diffuses ` +
          `over that remainder.`;
      }
    } else {
      ...
      message =
        `deposit requests no diffusion window while ${pendingLeftUsdr} USDr has ` +
        `not yet diffused (of ${vaultIn.pending_yield} pending, at execution ` +
        `time ${nowMs}); the active window is CLEARED and that remainder ` +
        `settles instantly.`;
    }
   ...
   */
  async deployMigration(): Promise<TxBuilder> {
    return deployScript(
      this.blaze,
  ...
  registerMigrationStake(): TxBuilder {
    return this.blaze
      .newTransaction()
      .addRegisterStake(credentialFromScript(this.migrationScript));
  }
   ...
   */
  private async readLegacyVaultDatum(): Promise<{
    vaultUtxo: Core.TransactionUnspentOutput;
    circulatingSusdr: bigint;
  }> {
    const { utxo, parsedDatum } = await getDatumFromNFT(
      this.blaze,
      this.stakingVaultNFTAssetId,
      V1_0Types.VaultDatumV1,
    );
    const legacy = parsedDatum as V1_0Types.VaultDatumV1;
    return { vaultUtxo: utxo, circulatingSusdr: legacy.circulating_susdr };
  }
   ...
   */
  async getMigrateStatePayload(): Promise<{
    signedPayload: string;
    payloadHash: string;
  }> {
    const { vaultUtxo } = await this.readLegacyVaultDatum();
    ...
    const payload = { action: "MigrateState", nonce } as unknown as Parameters<
      RealfiSDKV1Family<
        P,
        W,
        SettingsV1,
  ...
  async buildMigrateStateTx(params: {
    ...
    const signedPayload = parse(
      this.signing.SignedPayload_ProtocolRedeemerV1,
      PlutusData.fromCbor(HexBlob(params.signedPayload)),
    ) as Parameters<
      RealfiSDKV1Family<
        P,
        W,
    ...
    const { vaultUtxo, circulatingSusdr } = await this.readLegacyVaultDatum();
    ...
    const { proxyUtxo } = await this.getParsedProxyDatum();
    ...
    const feeUtxos = walletUtxos.filter(
      (u) => !excluded.has(utxoKey(u.input())),
    );

    // Vault input index in the ledger-sorted input set (txHash, then index).
    const sortedInputRefs = [
      vaultUtxo.input(),
      ...feeUtxos.map((u) => u.input()),
    ].sort((a, b) => {
    ...
    const vaultInputIdx = BigInt(
      sortedInputRefs.findIndex(
        (r) =>
          r.transactionId().toString() ===
            vaultUtxo.input().transactionId().toString() &&
          r.index() === vaultUtxo.input().index(),
      ),
    );
    ...
    const migratedDatum: VaultDatumV1 = {
    ...
    tx.lockAssets(
      ...
      Data.serialize(this.schemas.VaultDatumV1, migratedDatum),
    );
    ...
    tx.addWithdrawal(
      Core.RewardAccount.fromCredential(
        { type: Core.CredentialType.ScriptHash, hash: this.protocolScriptHash },
        this.network,
      ),
      0n,
      orchestratorRedeemer,
    );
