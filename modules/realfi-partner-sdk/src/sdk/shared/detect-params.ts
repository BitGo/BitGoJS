...
import { NETWORK_REGISTRY, type TNetworkPreset } from "./network-registry.js";
...
export type TDetectedSDKParams =
...
export interface IDetectInput {
  ...
  treasuryBootstrap: { txHash: Core.TransactionId; outputIndex: bigint };
  stakingVaultBootstrap: { txHash: Core.TransactionId; outputIndex: bigint };
  /**
   ...
   */
  yieldOracleBootstrap?: { txHash: Core.TransactionId; outputIndex: bigint };
  assetNameHex: string;
 ...
 */
export type TBlueprintValidators =
 ...
 */
export function selectBlueprintGeneration(
  logicHash: Core.ScriptHash,
  validators: TBlueprintValidators | undefined,
  ...
  return generations.find(
    (generation) =>
      resolveBlueprintVersion(logicHash, generation) !== undefined,
  );
}
 ...
 */
const BLUEPRINT_ORCHESTRATOR_VERSIONS: ReadonlyArray<
  readonly [prefix: string, version: TDetectedSDKParams["version"]]
> = [
  ["v1_1_rc1/", "V1_1_Rc1"],
  ["v1_0_rc1/", "V1_0_Rc1"],
  ["v1_0/", "V1_0"],
...
export function resolveBlueprintVersion(
  logicHash: Core.ScriptHash,
  validators: Readonly<Record<string, string>> | undefined,
): TDetectedSDKParams["version"] | undefined {
  if (!validators) return undefined;
  ...
  for (const [key, hash] of Object.entries(validators)) {
    ...
    const match = BLUEPRINT_ORCHESTRATOR_VERSIONS.find(([prefix]) =>
 ...
 */
export function computeOneShotPolicyId(
  proxyBootstrap: IDetectInput["proxyBootstrap"],
  enableTrace: boolean,
 ...
 */
export async function fetchProxyLogicSnapshot(
  provider: Provider,
  oneShotPolicyId: Core.Hash28ByteBase16,
): Promise<IProxyLogicSnapshot> {
  const { datum: rawDatum } = await readSingletonDatum(
    provider,
    Core.AssetId(oneShotPolicyId),
  );
  ...
  if (!logicBytes) {
    throw new Error(
      "First constructor field is not bytes (expected ScriptHash)",
    );
  }
  return {
    logicHash: toHex(logicBytes) as Core.ScriptHash,
    datumFields: fields,
  };
}
 ...
 */
export async function detectSDKParams(
  provider: Provider,
  config: IDetectInput,
...
export async function detectSDKParams(
  provider: Provider,
  config: IDetectInput | TNetworkPreset,
): Promise<TDetectedSDKParams> {
  if (typeof config === "string" && !Object.hasOwn(NETWORK_REGISTRY, config)) {
    throw new Error(
      `Unknown network preset "${config}" — expected one of: ${Object.keys(
        NETWORK_REGISTRY,
      ).join(", ")}`,
    );
  }
  const resolved =
    typeof config === "string"
      ? NETWORK_REGISTRY[config as TNetworkPreset]
      : config;
  const enableTrace = resolved.enableTrace ?? false;

  // ── Phase 1: Compute oneshot policy and fetch proxy datum ─────────────────

  const oneShotPolicyId = computeOneShotPolicyId(
    resolved.proxyBootstrap,
    enableTrace,
  );
 ...
 */
export async function matchProtocolVersion(
  snapshot: IProxyLogicSnapshot,
  oneShotPolicyId: Core.Hash28ByteBase16,
  config: IDetectInput,
): Promise<TDetectedSDKParams> {
  const enableTrace = config.enableTrace ?? false;
  const { logicHash, datumFields: fields } = snapshot;
  ...
  const deployedGeneration = selectBlueprintGeneration(
    logicHash,
    config.protocolValidators,
  );
  ...
  const suppliedGenerations = Array.isArray(config.protocolValidators)
    ? config.protocolValidators
    : config.protocolValidators
      ? [config.protocolValidators]
      : [];
  const hasAuthoritativeBlueprint = suppliedGenerations.some(
    (generation) => Object.keys(generation).length > 0,
  );
  const matchesV1Version = (
    derivedHash: Core.ScriptHash,
    version: "V1_0" | "V1_0_Rc1" | "V1_1_Rc1",
  ): boolean =>
    hasAuthoritativeBlueprint
      ? blueprintVersion === version
      : logicHash === derivedHash;
  ...
  // V1_1_Rc1 match on the first attempt with no failed-mismatch chunk fetches.
  {
    ...
    type TSettingsV1_1Rc1 = typeof SettingsV1_1Rc1Schema extends {
      static: infer T;
    }
      ? T
      : never;

    const protocolMintHashV1_1Rc1 = new V1_1Rc1ProtocolMintProtocolMintWithdraw(
      oneShotPolicyId,
      enableTrace,
    ).Script.hash();
    const protocolStakeHashV1_1Rc1 =
      new V1_1Rc1ProtocolStakeProtocolStakeWithdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script.hash();
    const protocolManagementHashV1_1Rc1 =
      new V1_1Rc1ProtocolManagementProtocolManagementWithdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script.hash();
    ...
    const yieldOracleBootstrap =
      config.yieldOracleBootstrap ?? YIELD_ORACLE_BOOTSTRAP_PLACEHOLDER;
    {
      ...
      const orchestratorHashV1_1Rc1 =
        new V1_1Rc1ProtocolOrchestratorProtocolOrchestratorWithdraw(
          oneShotPolicyId,
          protocolMintHashV1_1Rc1,
          protocolStakeHashV1_1Rc1,
          protocolManagementHashV1_1Rc1,
          oracleNftPolicyIdV1_1Rc1,
          enableTrace,
        ).Script.hash();
      ...
      if (matchesV1Version(orchestratorHashV1_1Rc1, "V1_1_Rc1")) {
        ...
        const { V0_1TreasuryTreasurySpend } =
          await import("../../generated-types/v0_1/index.js");
        const { V0_4StakingVaultStakingVaultSpend } =
        ...
        const useV0_4StakingVault =
          registry.staking_vault === v04StakingVaultHash;
  ...
  // matching early avoids failed-mismatch chunk fetches.
  {
    ...
    type TSettingsV1Rc1 = typeof SettingsV1Rc1Schema extends {
      static: infer T;
    }
      ? T
      : never;

    const protocolMintHashRc1 = new V1_0Rc1ProtocolMintProtocolMintWithdraw(
      oneShotPolicyId,
      enableTrace,
    ).Script.hash();
    const protocolStakeHashRc1 = new V1_0Rc1ProtocolStakeProtocolStakeWithdraw(
      oneShotPolicyId,
      enableTrace,
    ).Script.hash();
    const protocolManagementHashRc1 =
      new V1_0Rc1ProtocolManagementProtocolManagementWithdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script.hash();
    const orchestratorHashRc1 =
      new V1_0Rc1ProtocolOrchestratorProtocolOrchestratorWithdraw(
        oneShotPolicyId,
        protocolMintHashRc1,
        protocolStakeHashRc1,
  ...
  // V1_0 — orchestrator hash + backward-compat flag detection
  {
    ...
    type TSettingsV1 = typeof SettingsV1Schema extends { static: infer T }
      ? T
      : never;

    const protocolMintHash = new V1_0ProtocolMintProtocolMintWithdraw(
      oneShotPolicyId,
      enableTrace,
    ).Script.hash();
    const protocolStakeHash = new V1_0ProtocolStakeProtocolStakeWithdraw(
      oneShotPolicyId,
      enableTrace,
    ).Script.hash();
    const protocolManagementHash =
      new V1_0ProtocolManagementProtocolManagementWithdraw(
        oneShotPolicyId,
        enableTrace,
      ).Script.hash();
    const orchestratorHash =
      new V1_0ProtocolOrchestratorProtocolOrchestratorWithdraw(
        oneShotPolicyId,
        protocolMintHash,
        protocolStakeHash,
        protocolManagementHash,
        enableTrace,
      ).Script.hash();

    if (matchesV1Version(orchestratorHash, "V1_0")) {
      const settingsData = fields.get(1);
      const settings = parse(SettingsV1Schema, settingsData) as TSettingsV1;
      const registry = settings.registry;
  ...
  {
    const { V0_4ProtocolProtocolWithdraw } =
      await import("../../generated-types/v0_4/index.js");
    const v04ProtocolHash = new V0_4ProtocolProtocolWithdraw(
      oneShotPolicyId,
      enableTrace,
    ).Script.hash();
...
export class UnknownProtocolVersionError extends Error {
  constructor(public readonly logicHash: string) {
    super(
      `Unrecognized protocol logic hash "${logicHash}". ` +
        `The on-chain proxy datum references a script that does not match any known SDK version. ` +
        `Update the SDK or check that you are connected to the correct network.`,
    );
