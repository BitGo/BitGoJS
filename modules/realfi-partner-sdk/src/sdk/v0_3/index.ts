...
import {
  ...
  getSignedPayloadForDeposit,
  getSignedPayloadForWithdraw,
  parseCancelOwner,
...
import type { Request } from "../../generated-types/v0_3/index.js";
 ...
 */
interface ITreasuryState {
  treasuryUtxo: Core.TransactionUnspentOutput;
  treasuryValue: Core.Value;
 ...
 */
export class RealfiSDKV0_3<P extends Provider, W extends Wallet>
  extends RealfiSDKBase<P, W>
  implements IRealfiSDKWithTreasury
{
  readonly version = "V0_3" as const;
  ...
  static create<P extends Provider, W extends Wallet>(
    ...
    const protocolScript = new V0_3Types.V0_3ProtocolProtocolWithdraw(
      oneShotPolicyId,
      enableTrace,
    ).Script;
    ...
    const orderScript = new V0_3Types.V0_3OrderOrderSpend(
      oneShotPolicyId,
      enableTrace,
    ).Script;

    return new RealfiSDKV0_3(
      blaze,
      {
        version: "V0_3",
   ...
   */
  private parseOrderInfos(orderUtxos: Core.TransactionUnspentOutput[]): {
    orderInfos: IOrderInfo[];
    actionType: "mint" | "burn";
  } {
    const orderInfos: IOrderInfo[] = [];
    let actionType: "mint" | "burn" | null = null;
    ...
    for (const utxo of orderUtxos) {
      ...
      const datum = parse(V0_3Types.OrderDatum, datumData) as OrderDatum;

      let action: "mint" | "burn";
      let amount: bigint;
      ...
      orderInfos.push({ utxo, datum, action, amount });
    }
   ...
   */
  private async calculateTreasuryState(
    totalAmount: bigint,
    actionType: "mint" | "burn",
    reserveTokenAssetId: Core.AssetId,
  ): Promise<ITreasuryState> {
    const { treasuryUtxo, parsedTreasuryDatum } = await this.getTreasuryDatum();
    ...
    if (actionType === "burn") {
      if (currentReserve === 0n) {
        throw new Error(
          "Treasury has no reserve tokens. Cannot execute burn orders.",
        );
      }
      // totalAmount is negative for burns, so newReserve < 0 means insufficient funds
      if (newReserve < 0n) {
        throw new Error(
          `Insufficient reserve tokens in treasury. Treasury has ${currentReserve} but burn requires ${-totalAmount}.`,
        );
      }
    }
  ...
  private addDestinationOutputs(
    ...
    for (const orderInfo of orderInfos) {
      ...
      if (actionType === "mint") {
        const outputValue = makeValue(2000000n, [
          stablecoinAssetId,
          orderInfo.amount,
        ]);
  ...
  private updateTreasury(
    tx: TxBuilder,
    treasuryState: ITreasuryState,
    totalAmount: bigint,
    reserveTokenAssetId: Core.AssetId,
  ): void {
    const newTreasuryDatum: V0_1TreasuryDatum = {
      circulating_supply:
        treasuryState.parsedTreasuryDatum.circulating_supply + totalAmount,
    };
    ...
    const newTreasuryValue = Value.merge(
      makeValue(treasuryState.treasuryValue.coin(), [
        this.treasuryNFTAssetId,
        1n,
      ]),
      makeValue(0n, [reserveTokenAssetId, treasuryState.newReserve]),
    );
    tx.lockAssets(
  ...
  async buildOrderTx({
    ...
    if (!owner) {
      ...
      if (scriptTypes.length !== 1 || scriptTypes[0] !== "Signature") {
        ...
        throw new Error(
          "Only Signature multisig script is currently supported for owner",
        );
      }
    }
    const orderAction =
    ...
    const reserveTokenAssetId = Core.AssetId(
      proxyDatum.parsedProxyDatum.settings.reserve_token.join(""),
    );
    ...
    if (action === "mint") {
      ...
      valueToLock = makeValue(1000000n, [
        reserveTokenAssetId,
        reserveTokenAmount,
      ]);
    } else {
  ...
  async getSignedPayloadFromOrderInputs(
    orderUtxos: Core.TransactionInput[],
  ): Promise<string> {
    ...
    let actionType: "OMint" | "ORedeem" | null = null;
    ...
    for (const utxo of resolvedUtxos) {
      ...
      requestList.push(
        this.buildRequestFromUtxo(utxo, parsed.amount, parsed.destination),
      );
    }
    ...
    const action: V0_3Types.ProtocolRedeemer =
      actionType === "OMint"
        ? { Mint: { requests: requestList } }
        : { Burn: { requests: requestList } };
    ...
    const serialized = Data.serialize(
      V0_3Types.SignedPayload_ProtocolRedeemer,
      signedPayload,
    );
   ...
   */
  private parseOrderActionForPayload(utxo: Core.TransactionUnspentOutput): {
    actionType: "OMint" | "ORedeem";
    amount: bigint;
    destination: V0_3Types.Destination;
  } {
  ...
  async buildExecuteOrdersTx(params: {
    ...
    const { orderInputs, signedPayload, signatures } = params;
    ...
    const { orderInfos, actionType } = this.parseOrderInfos(orderUtxos);
    ...
    if (!protocolRefInput || !orderRefInput || !treasuryRefInput) {
      throw new Error(
        "Missing reference script inputs. Make sure all scripts are deployed.",
      );
    }
    ...
    const reserveTokenAssetId = Core.AssetId(
      parsedProxyDatum.settings.reserve_token.join(""),
    );
    const stablecoinAssetId = Core.AssetId(
      this.stablecoinPolicyId + this.assetNameHex,
    );
    ...
    const treasuryState = await this.calculateTreasuryState(
      totalAmount,
      actionType,
      reserveTokenAssetId,
    );
    ...
    const requestToOutputs: Record<number, bigint> = {};
    ...
    const serializedSignedRedeemer = Data.serialize(
      ...
      {
        extra: { request_to_outputs: requestToOutputs },
  ...
  async buildCancelOrdersTx(params: {
    ...
    const { orderInputs, destination, versionHint } = params;
  ...
  async buildWithdrawTx(
    ...
    const serializedPayload = getSignedPayloadForWithdraw(
      assetAmount.amount,
      treasuryUtxo!,
    );
    ...
    const tx = this.blaze
      .newTransaction()
      .addWithdrawal(rewardAccount, 0n, serializedSignedRedeemer)
  ...
  async buildDepositTx(assetAmount: AssetAmount): Promise<TxBuilder> {
    ...
    const serializedPayload = getSignedPayloadForDeposit(
      assetAmount.amount,
      treasuryUtxo!,
    );
    // Build SignedRedeemer_ExtraProtocolRedeemer
