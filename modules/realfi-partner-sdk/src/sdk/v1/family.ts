...
import {
  screenOrderAction,
  screenDepositBatch,
  screenOrderUtxoFacts,
  type IOrderUtxoFacts,
  type TOrderSanityResult,
} from "./order-sanity.js";
import type {
  IProxyDatum,
  IV1FamilySchemas,
  IV1FamilyScripts,
 ...
 */
export const DIRECT_ACTION_PADDING_ASSET: Asset = [
  "00".repeat(28),
...
const DEPLOYED_VALIDATOR_FIELDS = [
  [
    "protocol_orchestrator.protocol_orchestrator.withdraw",
    "protocolScriptHash",
  ],
  ["protocol_mint.protocol_mint.withdraw", "protocolMintScriptHash"],
  ["protocol_stake.protocol_stake.withdraw", "protocolStakeScriptHash"],
  [
    "protocol_management.protocol_management.withdraw",
    "protocolManagementScriptHash",
  ],
  ["order.order.spend", "orderScriptHash"],
  ["treasury.treasury.spend", "treasuryScriptHash"],
  ["staking_vault.staking_vault.spend", "stakingVaultScriptHash"],
] as const;
...
type TV1ProtocolVersion = keyof typeof DEPLOYED_VALIDATOR_PREFIXES;

const COMPATIBLE_STATE_VALIDATOR_FIELDS = new Set([
  "treasuryScriptHash",
  "stakingVaultScriptHash",
]);
...
export function calculateYieldShares(
  totalYield: bigint,
  vaultUSDr: bigint,
  ...
  const stakedYieldShare =
    treasuryCirculating > 0n
      ? (totalYield * vaultUSDr) / treasuryCirculating
      : 0n;
 ...
 */
function assertPositiveMinReceived(label: string, minReceived: bigint): void {
  if (minReceived <= 0n) {
    throw new Error(
      `${label} minReceived must be positive (got ${minReceived}); an order with a ` +
        "non-positive min_received can never execute and crashes every batch it joins",
    );
  }
}

function buildOrderDestinationValue(
  orderInfo: IOrderInfo,
  consumedAssets: Array<[Core.AssetId, bigint]>,
  deliveredAssets: Array<[Core.AssetId, bigint]> = [],
): Core.Value {
 ...
 */
export abstract class RealfiSDKV1Family<
  P extends Provider,
  W extends Wallet,
  TSettings,
  TVaultDatum extends IVaultDatumLike,
>
  ...
  implements IRealfiSDKWithTreasury
{
  ...
  protected readonly protocolScript: Core.Script; // Alias for orchestrator (base class compatibility)
  ...
  private readonly signingSchemas?: IV1SigningSchemas;

  protected constructor(
    blaze: Blaze<P, W>,
    params: IV1FamilyConstructorParams,
    schemas: IV1FamilySchemas<TSettings, TVaultDatum>,
    scripts: IV1FamilyScripts,
    cachedReferenceInputs?: ICachedReferenceInputs,
    signingSchemas?: IV1SigningSchemas,
  ) {
    super(
      blaze,
      {
  ...
  private applyDeployedValidators(
    deployed: Readonly<Record<string, string>> | undefined,
    version: TProtocolVersion,
  ): void {
    if (!deployed) return;

    if (!(version in DEPLOYED_VALIDATOR_PREFIXES)) return;
    const prefix = DEPLOYED_VALIDATOR_PREFIXES[version as TV1ProtocolVersion];

    const mutable = this as unknown as Record<string, unknown>;
    for (const [suffix, field] of DEPLOYED_VALIDATOR_FIELDS) {
      const versionedHash = deployed[`${prefix}${suffix}`];
      const activeStateHash = COMPATIBLE_STATE_VALIDATOR_FIELDS.has(field)
        ? deployed[suffix]
        : undefined;
      ...
      const addressField =
        DEPLOYED_ADDRESS_FIELDS[field as keyof typeof DEPLOYED_ADDRESS_FIELDS];
      if (addressField) {
        mutable[addressField] = addressFromCredentials(
          this.network,
  ...
  protected get signing(): IV1SigningSchemas {
    if (!this.signingSchemas) {
      throw new Error(
        `${this.version}: no signing schemas supplied — pass signingSchemas to the family constructor or override getSignedPayloadFromOrderInputs and buildExecuteOrdersTx`,
      );
    }
   ...
   */
  protected abstract buildUpdatedVaultDatum(
   ...
   */
  protected settledVaultBacking(
    _parsedVaultDatum: TVaultDatum,
    vaultUSDr: bigint,
  ): bigint {
    return vaultUSDr;
  }
   ...
   */
  protected async applyExecutionValidityBounds(
    _tx: TxBuilder,
    _context: IExecutionValidityContext<TVaultDatum>,
  ): Promise<void> {
    // no-op by default
  }
   ...
   */
  protected async buildDepositAction(
    requests: TreasuryRequestV1[],
    alpha?: IYieldSplitAlpha,
  ): Promise<ProtocolRedeemerV1> {
    if (alpha) {
      throw new Error(
        `${this.version}: a deposit batch takes no yield-split alpha (the validator recomputes the split from live state)`,
      );
    }
   ...
   */
  protected resolveDepositYieldShares(
    totalYield: bigint,
    vaultUSDr: bigint,
    treasuryCirculating: bigint,
    _action: ProtocolRedeemerV1,
  ): { stakedYieldShare: bigint; unstakedYieldShare: bigint } {
  ...
  protected serializeOrchestratorWithdrawalRedeemer(redeemer: {
    ...
    return Data.serialize(
      this.signing.SignedRedeemer_ExtraProtocolRedeemerV1,
      redeemer,
    );
  }
  ...
  async mintStakingVaultNFT(
    ...
    initialDatum?: TVaultDatum,
  ): Promise<{ tx: TxBuilder; nftAssetId: Core.AssetId }> {
    const vaultPolicyId = Core.PolicyId(this.stakingVaultScriptHash);
    const vaultAssetName = Core.AssetName(toHex(Buffer.from("staking_vault")));

    const datum = Data.serialize(
      this.schemas.VaultDatumV1,
      initialDatum ?? this.buildInitialVaultDatum(),
    );
    const tx = await this.blaze
      .newTransaction()
   ...
   */
  async deployProtocolMint(): Promise<TxBuilder> {
    return deployScript(
      this.blaze,
      this.protocolMintScript,
      this.scriptDeploymentAddress,
    );
  }
   ...
   */
  async deployProtocolStake(): Promise<TxBuilder> {
    return deployScript(
      this.blaze,
      this.protocolStakeScript,
      this.scriptDeploymentAddress,
    );
  }
   ...
   */
  async deployProtocolManagement(): Promise<TxBuilder> {
    return deployScript(
      this.blaze,
      this.protocolManagementScript,
      this.scriptDeploymentAddress,
    );
  }
  ...
  registerAllStakes(): TxBuilder {
    return this.blaze
      .newTransaction()
      .addRegisterStake(credentialFromScript(this.protocolScript))
      .addRegisterStake(credentialFromScript(this.protocolMintScript))
      .addRegisterStake(credentialFromScript(this.protocolStakeScript))
      .addRegisterStake(credentialFromScript(this.protocolManagementScript))
      .addRegisterStake(credentialFromScript(this.stakingVaultScript));
  }
   ...
   */
  deployScriptsBatched(budgetBytes = 13_000): TxBuilder[] {
    ...
    const batches: Core.Script[][] = [];
    let current: Core.Script[] = [];
    let currentBytes = 0;
  ...
  async getVaultDatum(): Promise<IVaultDatumResult<TVaultDatum>> {
    const {
      utxo: vaultUtxo,
      datum: vaultDatum,
    ...
    return {
      vaultUtxo,
      vaultDatum,
      parsedVaultDatum: parsedVaultDatum as TVaultDatum,
    };
  }
  ...
  async updateOneShotDatum(
    receiverAddress: Core.Address,
    newDatum: IProxyDatum<TSettings>,
  ): Promise<TxBuilder> {
  ...
  async getParsedProxyDatum(): Promise<
    IProxyDatumResult<IProxyDatum<TSettings>>
  > {
    if (this.cachedProxyDatumResult) {
      return this.cachedProxyDatumResult as IProxyDatumResult<
        IProxyDatum<TSettings>
      >;
    }
   ...
   */
  async isProtocolUpgraded(): Promise<boolean> {
   ...
   */
  async getSettingsConfig(): Promise<TV1SettingsConfig> {
  ...
  async batchNeedsAlpha(
    ...
    return (
      this.classifyOrderUtxo(utxo!).action.actionType === "deposit" &&
      this.splitsYield
    );
  }
  ...
  classifyOrderUtxo(utxo: Core.TransactionUnspentOutput): IClassifiedOrderUtxo {
    ...
    const datum = parse(this.schemas.OrderDatumV1, datumData) as OrderDatumV1;
    return { datum, action: this.classifyOrderAction(datum) };
  }
   ...
   */
  protected async _buildOrderTx(params: {
    ...
    const serializedDatum = this.serializeOrderDatum(
      params.action,
      params.destination,
      owner,
      params.data,
  ...
  protected async resolveOrderOwner(
    owner?: MultisigScript,
  ...
  async buildStakeOrderTx(params: {
    ...
    const minReceived =
      params.minReceived ??
      (await this.computeStakeMinReceived(
        params.amount,
  ...
  async buildStakeContinuation(
    ...
    if (params.swap.minReceived.metadata.assetId !== expectedSundaeAssetId) {
      throw new Error(
        `Swap minReceived must be USDr (${expectedSundaeAssetId}), got ${params.swap.minReceived.metadata.assetId}`,
      );
    }

    const minReceived = await this.computeStakeMinReceived(
      amount,
      params.slippageToleranceBps,
    );
    assertPositiveMinReceived("Stake", minReceived);
  ...
  protected async _buildUnstakeOrderTx(params: {
    amount: bigint;
    destination: Destination;
    forfeit?: bigint;
    minReceived?: bigint;
    ...
    const minReceived =
      params.minReceived ??
      (await this.computeUnstakeMinReceived(
        params.amount,
        forfeit,
        params.slippageToleranceBps,
      ));
   ...
   */
  protected async computeStakeMinReceived(
    amount: bigint,
    perCallToleranceBps?: bigint,
  ): Promise<bigint> {
    const toleranceBps =
      this.resolveStakeSlippageToleranceBps(perCallToleranceBps);
    const stablecoinAssetId = Core.AssetId(
      this.stablecoinPolicyId + this.assetNameHex,
    );
    const { vaultUtxo, parsedVaultDatum } = await this.getVaultDatum();
    const vaultUsdr = this.settledVaultBacking(
      parsedVaultDatum,
      vaultUtxo.output().amount().multiasset()?.get(stablecoinAssetId) ?? 0n,
    );
  ...
  protected resolveStakeSlippageToleranceBps(
    perCallToleranceBps?: bigint,
  ): bigint {
    const toleranceBps =
      perCallToleranceBps ?? this.defaultSlippageToleranceBps ?? 50n;
   ...
   */
  protected async computeUnstakeMinReceived(
    amount: bigint,
    forfeit: bigint,
    perCallToleranceBps?: bigint,
  ): Promise<bigint> {
    ...
    const circulatingSusdr = parsedVaultDatum.circulating_susdr;
    const expectedGross =
      vaultUsdr === 0n || circulatingSusdr === 0n
        ? amount
        : (amount * vaultUsdr) / circulatingSusdr;
  ...
  async buildDirectMintOrderTx(params: {
    ...
    return this._buildOrderTx({
      action: { ODirectMint: { amount: params.amount } },
      valueToLock: makeValue(MIN_LOVELACE),
      owner: params.owner,
      destination: params.destination,
      data: params.data,
  ...
  async buildDirectBurnOrderTx(params: {
    ...
    return this._buildOrderTx({
      action: { ODirectBurn: { amount: params.amount } },
      valueToLock: makeValue(MIN_LOVELACE, [stablecoinAssetId, params.amount]),
      owner: params.owner,
      destination: params.destination,
      data: params.data,
  ...
  static buildMultisigTimelockNativeScript = buildMultisigTimelockNativeScript;
  ...
  async getSignedPayloadFromOrderInputs(
    ...
    const nonce = buildNonceFromUtxo(sortedInputs[0]!) as Nonce;

    const resolvedUtxos =
      await this.blaze.provider.resolveUnspentOutputs(sortedInputs);

    let actionType: TOrderActionType | null = null;
    const exchangeRequests: ExchangeRequestV1[] = [];
    const treasuryRequests: TreasuryRequestV1[] = [];
    const stakeRequests: StakeRequestV1[] = [];
    const directRequests: DirectRequestV1[] = [];
    ...
    for (const utxo of resolvedUtxos) {
      ...
      if (parsed.actionType === "mint" || parsed.actionType === "burn") {
        ...
        parsed.actionType === "deposit" ||
        parsed.actionType === "withdraw"
      ) {
        treasuryRequests.push(this.buildTreasuryRequest(datum, parsed, origin));
      } else if (
        parsed.actionType === "stake" ||
        parsed.actionType === "unstake"
      ) {
        stakeRequests.push({
          destination: datum.destination,
          amount: parsed.amount,
    ...
    if (alpha && actionType !== "deposit") {
      throw new Error(
        `a ${actionType} batch takes no yield-split alpha (only a deposit splits yield)`,
      );
    }
    ...
    let action: ProtocolRedeemerV1;
    switch (actionType) {
      case "mint":
        action = { Mint: { requests: exchangeRequests } };
        break;
      case "burn":
        action = { Burn: { requests: exchangeRequests } };
        break;
      case "withdraw":
      ...
      case "deposit":
        ...
        break;
      case "stake":
        action = { Stake: { requests: stakeRequests } };
        break;
      case "unstake":
        action = { Unstake: { requests: stakeRequests } };
        break;
      case "direct_mint":
        action = { DirectMint: { requests: directRequests } };
        break;
      case "direct_burn":
        action = { DirectBurn: { requests: directRequests } };
        break;
      default:
        throw new Error("No orders to process");
    }

    const payload: SignedPayload_ProtocolRedeemerV1 = {
      action,
      nonce,
    };
    const serialized = Data.serialize(
      this.signing.SignedPayload_ProtocolRedeemerV1,
      payload,
    );
  ...
  async buildExecuteOrdersTx(params: {
    ...
    ) as SignedPayload_ProtocolRedeemerV1;
    ...
    let parsedVaultDatum: TVaultDatum | undefined;
    if (needsVault) {
    ...
    if (actionType === "deposit") {
      ...
      if (totalYield > 0n) {
        ...
        const { unstakedYieldShare } = this.resolveDepositYieldShares(
          totalYield,
          vaultUSDr,
          treasuryCirculating,
    ...
    const extra: ExtraProtocolRedeemerV1 = {
    ...
    const serializedSignedRedeemer =
      this.serializeOrchestratorWithdrawalRedeemer({
        extra,
  ...
  protected buildMintExecute(
    ...
    const totalDelivered = orderInfos.reduce(
      (sum, o) => sum + (o.amount - o.fee),
      0n,
    );
    ...
    for (const orderInfo of orderInfos) {
      ...
      addDestinationOutput(
        ...
        buildOrderDestinationValue(
          orderInfo,
          [[reserveAssetId, reserveAmount]],
          [[stablecoinAssetId, userAmount]],
        ),
      );
    }
    ...
    tx.addMint(
      ...
      new Map([[Core.AssetName(this.assetNameHex), totalDelivered]]),
      Data.Void(),
    );
  ...
  protected buildBurnExecute(
    ...
    const deliveredByAsset = new Map<string, bigint>();
    for (const orderInfo of orderInfos) {
      const ra = findReserveAsset(settings, orderInfo.reserveAsset!);
      const reserveAssetId = Core.AssetId(
        orderInfo.reserveAsset![0] + orderInfo.reserveAsset![1],
      ...
      addDestinationOutput(
        ...
        buildOrderDestinationValue(
          orderInfo,
          [[stablecoinAssetId, -orderInfo.amount]],
          [[reserveAssetId, reserveAmount]],
        ),
      );
      const assetKey = orderInfo.reserveAsset![0] + orderInfo.reserveAsset![1];
      deliveredByAsset.set(
        assetKey,
        (deliveredByAsset.get(assetKey) ?? 0n) + reserveAmount,
      );
    }
   ...
   */
  protected buildWithdrawExecute(
    tx: TxBuilder,
    orderInfos: IOrderInfo[],
    treasuryUtxo: Core.TransactionUnspentOutput,
    parsedTreasuryDatum: V0_1TreasuryDatum,
  ...
  protected buildDepositExecute(
    ...
    const { stakedYieldShare, unstakedYieldShare } =
      this.resolveDepositYieldShares(
        totalYield,
        vaultUSDr,
        treasuryCirculating,
        action,
    ...
    let remainingOrderUSDrToBurn =
      unstakedYieldShare < 0n ? -unstakedYieldShare : 0n;
    ...
    for (const orderInfo of orderInfos) {
      ...
      const consumedAssets: Array<[Core.AssetId, bigint]> = [
        [reserveAssetId, reserveAmount],
      ];
      ...
      if (remainingOrderUSDrToBurn > 0n) {
        const orderUSDr =
          orderInfo.utxo
            ...
            .amount()
            .multiasset()
            ?.get(stablecoinAssetId) ?? 0n;
        const consumedUSDr =
          orderUSDr < remainingOrderUSDrToBurn
            ? orderUSDr
            : remainingOrderUSDrToBurn;
        if (consumedUSDr > 0n) {
      ...
      addDestinationOutput(
        ...
        this.network,
        orderInfo.datum.destination,
        buildOrderDestinationValue(orderInfo, consumedAssets),
      );
    }
    ...
    if (totalYield > 0n) {
      ...
      if (unstakedYieldShare > 0n) {
        ...
        addDestinationOutput(
          tx,
          this.network,
          { address: settings.unstaked_yield_pot, datum: "NoDatum" },
   ...
   */
  protected updateDepositVaultOutput(
    tx: TxBuilder,
    vaultUtxo: Core.TransactionUnspentOutput,
    parsedVaultDatum: TVaultDatum,
    ctx: {
  ...
  protected buildStakeExecute(
    ...
    const settledBacking = this.settledVaultBacking(
      parsedVaultDatum,
      vaultUSDr,
    );
    const circulatingSUSDr = parsedVaultDatum.circulating_susdr;
    ...
    for (const orderInfo of orderInfos) {
      ...
      addDestinationOutput(
        ...
        buildOrderDestinationValue(
          orderInfo,
          [[stablecoinAssetId, usDrToVault]],
          [[sUSDrAssetId, sUSDrAmount]],
        ),
      );
    }
  ...
  protected buildUnstakeExecute(
    ...
    for (const orderInfo of orderInfos) {
      const uSDrEntitled =
        (orderInfo.amount * settledBacking) / circulatingSUSDr;
      ...
      addDestinationOutput(
        ...
        buildOrderDestinationValue(
          orderInfo,
          [[sUSDrAssetId, orderInfo.amount]],
          [[stablecoinAssetId, uSDrAmount]],
        ),
      );
    }
   ...
   */
  protected buildDirectMintExecute(
    tx: TxBuilder,
    orderInfos: IOrderInfo[],
    stablecoinAssetId: Core.AssetId,
    treasuryUtxo: Core.TransactionUnspentOutput,
   ...
   */
  protected buildDirectBurnExecute(
    tx: TxBuilder,
    orderInfos: IOrderInfo[],
    stablecoinAssetId: Core.AssetId,
    treasuryUtxo: Core.TransactionUnspentOutput,
    ...
    for (const orderInfo of orderInfos) {
      addDestinationOutput(
        ...
        this.network,
        orderInfo.datum.destination,
        buildOrderDestinationValue(orderInfo, [
          [stablecoinAssetId, -orderInfo.amount],
        ]),
      );
    }
  ...
  async buildCancelOrdersTx(params: {
    ...
    const cancelRedeemer = Data.serialize(
      this.schemas.OrderRedeemerV1,
      "Cancel",
    );
    const destAddress =
  ...
  async buildInvalidatedOrdersTx(params: {
    ...
    try {
      const parsedProxyDatum = parse(this.schemas.ProxyDatumV1, proxyDatum);
      const registry = this.settingsRegistry(parsedProxyDatum.settings);
      const isProtocolUpgraded =
        parsedProxyDatum.logic !== this.protocolScriptHash;
      const hasActiveOrder = [...orderRefInputs.keys()].some(
        (orderScriptHash) => orderScriptHash === registry.order,
      );

      if (hasActiveOrder && !isProtocolUpgraded) {
        throw new Error(
          "Cannot use Invalidated redeemer: protocol has not been upgraded",
        );
      }
    } catch (err) {
      if (
    ...
    const invalidatedRedeemer = Data.serialize(
      this.schemas.OrderRedeemerV1,
      "Invalidated",
    );
    ...
    const utxoKey = (inp: Core.TransactionInput) =>
      `${inp.transactionId().toString()}#${inp.index().toString()}`;
    const excludedInputIds = new Set<string>([
      utxoKey(proxyUtxo.input()),
      ...[...orderRefInputs.values()].map((refInput) =>
        utxoKey(refInput.input()),
      ),
      ...orderUtxos.map((u) => utxoKey(u.input())),
    ]);
    ...
    const feeCandidates = walletUtxos
      .filter((u) => !excludedInputIds.has(utxoKey(u.input())))
      .sort((a, b) => {
    ...
    const feeUtxo = feeCandidates.find((u) => {
      ...
      const fillerValue = new Core.Value(
        inputValue.coin() - FEE_BUFFER,
        inputValue.multiasset(),
      );
      const fillerOutput = new Core.TransactionOutput(
        walletAddress,
        fillerValue,
      );
      return (
        fillerValue.coin() >=
        calculateMinAda(fillerOutput, this.blaze.params.coinsPerUtxoByte)
      );
    });
    if (!feeUtxo) {
      throw new Error(
        "buildInvalidatedOrdersTx: no wallet UTxO can cover fees while retaining its minimum ADA",
      );
    }
    ...
    type TSortedItem =
      ...
      | { kind: "fee"; utxo: Core.TransactionUnspentOutput };

    const items: TSortedItem[] = orderUtxos.map((utxo) => {
      const datumData = utxo.output().datum()?.asInlineData();
      if (!datumData) {
        throw new Error("Order UTXO has no inline datum");
      }
      ...
      if (!("Signature" in datum.owner)) {
        throw new Error(
          "Invalidated redeemer only supports simple Signature owners",
        );
      }
      return {
        kind: "order" as const,
    ...
    items.push({ kind: "fee" as const, utxo: feeUtxo });
    items.sort((a, b) => {
    ...
    for (const item of items) {
      if (item.kind === "order") {
        ...
        const ownerAddress = addressFromCredentials(
          this.network,
          ownerCredential,
        );
   ...
   */
  async screenOrderForExecution(
    utxo: Core.TransactionUnspentOutput,
    action: IClassifiedOrderAction,
  ): Promise<TOrderSanityResult> {
    const datumVerdict = screenOrderAction(action);
    if (!datumVerdict.ok) {
      return datumVerdict;
    }
    const settings = await this.getValidatedScreeningSettings();
    return screenOrderUtxoFacts(
      action,
      await this.deriveOrderUtxoFacts(utxo, action, settings),
    );
  }
   ...
   */
  protected async deriveOrderUtxoFacts(
    utxo: Core.TransactionUnspentOutput,
    action: IClassifiedOrderAction,
    settings: TV1SettingsConfig,
  ): Promise<IOrderUtxoFacts> {
    const multiasset = utxo.output().amount().multiasset();
    const quantityOf = (assetId: Core.AssetId): bigint =>
      multiasset?.get(assetId) ?? 0n;
    ...
    const usdrAssetId = Core.AssetId(
      this.stablecoinPolicyId + this.assetNameHex,
    );
    ...
    try {
      return this.deriveOrderUtxoFactsUnsafe(
        action,
        settings,
        quantityOf,
        usdrAssetId,
  ...
  private deriveOrderUtxoFactsUnsafe(
    action: IClassifiedOrderAction,
    settings: TV1SettingsConfig,
    quantityOf: (assetId: Core.AssetId) => bigint,
    usdrAssetId: Core.AssetId,
  ): IOrderUtxoFacts {
    switch (action.actionType) {
      ...
      case "unstake":
        return {
          consumedRequired: action.amount,
          consumedLocked: quantityOf(
            Core.AssetId(this.stablecoinPolicyId + this.sUSDrAssetNameHex),
          ),
        };
      case "mint": {
        if (!action.reserveAsset) {
          return { consumedRequired: 0n, consumedLocked: 0n };
        }
        const ra = findReserveAsset(settings, action.reserveAsset);
        return {
          consumedRequired: usdrToReserveCeil(action.amount, ra),
          consumedLocked: quantityOf(
            Core.AssetId(action.reserveAsset[0] + action.reserveAsset[1]),
          ),
  ...
  private async getValidatedScreeningSettings(): Promise<TV1SettingsConfig> {
    const settings = this.settingsConfig(await this.getVersionSettings());
   ...
   */
  protected buildTreasuryRequest(
    datum: OrderDatumV1,
    parsed: IClassifiedOrderAction,
    origin: IRequestOrigin,
  ): TreasuryRequestV1 {
    return {
      destination: datum.destination,
      amount: parsed.amount,
   ...
   */
  protected updateTreasuryOutput(
    tx: TxBuilder,
    treasuryUtxo: Core.TransactionUnspentOutput,
    parsedTreasuryDatum: V0_1TreasuryDatum,
   ...
   */
  protected updateTreasuryOutputNoReserve(
    tx: TxBuilder,
    treasuryUtxo: Core.TransactionUnspentOutput,
    parsedTreasuryDatum: V0_1TreasuryDatum,
    circulatingSupplyDelta: bigint,
  ...
  protected updateVaultOutput(
    ...
    this.updateVaultOutputWithDatum(
      tx,
      vaultUtxo,
      this.buildUpdatedVaultDatum(parsedVaultDatum, sUSDrDelta),
      uSDrDelta,
    );
  }
   ...
   */
  protected updateVaultOutputWithDatum(
    tx: TxBuilder,
    vaultUtxo: Core.TransactionUnspentOutput,
    newVaultDatum: TVaultDatum,
    uSDrDelta: bigint = 0n,
  ): void {
  ...
  protected async getVersionSettings(): Promise<TSettings> {
    const { parsedProxyDatum } = await this.getParsedProxyDatum();
