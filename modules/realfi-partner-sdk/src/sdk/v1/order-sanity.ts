...
export type TOrderRejectCode =
  | "amount_not_positive"
  | "min_received_not_positive"
  | "forfeit_negative"
  | "deposit_principal_negative"
  | "deposit_batch_empty"
  | "withdraw_yield_not_zero"
  | "input_underfunded"
  | "min_received_above_max_delivered"
  | "order_facts_unresolvable";
...
export type TOrderSanityResult =
  | { ok: true }
  | { ok: false; code: TOrderRejectCode; reason: string };

const OK: TOrderSanityResult = { ok: true };

function reject(code: TOrderRejectCode, reason: string): TOrderSanityResult {
 ...
 */
export function screenOrderAction(
  action: IOrderActionSanityInput,
): TOrderSanityResult {
  ...
  const floor = action.minReceived;
  ...
  switch (actionType) {
    ...
    case "unstake":
      if (action.amount <= 0n) {
        return reject(
          "amount_not_positive",
          `Invalid ${actionType} amount (<= 0)`,
        );
      }
      if (floor !== undefined && floor <= 0n) {
        return reject(
          "min_received_not_positive",
          `Invalid ${actionType} min_received (<= 0)`,
        );
      }
      ...
      if (actionType === "unstake" && (action.forfeit ?? 0n) < 0n) {
    ...
    case "withdraw":
      ...
      if ((action.yield ?? 0n) !== 0n) {
        return reject(
          "withdraw_yield_not_zero",
          "Invalid withdraw request (yield must be 0)",
        );
      }
      return OK;
 ...
 */
export function screenDepositBatch(
  actions: readonly IOrderActionSanityInput[],
): TOrderSanityResult {
  const { principal, totalYield } = actions.reduce(
    (totals, action) => ({
      principal: totals.principal + action.amount,
      totalYield: totals.totalYield + (action.yield ?? 0n),
    }),
    { principal: 0n, totalYield: 0n },
  );
  if (principal === 0n && totalYield === 0n) {
    return reject(
      "deposit_batch_empty",
      "Invalid deposit batch (principal and yield both total 0)",
    );
  }
  return OK;
}
 ...
 */
export function screenOrderUtxoFacts(
  action: IOrderActionSanityInput,
  facts: IOrderUtxoFacts,
): TOrderSanityResult {
  if (facts.unresolvable !== undefined) {
    return reject(
      "order_facts_unresolvable",
      `Order datum cannot be resolved against protocol settings: ${facts.unresolvable}`,
    );
  }

  if (
    facts.consumedRequired > 0n &&
    facts.consumedLocked < facts.consumedRequired
  ) {
    return reject(
      "input_underfunded",
      `Order UTxO locks ${facts.consumedLocked} of the consumed asset but its ` +
        `${action.actionType} datum requires ${facts.consumedRequired}`,
    );
  }
  ...
  if (
    floor !== undefined &&
    facts.maxDelivered !== undefined &&
    floor > facts.maxDelivered
  ) {
    return reject(
      "min_received_above_max_delivered",
      `Invalid ${action.actionType} min_received (${floor}) — above the maximum ` +
        `deliverable ${facts.maxDelivered}, so no execution can satisfy it`,
    );
  }
