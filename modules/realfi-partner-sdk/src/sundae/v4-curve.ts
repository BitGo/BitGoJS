import { EContractVersion, EPoolCurve, type IPoolData } from "@sundaeswap/core";
...
interface IPoolCurveRow {
  id: string;
  modules?: { identifier: string }[] | null;
...
const CURVES: readonly EPoolCurve[] = [
  EPoolCurve.ConstantProduct,
  EPoolCurve.ConstantSum,
  EPoolCurve.ConcentratedLiquidity,
];
 ...
 */
function curveOf(row: IPoolCurveRow): EPoolCurve | undefined {
  return CURVES.find((curve) =>
    (row.modules ?? []).some((module) => module.identifier === curve),
  );
}
 ...
 * cannot describe.
 */
function pricesOf(row: IPoolCurveRow): [bigint, bigint] | undefined {
...
function needsCurveData(pool: IPoolData): boolean {
  return (
    pool.version === EContractVersion.V4 &&
    (pool.curve === undefined || pool.prices === undefined)
  );
}
 ...
 */
export async function withV4CurveData(
  pools: readonly IPoolData[],
  assetId: string,
  endpoint: string,
): Promise<IPoolData[]> {
  ...
  try {
    ...
    const payload = (await response.json()) as {
      data?: { pools?: { byAsset?: IPoolCurveRow[] } };
    };
