...
import {
  DatumBuilderV3,
  type IDatumBuilderDepositV3Args,
  type IDatumBuilderSwapV3Args,
  type IDatumBuilderWithdrawV3Args,
...
export interface SundaeV3SwapTxFlowStep
  extends
    SundaeV3BaseTxFlowStep,
    Omit<IDatumBuilderSwapV3Args, "destinationAddress"> {
  orderKind: "swap";
}

export interface SundaeV3DepositTxFlowStep
  extends
    SundaeV3BaseTxFlowStep,
    Omit<IDatumBuilderDepositV3Args, "destinationAddress"> {
  orderKind: "deposit";
}

export interface SundaeV3WithdrawTxFlowStep
  extends
    SundaeV3BaseTxFlowStep,
    Omit<IDatumBuilderWithdrawV3Args, "destinationAddress"> {
  orderKind: "withdraw";
}

export type SundaeV3TxFlowStep =
  | SundaeV3SwapTxFlowStep
  | SundaeV3DepositTxFlowStep
  | SundaeV3WithdrawTxFlowStep;
 ...
 * @deprecated V1_0 only. Prefer the SDK-backed stake continuation helper.
 */
export function computeStakeMinReceivedFromVaultRatio(
  amount: bigint,
  vaultRatio: bigint,
  slippageToleranceBps: bigint = DEFAULT_STAKE_SLIPPAGE_TOLERANCE_BPS,
): bigint {
  if (amount <= 0n) {
  ...
  const minReceived =
    (expected * (BPS_DENOMINATOR - slippageToleranceBps)) / BPS_DENOMINATOR;
  ...
  if (minReceived <= 0n) {
    throw new Error(
      `Quoted min_received must be positive (got ${minReceived}); the stake amount ` +
        "is too small for this vault ratio and tolerance to quote a non-zero floor",
    );
  }
  return minReceived;
}
...
}: RealFiStakeActionInput): OrderActionV1 {
  return {
    OStake: {
      amount,
      min_received: computeStakeMinReceivedFromVaultRatio(
        amount,
        vaultRatio,
        slippageToleranceBps,
      ),
 ...
 */
export function buildTxFlow(
  steps: readonly TxFlowStep[],
  finalStep: TxBuilderFinalStep,
): TxBuilderFlowResult {
  return steps.reduceRight<TxBuilderFlowResult>(
    (nextStep, step) => buildTxFlowStep(step, nextStep),
    finalStep,
  );
}
 ...
 */
export function buildTxFlowStep(
  step: TxFlowStep,
  nextStep: TxBuilderFlowResult,
): TxBuilderFlowResult {
  switch (step.kind) {
    case "realfi":
 ...
 * @deprecated V1_0 only. Prefer the SDK-backed stake continuation helper.
 */
export function buildRealFiTxFlowStep(
  step: RealFiTxFlowStep,
  nextStep: TxBuilderStepResult,
): TxBuilderFlowResult {
 ...
 */
export function txBuilderFlowResultToCoreOutput(
  result: TxBuilderFlowResult,
