...
exports.slotAlignedTimeMs = slotAlignedTimeMs;
exports.slotFloor = slotFloor;
 ...
 */
function slotFloor(clock, timeMs) {
  var candidate = Math.floor(Number(clock.unixToSlot(timeMs)));
  var overshoots = Number(clock.slotToUnix(_sdk.Core.Slot(candidate))) > Number(timeMs);
  return _sdk.Core.Slot(overshoots ? candidate - 1 : candidate);
}
 ...
 */
function slotAlignedTimeMs(clock, timeMs) {
