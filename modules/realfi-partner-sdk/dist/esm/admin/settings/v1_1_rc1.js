...
import { calculateMinAda, Core } from "@blaze-cardano/sdk";
import { credentialFromScript, deployScript, lockOrPayAssets, readSingletonDatum, rewardAccountFromScript } from "../../sdk/shared/index.js";
import { buildRawProxyDatum, buildSettingsAuthPayload, createSettingsValidatorScript, freezeSettingsData, hashSettingsAuthPayload, isFrozenSettingsData, parseLiveSettings, parseProxyDatumRaw, serializeSettings, serializeSettingsSignedRedeemer, unwrapFrozenSettingsData } from "./utils.js";
...
export class RealfiProtocolSettingsAdmin {
  constructor(blaze, params) {
    ...
    this.settingsScript = createSettingsValidatorScript(this.proxyPolicyId, this.governanceConfig, this.enableTrace);
    this.settingsScriptHash = this.settingsScript.hash();
    this.settingsValidatorAddress = addressFromValidator(this.blaze.provider.network, this.settingsScript);
    this.settingsRewardAccount = rewardAccountFromScript(this.settingsScript, this.blaze.provider.network);
  }
  static create(blaze, params) {
    return new RealfiProtocolSettingsAdmin(blaze, params);
  }
  static fromProtocolSdk(sdk, params) {
  ...
  async deploySettingsValidator() {
    return deployScript(this.blaze, this.settingsScript);
  }
  registerSettingsStake() {
    return this.blaze.newTransaction().addRegisterStake(credentialFromScript(this.settingsScript));
  }
  async getSettingsState() {
    ...
    const {
      utxo: proxyUtxo,
      datum: proxyDatum
    } = await readSingletonDatum(this.blaze.provider, proxyAssetId);
    const {
      logicHash,
      settingsData
    } = parseProxyDatumRaw(proxyDatum);
  ...
  async buildDepositProxyTx() {
    ...
    if (state.proxyAddress.getProps().paymentPart?.type === Core.CredentialType.ScriptHash) {
    ...
    return lockOrPayAssets(this.blaze.newTransaction().addInput(state.proxyUtxo), this.settingsValidatorAddress,
   ...
   */
  async getSettingsAuthPayloadHash(change) {
    const state = await this.requireGovernedState();
    const resolved = this.resolveChange(state, change);
    return hashSettingsAuthPayload(buildSettingsAuthPayload(resolved.redeemer, this.scriptHashOfAddress(resolved.receiverAddress), resolved.nextLogicHash, resolved.nextSettingsData, state.proxyUtxo.input()));
  }
  async buildChangePermissionsTx(params) {
    return this.buildChange({
      type: "ChangePermissions",
      nextPermissions: params.nextPermissions
    }, params.signatures);
  }
  async buildChangeConfigTx(params) {
    return this.buildChange({
      type: "ChangeConfig",
      nextConfig: params.nextConfig
    }, params.signatures);
  }
  async buildChangeLogicTx(params) {
    return this.buildChange({
      type: "ChangeLogic",
      ...
      case "Restore":
        {
          ...
          return {
            redeemer: "Restore",
            nextLogicHash: state.logicHash,
            nextSettingsData: this.canonicalizeSettings(unwrapFrozenSettingsData(state.settingsData)),
            receiverAddress: here
          };
        }
      case "Migrate":
   ...
   */
  canonicalizeSettings(settingsData) {
    const live = parseLiveSettings(settingsData);
    return live ? serializeSettings(live) : settingsData;
  }
  async buildChange(change, signatures, destinationSignatures) {
    const state = await this.requireGovernedState();
    const resolved = this.resolveChange(state, change);
    const tx = this.blaze.newTransaction();
    tx.addInput(state.proxyUtxo, Data.Void());
    tx.addWithdrawal(this.settingsRewardAccount, 0n, serializeSettingsSignedRedeemer(resolved.redeemer, signatures));
    ...
    if (resolved.coValidateWith) {
      ...
      tx.addWithdrawal(resolved.coValidateWith.rewardAccount, 0n, serializeSettingsSignedRedeemer("Migrate", destinationSignatures));
      await resolved.coValidateWith.applyWitness(tx);
    }
    const nextDatum = buildRawProxyDatum(resolved.nextLogicHash, resolved.nextSettingsData);
    return lockOrPayAssets(tx, resolved.receiverAddress, this.valueWithDatumMinAda(resolved.receiverAddress, state.proxyUtxo.output().amount(), nextDatum), nextDatum);
  }
   ...
   */
  valueWithDatumMinAda(address, value, datum) {
    let result = value;
  ...
  async applySettingsWitness(tx) {
    if (this.settingsRefInput) {
      tx.addReferenceInput(this.settingsRefInput);
      return;
    }
    const refInput = await this.blaze.provider.resolveScriptRef(this.settingsScriptHash);
    if (refInput) {
