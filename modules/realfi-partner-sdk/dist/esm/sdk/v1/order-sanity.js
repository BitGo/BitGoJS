 ...
 */
export function screenOrderAction(action) {
  const {
    actionType
  } = action;
  ...
  switch (actionType) {
    ...
    case "unstake":
      if (action.amount <= 0n) {
        return reject("amount_not_positive", `Invalid ${actionType} amount (<= 0)`);
      }
      if (floor !== undefined && floor <= 0n) {
        return reject("min_received_not_positive", `Invalid ${actionType} min_received (<= 0)`);
      }
 ...
 */
export function screenDepositBatch(actions) {
  const {
    principal,
    totalYield
  } = actions.reduce((totals, action) => ({
 ...
 */
export function screenOrderUtxoFacts(action, facts) {
  if (facts.unresolvable !== undefined) {
    return reject("order_facts_unresolvable", `Order datum cannot be resolved against protocol settings: ${facts.unresolvable}`);
  }
  if (facts.consumedRequired > 0n && facts.consumedLocked < facts.consumedRequired) {
    return reject("input_underfunded", `Order UTxO locks ${facts.consumedLocked} of the consumed asset but its ` + `${action.actionType} datum requires ${facts.consumedRequired}`);
  }
  const floor = action.minReceived;
  if (floor !== undefined && facts.maxDelivered !== undefined && floor > facts.maxDelivered) {
    return reject("min_received_above_max_delivered", `Invalid ${action.actionType} min_received (${floor}) — above the maximum ` + `deliverable ${facts.maxDelivered}, so no execution can satisfy it`);
  }
  return OK;
}
