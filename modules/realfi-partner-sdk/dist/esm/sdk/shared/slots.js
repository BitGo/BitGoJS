 ...
 */
export function slotFloor(clock, timeMs) {
  const candidate = Math.floor(Number(clock.unixToSlot(timeMs)));
  const overshoots = Number(clock.slotToUnix(Core.Slot(candidate))) > Number(timeMs);
 ...
 */
export function slotAlignedTimeMs(clock, timeMs) {
