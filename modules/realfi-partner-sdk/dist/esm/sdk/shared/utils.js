 ...
 */
export function rewardAccountFromScript(script, network) {
 ...
 */
export function credentialFromScript(script) {
 ...
 */
export function credentialFromScriptHash(scriptHash) {
 ...
 */
export function lockOrPayAssets(tx, address, value, datum) {
 ...
 * chaining.
 */
export function addDirectOutput(tx, address, value, datum) {
 ...
 */
const BLOCKFROST_ADDRESS_NOT_FOUND = /^getUnspentOutputs: Blockfrost threw "The requested component has not been found\." for query: \/?addresses\//;
 ...
 */
async function hydrateUnspentOutputs(blaze, address) {
  const unspent = await blaze.provider.getUnspentOutputs(address);
  if (unspent.length === 0) return [];
  return blaze.provider.resolveUnspentOutputs(unspent.map(utxo => utxo.input()));
}
 ...
 */
async function resolveScriptRefAtAddress(blaze, hash, address, hydrate = () => hydrateUnspentOutputs(blaze, address)) {
  try {
 ...
 */
export async function deployScript(blaze, script, address) {
  const refInput = address ? await resolveScriptRefAtAddress(blaze, script.hash(), address) : await blaze.provider.resolveScriptRef(script.hash());
  if (refInput) {
 ...
 */
export async function getUnspentOutputByNftWithRetry(provider, assetId) {
  let lastError;
...
function withInlineDatum(utxo, datum) {
  ...
  const promoted = new Core.TransactionOutput(output.address(), output.amount());
 ...
 */
export async function readSingletonDatum(provider, nftAssetId) {
  ...
  if (preimageHash !== datumHash) {
    throw new Error(`Datum resolved for UTXO with NFT ${nftAssetId} does not match its datum hash: ` + `resolved preimage hashes to ${preimageHash}, UTXO reports ${datumHash}`);
  }
  const input = read.input();
  console.warn(`[realfi-sdk] singleton ${nftAssetId} read at ${input.transactionId()}#${input.index()} ` + `reported datum hash ${datumHash} instead of its inline datum; reattaching the resolved preimage inline`);
  return {
 ...
 */
export async function getDatumFromNFT(blaze, nftAssetId, datumSchema) {
  const {
    utxo,
    datum
  } = await readSingletonDatum(blaze.provider, nftAssetId);
  if (datumSchema) {
    const parsedDatum = parse(datumSchema, datum);
    return {
      utxo,
      datum,
      parsedDatum
 ...
 */
export async function getReferenceInputs(blaze, scriptHashes, cachedInputs, address) {
  ...
  for (const [name, hash] of Object.entries(scriptHashes)) {
    ...
    const refInput = address && (await resolveScriptRefAtAddress(blaze, hash, address, hydrate)) || (await blaze.provider.resolveScriptRef(hash));
    if (!refInput) {
      throw new Error(`No reference input found for ${name} script (${hash}). Make sure it is deployed.`);
    }
 ...
 */
export const getSignatureKeyHashesFromMultisigScript = multisig => {
  const result = [];
  if ("Signature" in multisig) {
    result.push(multisig.Signature.key_hash);
  } else if ("AtLeast" in multisig) {
    multisig.AtLeast.scripts.map(s => getSignatureKeyHashesFromMultisigScript(s)).flat().forEach(s => {
      result.push(s);
    });
  } else if ("AllOf" in multisig) {
    multisig.AllOf.scripts.map(s => getSignatureKeyHashesFromMultisigScript(s)).flat().forEach(s => {
      result.push(s);
    });
  } else if ("AnyOf" in multisig) {
    multisig.AnyOf.scripts.map(s => getSignatureKeyHashesFromMultisigScript(s)).flat().forEach(s => {
      result.push(s);
    });
  }
 ...
 */
export const getScriptKeyHashesFromMultisigScript = multisig => {
  const result = [];
  if ("Script" in multisig) {
    result.push(multisig.Script.script_hash);
  } else if ("AtLeast" in multisig) {
    multisig.AtLeast.scripts.map(s => getScriptKeyHashesFromMultisigScript(s)).flat().forEach(s => {
      result.push(s);
    });
  } else if ("AllOf" in multisig) {
    multisig.AllOf.scripts.map(s => getScriptKeyHashesFromMultisigScript(s)).flat().forEach(s => {
      result.push(s);
    });
  } else if ("AnyOf" in multisig) {
    multisig.AnyOf.scripts.map(s => getScriptKeyHashesFromMultisigScript(s)).flat().forEach(s => {
      result.push(s);
    });
  }
  return [...new Set(result).values()];
 ...
 */
export const getAllKeyHashesFromMultisigScript = multisig => {
 ...
 */
export const sortOrderInputs = orderInputs => {
 ...
 */
export const buildNonceFromUtxo = utxo => ({
  UTxO: [{
 ...
 */
export const buildOriginFromUtxo = utxo => {
 ...
 */
export const signPayloadHashWithWallet = async (blaze, payloadHash) => {
 ...
 */
export const createSignedRedeemer = (extra, payload, signatures) => {
 ...
 */
const extractPublicKeyFromCoseKey = coseKeyHex => {
 ...
 */
const readCborHeader = (bytes, offset) => {
  ...
  if (additional === CBOR_ADDITIONAL_INDEFINITE) {
    ...
    if (major < CBOR_MAJOR_BYTES || major > CBOR_MAJOR_MAP) {
      throw new Error(`Unsupported indefinite-length CBOR major type: ${major}`);
    }
    return {
      major,
      argument: 0,
 ...
 */
const readIndefiniteStringChunks = (bytes, major, offset) => {
  const chunks = [];
  let next = offset;
 ...
 */
const skipCborItem = (bytes, offset) => {
  const {
    major,
    argument,
    indefinite,
    contentOffset
  } = readCborHeader(bytes, offset);
  if (indefinite) {
  ...
  switch (major) {
    ...
    case CBOR_MAJOR_TAG:
      return skipCborItem(bytes, contentOffset);
    // tagged content
    default:
 ...
 * after it.
 */
const readCborByteString = (bytes, offset) => {
  const {
    major,
    argument,
    indefinite,
  ...
  if (major !== CBOR_MAJOR_BYTES) {
    throw new Error(`Expected a CBOR byte string in COSE_Sign1, got major type ${major}`);
  }
  if (indefinite) {
    const {
      content,
      nextOffset
    } = readIndefiniteStringChunks(bytes, major, contentOffset);
    return {
 ...
 */
export const readCoseSign1Parts = coseSign1Hex => {
  ...
  if (first.major === CBOR_MAJOR_TAG) {
    if (first.argument !== CBOR_TAG_COSE_SIGN1) {
      throw new Error(`COSE_Sign1 has unexpected CBOR tag ${first.argument} (expected 18)`);
    }
  ...
  if (unprotectedHeader.major !== CBOR_MAJOR_MAP) {
    throw new Error(`COSE_Sign1 unprotected header is not a CBOR map (got major type ${unprotectedHeader.major})`);
  }
  ...
  const isNullPayload = payloadHeader.major === CBOR_MAJOR_SIMPLE && !payloadHeader.indefinite && payloadHeader.argument === CBOR_SIMPLE_NULL;
  if (payloadHeader.major !== CBOR_MAJOR_BYTES && !isNullPayload) {
    throw new Error(`COSE_Sign1 payload is not a byte string or null (got major type ${payloadHeader.major})`);
  }
 ...
 */
export const getSignedPayloadForWithdraw = (amount, treasuryUtxo) => {
  if (amount <= 0n) {
  ...
  return Data.serialize(V0_3Types.SignedPayload_ProtocolRedeemer, signedPayload).toCbor().toString();
};
 ...
 * @returns SignedPayload_ProtocolRedeemer CBOR hex string
 */
export const getSignedPayloadForDeposit = (amount, treasuryUtxo) => {
  if (amount <= 0n) {
 ...
 */
export function addressToDestination(address, datum = "NoDatum") {
 ...
 */
export function destinationToAddress(network, destination) {
  const paymentCred = destination.address.payment_credential;
  const stakingCred = destination.address.stake_credential;
  let paymentCredCore;
  let stakingCredCore;
  if (stakingCred && typeof stakingCred === "object" && "Inline" in stakingCred) {
 ...
 */
export function addDestinationOutput(tx, network, destination, value) {
  const output = new Core.TransactionOutput(destinationToAddress(network, destination), value);
  const {
    datum
  } = destination;
  if (datum !== "NoDatum") {
    if ("DatumHash" in datum) {
      ...
      output.setDatum(Core.Datum.newInlineData(datum.InlineDatum[0]));
    }
  }
  tx.addOutput(output);
}
...
const NORMALIZED_PROTOCOL_VERSION_MAP = {
  v0: "V0",
  v0_1: "V0_1",
  v0_2: "V0_2",
  v0_3: "V0_3",
 ...
 */
export const normalizeProtocolVersion = version => {
  if (!version) {
    return undefined;
  }
 ...
 */
export const getInlineDatumFromUtxo = (utxo, label = "Order UTXO") => {
 ...
 */
export const getScriptHashFromOrderUtxo = utxo => {
 ...
 */
export async function resolveReferenceInputsByScriptHash(blaze, scriptHashes, cachedInputs, address) {
  const referenceInputs = new Map();
  for (const hash of new Set(scriptHashes)) {
    const cachedInput = cachedInputs instanceof Map ? cachedInputs.get(hash) : cachedInputs?.[hash];
    if (cachedInput) {
      referenceInputs.set(hash, cachedInput);
      continue;
    }
    const refInput = address && (await resolveScriptRefAtAddress(blaze, hash, address)) || (await blaze.provider.resolveScriptRef(hash));
    if (!refInput) {
      throw new Error(`Missing order reference script input for hash ${hash}. Make sure the order script is deployed.`);
    }
 ...
 */
export async function resolveOrderReferenceInputs(blaze, orderUtxos, cachedInputs, address) {
  const scriptHashes = [];
  for (const utxo of orderUtxos) {
    scriptHashes.push(getScriptHashFromOrderUtxo(utxo));
  }
  return resolveReferenceInputsByScriptHash(blaze, scriptHashes, cachedInputs, address);
}
 ...
 */
export const parseOrderOwnerGenericFromDatum = datum => {
  ...
  if (constr.getAlternative() !== 0n) {
    throw new Error(`Order datum has unexpected constructor index ${constr.getAlternative().toString()} (expected 0)`);
  }
  const fields = constr.getData();
  if (fields.getLength() < 4) {
    throw new Error(`Order datum has ${fields.getLength().toString()} fields (expected at least 4)`);
  }
  return parse(V0_3Types.MultisigScript, fields.get(0));
};

/**
 * Strict owner-only parser for cancel flows.
 */
export const parseOrderOwnerGeneric = utxo => {
