...
import { addressFromValidator, blake2b_256, HexBlob, PlutusData, toHex } from "@blaze-cardano/core";
import * as Data from "@blaze-cardano/data";
import { parse } from "@blaze-cardano/data";
import { Core, makeValue } from "@blaze-cardano/sdk";
import { BaseTypes, V0_1Types, V0_4Types, V1_0Rc1Types } from "../../generated-types/index.js";
import { addDirectOutput, buildNonceFromUtxo, buildMultisigTimelockNativeScript, buildTimelockDestination, buildUnstakeMetadatum, UNSTAKE_METADATA_LABEL, destinationToAddress, findReserveAsset, usdrToReserve, usdrToReserveCeil, computeReserveDeltas, sortOrderInputs } from "../shared/index.js";
 ...
 * overridden here.
 */
export class RealfiSDKV1_0Rc1 extends RealfiSDKV1Family {
  constructor(...args) {
    super(...args);
    _defineProperty(this, "version", "V1_0_Rc1");
  }
  /**
  ...
  static create(blaze, params) {
    ...
    const protocolMintScript = new V1_0Rc1Types.V1_0Rc1ProtocolMintProtocolMintWithdraw(oneShotPolicyId, enableTrace).Script;
    const protocolStakeScript = new V1_0Rc1Types.V1_0Rc1ProtocolStakeProtocolStakeWithdraw(oneShotPolicyId, enableTrace).Script;
    const protocolManagementScript = new V1_0Rc1Types.V1_0Rc1ProtocolManagementProtocolManagementWithdraw(oneShotPolicyId, enableTrace).Script;

    // 3. Create orchestrator with sub-validator hashes
    const protocolOrchestratorScript = new V1_0Rc1Types.V1_0Rc1ProtocolOrchestratorProtocolOrchestratorWithdraw(oneShotPolicyId, protocolMintScript.hash(), protocolStakeScript.hash(), protocolManagementScript.hash(), enableTrace).Script;
    ...
    }, oneShotPolicyId, enableTrace).Script : new V1_0Rc1Types.V1_0Rc1TreasuryTreasurySpend({
      transaction_id: params.treasuryBootstrap.txHash,
      output_index: params.treasuryBootstrap.outputIndex
    }, oneShotPolicyId, enableTrace).Script;

    // 4. Create order script with orchestrator hash (order needs to know the protocol)
    const orderScript = new V1_0Rc1Types.V1_0Rc1OrderOrderSpend(oneShotPolicyId, protocolOrchestratorScript.hash(), enableTrace).Script;
    ...
    }, oneShotPolicyId, enableTrace).Script : new V1_0Rc1Types.V1_0Rc1StakingVaultStakingVaultSpend({
      transaction_id: params.stakingVaultBootstrap.txHash,
      output_index: params.stakingVaultBootstrap.outputIndex
    }, oneShotPolicyId, enableTrace).Script;
  ...
  async buildStakeContinuation(_params) {
  ...
  async _buildOrderTx(params) {
    ...
    const serializedDatum = Data.serialize(V1_0Rc1Types.OrderDatumV1, orderDatum);
  ...
  async getSignedPayloadFromOrderInputs(orderInputs) {
    ...
    for (const utxo of resolvedUtxos) {
      ...
      const datum = parse(V1_0Rc1Types.OrderDatumV1, datumData);
      const origin = {
    ...
    const serialized = Data.serialize(V1_0Rc1Types.SignedPayload_v1_0_ProtocolRedeemerV1, payload);
  ...
  async buildExecuteOrdersTx(params) {
    ...
    const signedPayload = parse(V1_0Rc1Types.SignedPayload_v1_0_ProtocolRedeemerV1, PlutusData.fromCbor(HexBlob(signedPayloadCbor)));
    ...
    const serializedSignedRedeemer = Data.serialize(V1_0Rc1Types.SignedRedeemer_v1_0_ExtraProtocolRedeemerV1, {
      extra,
      payload: signedPayload,
      signatures
    });
    const executeRedeemer = Data.serialize(V1_0Rc1Types.OrderRedeemerV1, "Execute");
  ...
  buildMintExecute(tx, orderInfos, stablecoinAssetId, treasuryUtxo, parsedTreasuryDatum, settings) {
    ...
    for (const orderInfo of orderInfos) {
      ...
      addDirectOutput(tx, destAddress, makeValue(MIN_LOVELACE, [stablecoinAssetId, orderInfo.amount]));
    }
  ...
  buildBurnExecute(tx, orderInfos, stablecoinAssetId, treasuryUtxo, parsedTreasuryDatum, settings) {
    ...
    for (const orderInfo of orderInfos) {
      ...
      addDirectOutput(tx, destAddress, makeValue(MIN_LOVELACE, [reserveAssetId, reserveAmount]));
    }
  ...
  buildDepositExecute(tx, orderInfos, stablecoinAssetId, treasuryUtxo, parsedTreasuryDatum, vaultUtxo, parsedVaultDatum, settings) {
    ...
    if (totalYield > 0n) {
      ...
      if (unstakedYieldShare > 0n) {
        ...
        addDirectOutput(tx, yieldPotAddress, makeValue(MIN_LOVELACE, [stablecoinAssetId, unstakedYieldShare]));
      }
  ...
  buildStakeExecute(tx, orderInfos, stablecoinAssetId, sUSDrAssetId, vaultUtxo, parsedVaultDatum) {
    ...
    for (const orderInfo of orderInfos) {
      ...
      const destAddress = destinationToAddress(this.network, orderInfo.datum.destination);
      addDirectOutput(tx, destAddress, makeValue(MIN_LOVELACE, [sUSDrAssetId, sUSDrAmount]));
    }
  ...
  buildUnstakeExecute(tx, orderInfos, stablecoinAssetId, sUSDrAssetId, vaultUtxo, parsedVaultDatum, settings) {
    ...
    if (totalForfeit > 0n) {
      ...
      addDirectOutput(tx, yieldPotAddress, makeValue(MIN_LOVELACE, [stablecoinAssetId, totalForfeit]));
    }
   ...
   */
  buildDirectBurnExecute(tx, orderInfos, _stablecoinAssetId, treasuryUtxo, parsedTreasuryDatum) {
