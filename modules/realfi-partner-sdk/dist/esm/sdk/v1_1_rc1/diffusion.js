 ...
 */
export function pendingRemaining(vault, atTimeMs) {
  const {
    pending_yield,
    diffusion_start,
    diffusion_end
 ...
 * validity bound).
 */
export function settledBacking(usdrBalance, vault, atTimeMs) {
 ...
 */
export function calculateSusdrExchangeRate(inputs, atTimeMs, precision = SUSDR_EXCHANGE_RATE_PRECISION) {
 ...
 * lower bound).
 */
export function nextDepositDiffusion(vaultIn, stakedYieldShare, totalYield, windowEndMs, nowMs) {
  if (totalYield >= 0n) {
