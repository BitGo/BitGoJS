import { QueryProviderSundaeSwap, SundaeUtils } from "@sundaeswap/core";
...
const SUNDAE_POOL_DISCOVERY_SCOPES = ["curated", "buildable", "all"];
...
function isCuratedPool(pool, {
  baseAssetId,
  counterpartAssetIds
}) {
  const aIsBase = isPoolAsset(pool.assetA.assetId, baseAssetId);
  const bIsBase = isPoolAsset(pool.assetB.assetId, baseAssetId);
  const counterpartId = aIsBase ? pool.assetB.assetId : bIsBase ? pool.assetA.assetId : undefined;
  return counterpartId !== undefined && counterpartAssetIds.some(assetId => isPoolAsset(counterpartId, assetId));
}
 ...
 */
export function createPoolDiscovery(network, options = {}, loadCuratedPoolAssets) {
  const queryProvider = new QueryProviderSundaeSwap(network);
  if (options.endpoint !== undefined) {
    queryProvider.baseUrl = options.endpoint;
  }
  return {
    async findPoolsByAsset(assetId, {
      scope = "curated"
    } = {}) {
      ...
      const curatedAssets = scope === "curated" ? await loadCuratedPoolAssets(network) : undefined;
      ...
      if (rawPools.length >= SUNDAE_POOL_QUERY_RESULT_LIMIT) {
        throw new Error(`Sundae pool discovery returned ${rawPools.length} pools for ${assetId}; results may be truncated`);
      }
      ...
      const all = await withV4CurveData(deterministicUniquePools(rawPools), assetId, queryProvider.baseUrl);
      if (scope === "all") {
        return all;
      }
      ...
      // derived from this.
      const buildable = all.filter(pool => isSwappableSundaeSwapVersion(pool.version) && poolContainsAsset(pool, assetId));
      if (scope === "buildable") {
        return buildable;
      }
...
export function createRuntimeCuratedPoolAssetsLoader(partnerConfigUrl) {
  return async network => {
    const api = RealfiApi.create({
      ...API_REGISTRY[network],
      ...(partnerConfigUrl === undefined ? {} : {
        partnerConfigUrl
      })
    });
