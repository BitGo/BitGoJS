...
import { BlazeHelper, EContractVersion, EDatumType, EPoolCurve, ESwapType, SundaeSDK, SundaeUtils } from "@sundaeswap/core";
...
import { addressToRealFiDestination, plutusDataFromCbor, realfiOwnerFromSundaeV3OwnerAddress, sundaeV3DestinationAddressFromStepResult } from "../tx-builder/destination.js";
import { createPoolDiscovery, createRuntimeCuratedPoolAssetsLoader } from "./discovery.js";
import { assertSupportedSundaeSwapPool, assertSwappableSundaeSwapPool } from "./support.js";
export { assertSupportedSundaeSwapPool, assertSwappableSundaeSwapPool, isSupportedSundaeSwapVersion, isSwappableSundaeSwapVersion, SUPPORTED_SUNDAE_SWAP_VERSIONS, SWAPPABLE_SUNDAE_SWAP_VERSIONS } from "./support.js";
...
export function forNetwork(network, options = {}) {
  return createPoolDiscovery(network, options, createRuntimeCuratedPoolAssetsLoader(options.partnerConfigUrl));
}
function canonicalizePoolAsset(pool, asset) {
  const metadata = [pool.assetA, pool.assetB].find(poolAsset => SundaeUtils.isAssetIdsEqual(poolAsset.assetId, asset.metadata.assetId));
  if (!metadata) {
    throw new Error(`Asset is not part of the Sundae swap pool: ${asset.metadata.assetId}`);
  }
...
function canonicalizeSwapType(pool, suppliedAsset, swapType) {
  ...
  if (minReceivable.metadata.assetId !== expectedAsset.assetId) {
    throw new Error(`Sundae LIMIT minimum must use the pool asset opposite the supplied asset: ${expectedAsset.assetId}`);
  }
  return {
    ...swapType,
    minReceivable
...
function decimalFraction(value) {
  ...
  return scale > 0 ? {
    ...
    numerator: digits * 10n ** BigInt(-scale),
    denominator: 1n
  };
}
...
function constantSumRefusal(pool) {
  ...
  if (pool.curve !== EPoolCurve.ConstantSum) {
    return `Sundae V4 pool ${pool.ident} is not known to be ${EPoolCurve.ConstantSum} ` + `(curve: ${pool.curve ?? "unknown"}). Resolve pools through pool ` + `discovery, which supplies the curve.`;
  }
  ...
  if (pool.prices === undefined) {
    return `Sundae V4 pool ${pool.ident} declares no price weights. Resolve pools ` + `through pool discovery, which supplies them.`;
  }
  const [priceIn, priceOut] = pool.prices;
  if (priceIn !== priceOut) {
    return `Sundae V4 pool ${pool.ident} weights its assets ` + `${priceIn}:${priceOut}; only equal weights are priced here`;
  }
  if (priceIn <= 0n) {
  ...
  if (pool.assetA.decimals !== pool.assetB.decimals) {
    return `Sundae V4 pool ${pool.ident} pairs assets of differing decimals ` + `(${pool.assetA.decimals} and ${pool.assetB.decimals}); the 1:1 ` + `constant-sum assumption does not hold`;
  }
  return undefined;
}
 ...
 * on a V4 pool.
 */
export function selectV4QuotePool(pools) {
  return pools.filter(pool => pool.version === EContractVersion.V4 && isSelectableV4Pool(pool)).reduce((best, pool) => best === undefined || pool.currentFee < best.currentFee ? pool : best, undefined);
}
function quoteConstantSumSwap(pool, suppliedAsset, receivedAsset, outputReserve) {
  const priceIn = constantSumPriceWeight(pool);
  if (outputReserve <= 0n) {
    throw new Error(`Sundae V4 pool ${pool.ident} holds no ${receivedAsset.assetId} to swap into`);
  }
  const output = constantSumSwapOutput(suppliedAsset.amount, pool.currentFee, priceIn);
  return {
...
export function quoteSwap({
  ...
  const receivesAssetA = SundaeUtils.isAssetIdsEqual(pool.assetB.assetId, canonicalSuppliedAsset.metadata.assetId);
...
export function quoteSwapInput({
  ...
  if (pool.version === EContractVersion.V4) {
    ...
    return {
      requiredInput: new AssetAmount(constantSumSwapInput(canonicalDesiredOutput.amount, pool.currentFee, constantSumPriceWeight(pool)), suppliedAsset)
    };
  }
  return {
    requiredInput: new AssetAmount(SundaeUtils.getSwapInput(pool, canonicalDesiredOutput).input, suppliedAsset)
  };
}
 ...
 */
export async function buildSwapOrderTx(blaze, params) {
  ...
  const suppliedAsset = canonicalizePoolAsset(params.pool, params.suppliedAsset);
  const swapType = canonicalizeSwapType(params.pool, suppliedAsset, params.swapType);
  const builderSwapType = swapType.type === ESwapType.MARKET ? {
    type: ESwapType.LIMIT,
    minReceivable: minReceivableFromSlippage(SundaeUtils.getSwapOutput(params.pool, suppliedAsset).output, oppositePoolAsset(params.pool, suppliedAsset), swapType.slippage)
  } : swapType;
 ...
 */
export async function buildSwapIntentTx(blaze, params) {
  ...
  if (SundaeUtils.isAssetIdsEqual(params.offered.metadata.assetId, params.minReceived.metadata.assetId)) {
    throw new Error("Sundae V4 swap requires the offered and received assets to differ");
  }
  const builder = SundaeSDK.new({
    blazeInstance: blaze
  }).builder(EContractVersion.V4);
 ...
 */
export async function buildSwapToStakeOrderTx(blaze, {
  sdk,
  swap,
  stake
}) {
  ...
  const swapType = canonicalizeSwapType(swap.pool, suppliedAsset, swap.swapType);
  const minReceived = swapType.type === ESwapType.LIMIT ? swapType.minReceivable : quoteSwap({
    pool: swap.pool,
    suppliedAsset,
  ...
  const continuation = await sdk.buildStakeContinuation({
    ...stake,
    owner: stake?.owner ?? realfiOwnerFromSundaeV3OwnerAddress(Core.Address.fromBech32(swap.ownerAddress).getNetworkId() === Core.NetworkId.Mainnet ? "mainnet" : "preview", swap.ownerAddress),
    swap: {
      minReceived
    },
  ...
  return buildSwapOrderTx(blaze, {
    ...swap,
    orderAddresses: {
      DestinationAddress: sundaeV3DestinationAddressFromStepResult(continuation),
  ...
  const realfiDatum = (() => {
    switch (datum.type) {
      ...
      case EDatumType.INLINE:
        return {
          InlineDatum: [plutusDataFromCbor(datum.value)]
        };
      default:
        {
          const unreachable = datum;
          return unreachable;
        }
    }
  })();
  return addressToRealFiDestination(Core.Address.fromBech32(address), realfiDatum);
}
