...
import { KEYS, ADDRESSES, UTXOS } from '../fixtures/kas.fixtures';

const coinConfig = coins.get('kas');
...
describe('Kaspa — End-to-End Transaction Flow', function () {
  it('should build, sign, serialize, deserialize, and verify a simple transaction', async function () {
    ...
    // Step 4: Serialize to broadcast format (JSON string for Kaspa)
    ...
    // Step 5: Serialize to hex (for storage/transport)
    const hex = tx.toHex();
  ...
  it('should NOT be considered fully signed when unsigned', async function () {
    ...
    // Unsigned: all signatureScript fields should be empty
    assert.ok(
      sigs.every((s) => s === ''),
      'unsigned transaction should have empty signature scripts'
    );

    // toBroadcastFormat still works but produces an unsigned payload
  ...
  it('should produce a valid RPC-submittable JSON broadcast payload', async function () {
    ...
    // Required Kaspa transaction fields
    ...
    // Each signed input must have a signatureScript
