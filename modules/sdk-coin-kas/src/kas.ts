...
export class Kas extends BaseCoin {
  ...
  static createInstance(bitgo: BitGoBase, staticsCoin?: Readonly<StaticsBaseCoin>): BaseCoin {
    return new Kas(bitgo, staticsCoin);
  }
  ...
  /**
   * Return the base factor (sompi per KAS).
  ...
  generateKeyPair(seed?: Buffer): IKeyPair {
    const keyPair = seed ? new KasLib.KeyPair({ seed }) : new KasLib.KeyPair();
  ...
  async isWalletAddress(params: VerifyAddressOptions): Promise<boolean> {
    ...
    const derivedAddress = new KasLib.KeyPair({ pub: keychains[0].pub }).getAddress(networkType);
  ...
  private getBuilder(): KasLib.TransactionBuilderFactory {
    return new KasLib.TransactionBuilderFactory(coins.get(this.getChain()));
  }
  ...
  async verifyTransaction(params: KaspaVerifyTransactionOptions): Promise<boolean> {
    ...
    let tx: KasLib.Transaction;
    try {
      const txBuilder = this.getBuilder().from(txHex);
      tx = (await txBuilder.build()) as KasLib.Transaction;
    } catch (error) {
  ...
  async explainTransaction(params: KaspaExplainTransactionOptions): Promise<TransactionExplanation> {
    ...
    try {
      const txBuilder = this.getBuilder().from(txHex);
      const tx = (await txBuilder.build()) as KasLib.Transaction;
   ...
   *
   * Each input is signed with the provided private key. The transaction is
   * considered fully signed when all inputs have a valid signatureScript.
   */
