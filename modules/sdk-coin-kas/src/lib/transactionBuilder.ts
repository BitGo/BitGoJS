import { BaseTransactionBuilder, BaseTransaction } from '@bitgo/sdk-core';
...
export class TransactionBuilder extends BaseTransactionBuilder {
  ...
  protected fromImplementation(rawTransaction: string): Transaction {
    const tx = Transaction.fromHex((this as any)._coinConfig?.name || 'kas', rawTransaction);
  ...
  protected async buildImplementation(): Promise<BaseTransaction> {
    ...
    this._transaction = new Transaction((this as any)._coinConfig?.name || 'kas', txData);
    return this._transaction;
  }

  /** @inheritDoc */
  protected signImplementation(key: any): BaseTransaction {
    // Signing will be performed outside via signTransaction on the coin class
    return this._transaction;
  }
  ...
  validateRawTransaction(rawTransaction: string): void {
    try {
      Transaction.fromHex((this as any)._coinConfig?.name || 'kas', rawTransaction);
    } catch {
