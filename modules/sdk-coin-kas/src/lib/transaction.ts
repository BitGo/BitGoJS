...
export class Transaction extends BaseTransaction {
   ...
   *
   * Each input's `scriptPublicKey` must be populated (hex-encoded P2PK script).
   * After signing, the 65-byte signature (64-byte Schnorr || 1-byte sighash type)
   * is stored in `input.signatureScript` as a hex string.
   *
