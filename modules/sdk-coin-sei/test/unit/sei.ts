...
describe('SEI', function () {
  ...
  describe('Parse Transactions: ', () => {
    ...
    it('should fail to parse a transfer transaction when explainTransaction response is undefined', async function () {
      const stub = sinon.stub(Sei.prototype, 'explainTransaction');
  ...
  describe('Recover transaction: success path', () => {
    ...
    beforeEach(() => {
      const accountBalance = sandBox.stub(Sei.prototype, 'getAccountBalance' as keyof Sei);
      accountBalance.withArgs(wrwUser.senderAddress).resolves(testBalance);

      const accountDetails = sandBox.stub(Sei.prototype, 'getAccountDetails' as keyof Sei);
      accountDetails.withArgs(wrwUser.senderAddress).resolves([testAccountNumber, testSequenceNumber]);

      const chainId = sandBox.stub(Sei.prototype, 'getChainId' as keyof Sei);
