...
import { DEFAULT_FEE, DEFAULT_STORAGE_LIMIT, generateDataToSign, isValidOriginatedAddress } from './lib/utils';
...
export class Xtz extends BaseCoin {
  ...
  /**
   * Queries public block explorer to get the next XTZ coin's nonce that should be used for the given address
   * @param {string} address
   * @param {string} apiKey - optional API key to use instead of the one from the environment
   * @returns {Promise<number>}
   */
  async getAddressNonce(address: string, apiKey?: string): Promise<BigNumber> {
    let parameterAddress;
    if (isValidOriginatedAddress(address)) {
      const contractDetails: any = await this.recoveryBlockchainExplorerQuery({
        actionPath: 'contracts',
        address,
      });
      if (!contractDetails || !contractDetails.creator || !contractDetails?.creator?.address) {
        throw new Error('Unable to find contract details');
      }

      parameterAddress = contractDetails?.creator?.address;
    } else {
    ...
    if (!result || !result.counter) {
      throw new Error('Unable to find next nonce from TZKT, got: ' + JSON.stringify(result.counter));
    }
    const nonce = new BigNumber(result.counter, 10);
    return nonce;
  }
  ...
  async recover(params: RecoverOptions): Promise<unknown> {
    ...
    const gasLimit = isValidOriginatedAddress(params.recoveryDestination)
      ? DEFAULT_GAS_LIMIT.CONTRACT_TRANSFER
      : DEFAULT_GAS_LIMIT.TRANSFER;
    const gasPrice = DEFAULT_FEE.TRANSFER;
    ...
    const feeInfo = {
      fee: new BigNumber(DEFAULT_FEE.TRANSFER),
      gasLimit: new BigNumber(gasLimit),
      storageLimit: new BigNumber(DEFAULT_STORAGE_LIMIT.TRANSFER),
    };
    ...
    if (!backupAddressDetails.revealed) {
      feeInfo.fee = feeInfo.fee.plus(DEFAULT_FEE.REVEAL);
      feeInfo.gasLimit = feeInfo.gasLimit.plus(DEFAULT_GAS_LIMIT.REVEAL);
      feeInfo.storageLimit = feeInfo.storageLimit.plus(DEFAULT_STORAGE_LIMIT.REVEAL);
    ...
    const userAddressCounter = await this.getAddressNonce(params.walletContractAddress, params.apiKey);
    ...
    txBuilder
      ...
      .counter(userAddressCounter.plus(1).toString())
      .fee(DEFAULT_FEE.TRANSFER.toString())
      .storageLimit(DEFAULT_STORAGE_LIMIT.TRANSFER.toString())
