...
describe('HBAR Account Update Builder', () => {
  ...
  describe('should build', () => {
    describe('non serialized transactions', () => {
      ...
      it('a stake transaction without declineReward should omit declineReward from output', async () => {
        const builder = initTxBuilder();
        builder.node({ nodeId: '0.0.2345' });
        builder.startTime('1596110493.372646570');
        const tx = await builder.build();
        const txJson = tx.toJson();
        should.not.exist(txJson.instructionsData.params.declineReward);
        txJson.instructionsData.params.stakedNodeId.should.deepEqual(NODE_ID.toString());
      });

      it('a full unstake transaction with stakedNodeId -1 and declineStakingReward true', async () => {
        const txBuilder = factory.getAccountUpdateBuilder();
        txBuilder.fee({ fee: testData.FEE });
        txBuilder.source({ address: testData.ACCOUNT_1.accountId });
        txBuilder.stakedNodeId(-1);
  ...
  describe('should fail', () => {
    ...
    it('a stake transaction with an invalid stakedNodeId less than -1', () => {
      const txBuilder = factory.getAccountUpdateBuilder();
      assert.throws(
        () => txBuilder.stakedNodeId(-2),
        (e: any) => e.message === 'Invalid stakedNodeId: only -1 is a valid sentinel for unstaking'
      );
    });
