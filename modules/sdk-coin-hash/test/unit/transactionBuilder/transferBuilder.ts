...
describe('Hash Transfer Builder', () => {
...
describe('Hash Token Transfer Builder', () => {
  let bitgo: TestBitGoAPI;
  let basecoin;
  let factory;
  let testTx;
  ...
  before(function () {
    bitgo = TestBitGo.decorate(BitGoAPI, { env: 'mock' });
    bitgo.safeRegister('hash:ylds', Hash.createInstance);
    bitgo.safeRegister('thash:ylds', Thash.createInstance);
    bitgo.initializeTestVars();
    basecoin = bitgo.coin('thash:ylds');
    factory = basecoin.getBuilder();
    testTx = testData.TEST_SEND_TOKEN_TX;
    testTxWithMemo = testData.TEST_TOKEN_TX_WITH_MEMO;
  });
  ...
  it('should build a Transfer tx with signature', async function () {
    ...
    should.deepEqual(tx.inputs, [
      {
        address: testData.TEST_SEND_TOKEN_TX.sender,
        value: testData.TEST_SEND_TOKEN_TX.sendMessage.value.amount[0].amount,
        coin: basecoin.getChain(),
      },
    ]);
    should.deepEqual(tx.outputs, [
      {
        address: testData.TEST_SEND_TOKEN_TX.sendMessage.value.toAddress,
        value: testData.TEST_SEND_TOKEN_TX.sendMessage.value.amount[0].amount,
        coin: basecoin.getChain(),
      },
    ]);
